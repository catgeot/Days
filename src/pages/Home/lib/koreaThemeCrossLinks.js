...
function isOngjinFallbackMismatch(hubId, stayHubId) {
  return normId(hubId) === 'ongjin' && normId(stayHubId) !== 'ongjin';
}

/**
 * 같은 시도 시드 도시 — 거리(120km) 밖 섬·군(대청도)도 인천·강화 숙소·투어 폴백.
 * 주소가 옹진이 아니면 옹진은 제외(왕가의 산책).
 */
export function areaSeedStayHubs(areaCode, stayHubId) {
  if (!areaCode) return [];
  const stayId = normId(stayHubId);
  const out = [];
  const seen = new Set();
  for (const id of hubIdsForArea(areaCode) || []) {
    const hid = normId(id);
...
function omitBannedKeywords(keywords, banned) {
  const ban = new Set();
  for (const raw of banned || []) {
    const s = String(raw || '').trim().toLowerCase();
    if (s) ban.add(s);
  }
  if (!ban.size) return Array.isArray(keywords) ? keywords.filter(Boolean) : [];
  return (keywords || []).filter((k) => !ban.has(String(k || '').trim().toLowerCase()));
}
...
/**
 * 숙소 없을 때 상위·인접 시드 도시 칩 (행사명 제외).
 * @param {{
 *   stay?: { keyword?: string, altKeywords?: string[], location?: { hubId?: string } },
 *   nearbyHubs?: Array<{ hubId?: string, name?: string }>,
 * }} cross
 */
export function buildFestivalStayAreas(cross) {
  const areas = [];
  const seen = new Set();
  ...
  const push = (name, keyword, hubId) => {
    ...
    const kw = String(keyword || label).trim();
    if (!kw || !/[가-힣]/.test(kw)) return;
  ...
  const stayKw = String(cross?.stay?.keyword || '').trim();
  if (stayKw) {
    const hub = stayId ? resolveCityAttractionHub(stayId) : null;
    push(hub?.name || stayKw, stayKw, stayId);
  }
  const areaCode =
    areaCodeForHubId(stayId) ||
    areaCodeForHubId(cross?.nearbyHubs?.[0]?.hubId);
  const seedHubs = areaSeedStayHubs(areaCode, stayId);
...
export function resolveThemeCrossLinks(spot, opts = {}) {
  ...
  const fallbackHubs = [...nearbyHubs, ...areaSeedStayHubs(areaCode, stayHubId)];
  ...
  const tnaAlts = tnaQ
    ...
    : [];
  const tnaNearby = tnaQ
    ? mergeAltKeywords(tnaQ.nearbyKeywords, fallbackKw, tnaQ.keyword)
    : [];
  ...
  const bundle = {
    ...
    tna: tnaQ
      ? {
          ...
          altKeywords: tnaAlts,
          nearbyKeywords: tnaNearby,
          location,
        }
      : null,
    packageCta,
  };
  bundle.stayAreas = buildFestivalStayAreas(bundle);
  return bundle;
}
...
export function resolveFestivalThemeCrossLinks(item, opts = {}) {
  ...
  const cross = resolveThemeCrossLinks(
    {
      ...
      areaCode,
      region: opts.region,
      name: nearestHub?.name || nearestHubId,
      nameEn: nearestHub?.name_en,
      addr1: item.addr1,
      addr2: item.addr2,
      lat: pt?.lat,
      lng: pt?.lng,
  ...
  if (cross.tna) {
    if (eventTitle && isSameKeyword(cross.tna.keyword, eventTitle)) {
