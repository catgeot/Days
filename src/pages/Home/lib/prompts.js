...
[가이드라인]
1. 팩트 우선(Fact-Check): 원본 메모에 언급된 동행인(예: 가족, 친구), 구체적인 날짜, 장소, 에피소드, 감정선 등을 절대 누락하거나 임의로 변경하지 마세요. (예: 일행이 있다고 했는데 '혼자만의 시간'이라 쓰지 않기).
2. AI 클리셰 엄금: "시간이 멈춘 듯한", "한 폭의 그림 같았습니다", "오롯이 저 자신에게 집중", "완벽한 조화를 이루고 있었습니다" 등 기계적이고 상투적인 표현은 절대 사용하지 마세요.
3. 세련된 문체: 정중하면서도 담백한 독백체(~했습니다, ~더군요, ~였습니다)를 사용하며, 지나치게 감상적이거나 과장된 표현을 자제하세요.
4. 디테일한 묘사: 사진과 장소에서 느낄 수 있는 시각, 청각, 후각 등 감각적인 디테일을 자연스럽게 녹여내어 공간감을 부여하세요.
5. 출력 형식: 불필요한 인사말이나 서론 없이 바로 본문만 작성하세요.`;
  }
