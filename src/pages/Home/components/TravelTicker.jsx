...
// ?ö® [Fix] Dumb Component?? ?∏Î??êÏÑú ?∞Ïù¥?∞Î? Ï£ºÏûÖÎ∞õÎèÑÎ°?Î≥ÄÍ≤?
...
// ?ö® [Change] props??'data' Ï∂îÍ? (Í∏∞Î≥∏Í∞?Îπ?Î∞∞Ïó¥)
export default function TravelTicker({ data = [], onCityClick, isExpanded: externalExpanded, onToggle }) {
  ...
  // ?ö® [Fix] ?∞Ïù¥?∞Í? Î∞îÎÄ????∏Îç±??Ï¥àÍ∏∞??Î∞©Ï? Î°úÏßÅ ?ÑÏöî?òÎ©¥ Ï∂îÍ?
  const cities = data.length > 0 ? data : []; 

  useEffect(() => {
    if (cities.length === 0) return; // ?∞Ïù¥???ÜÏúºÎ©?Î£®ÌîÑ ????
  ...
  }, [isExpanded, cities.length]); // cities.length ?òÏ°¥??Ï∂îÍ?
  ...
  // ?ö® [Fix] ?∞Ïù¥?∞Í? ?ÑÏòà ?ÜÏùÑ ??Î°úÎî© ?? ?åÎçîÎß?Î∞©Ï?
