...
// // ?ö® [Fix] ?∞Ïù¥???êÎ¶Ñ ?òÏ†ï
// // usePlaceGallery??name(string) ?Ä??location(object) ?ÑÏ≤¥Î•??ÑÎã¨?òÏó¨
// // Hook ?¥Î??êÏÑú ?çÎ????∞Ïù¥??Íµ??Î™? ?ÅÎ¨∏Î™???Î•??úÏö©?????àÍ≤å ??
...
//   // ?ö® [Hook Connection] Ïª§Ïä§?Ä ???∞Í≤∞ ?òÏ†ï
//   // Í∏∞Ï°¥: usePlaceGallery(location?.name) -> Î¨∏Ïûê?¥Îßå ?ÑÎã¨??
//   // ?òÏ†ï: usePlaceGallery(location) -> Í∞ùÏ≤¥ ?ÑÏ≤¥ ?ÑÎã¨ (Unsplash Í≤Ä???ïÌôï???•ÏÉÅ)
...
//   // --- [Mode 1] ?ÅÏÑ∏ Î≥¥Í∏∞ (Deep Dive) - Ï°∞Î¶Ω ?åÌä∏ ---
//   if (isExpanded) {
//     // ?ö® [Note] ?¥Î? ?∞Ïù¥??Îß§Ïπ≠ ?ïÏù∏ (UI ?úÏãú?? ?ÑÏàò???ÑÎãò)
...
//                       "<span className="text-white font-medium">{selectedImg.user.name}</span> ?ëÍ????úÏÑ†.<br/>
//                       {selectedImg.alt_description ? ` '${selectedImg.alt_description}'??Î∂ÑÏúÑÍ∏∞Í? ?êÍª¥ÏßÄ??` : ''}
//                       ???•ÏÜå?êÏÑú ?πÎ≥Ñ???ÅÍ∞ê??Î∞õÏïÑÎ≥¥ÏÑ∏??"
//                     </p>
//                  </div>
//                </div>
//             ) : isChatMode ? (
//                // [Case B] Chat View (Component ?¨Ïö©)
//                <PlaceChatView 
...
//                     <p className="text-gray-300 text-sm leading-relaxed font-light">
//                       {location.country}??Î≥¥ÏÑù, <span className="text-blue-300 font-medium">{location.name}</span>.<br/>
//                       ?¥Í≥≥???®Í≤®Ïß?Îß§Î†•??Î∞úÍ≤¨?òÍ≥† AI?Ä ?®Íªò ?ÑÎ≤Ω???¨Ìñâ??Í≥ÑÌöç?¥Î≥¥?∏Ïöî.
//                     </p>
//                   </div>
//                   <button 
...
//                   >
//                     <MessageSquare size={16} /> AI?êÍ≤å ?¨Ìñâ ?ïÎ≥¥ Î¨ºÏñ¥Î≥¥Í∏∞
//                   </button>
//                </div>
//             )}
//           </div>
...
//   // --- [Mode 2 & 3] Compact & Card Mode (Í∏∞Ï°¥ ?†Ï?) ---
//   if (isCompactMode) {
//     return (
...
//               <p className="text-xs text-gray-300 leading-relaxed font-light line-clamp-3">{location.name}???®Í≤®Ïß?Îß§Î†•??Î∞úÍ≤¨?òÏÑ∏?? Ïπ¥ÎìúÎ•??¥Î¶≠?òÎ©¥ Í≥†ÌôîÏß?Í∞§Îü¨Î¶¨Ï? AI Í∞Ä?¥ÎìúÍ∞Ä ?úÏûë?©Îãà??</p>
//             </div>
//           )}
//         </div>
...
//              <span className="text-xs font-bold text-gray-200">AI Î¨ªÍ∏∞</span>
//            </button>
...
//              <span className="text-xs font-bold text-white">?¨Ìñâ Í≥ÑÌöç</span>
//            </button>
//         </div>
//       </div>
//     </div>
