...
export default function FlightCinemaAirportMarkers({ routeIatas = [] }) {
  const markers = useMemo(() => buildFlightCinemaAirportMarkers(routeIatas), [routeIatas]);
  const routeKey = useMemo(
    () => markers.map(({ iata }) => iata).join('>'),
    [markers]
  );
  ...
  return markers
    ...
    .map(({ iata, lng, lat, role }, index) => (
      <Marker
        key={`${routeKey}-${iata}-${index}`}
        longitude={lng}
        latitude={lat}
        anchor="left"
