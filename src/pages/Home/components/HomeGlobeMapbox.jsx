    ...
    >
      ...
      {globeMode === GLOBE_MODE.TOUR_READY && (reachBoundariesReady || reachBoundariesLoading) && !isZenMode && typeof document !== 'undefined' && createPortal(
        <div
          ...
          role="region"
          aria-label={t('home.globe.reachBoundary.title')}
        >
          <div className="pointer-events-auto rounded-2xl border border-white/10 bg-black/60 px-3 py-2.5 text-[11px] text-white/85 shadow-lg backdrop-blur-sm">
            <div className="mb-1.5 flex items-center justify-between gap-3">
              <p className="min-w-0 flex-1 font-bold tracking-wide text-white/95">{t('home.globe.reachBoundary.title')}</p>
              <button
                type="button"
                role="switch"
                aria-checked={reachBoundariesVisible}
                aria-label={reachBoundariesVisible ? t('home.globe.reachBoundary.hide') : t('home.globe.reachBoundary.show')}
            ...
            <div className={reachBoundariesVisible ? '' : 'opacity-45'}>
              <div className="flex items-center gap-2">
                ...
                <span>{t('home.globe.reachBoundary.walkMinutes', { minutes: REACH_CONTOUR_MINUTES.walk })}</span>
              </div>
              <div className="mt-1 flex items-center gap-2">
                <span className="inline-block h-0.5 w-4 border-t-2 border-solid border-blue-400 shrink-0" aria-hidden="true" />
                <span>{t('home.globe.reachBoundary.driveMinutes', { minutes: REACH_CONTOUR_MINUTES.drive })}</span>
              </div>
            </div>
            {reachBoundariesLoading && (
              <p className="mt-1.5 text-[10px] text-white/55">{t('home.globe.reachBoundary.computing')}</p>
            )}
            {reachBoundariesReady && !reachBoundariesLoading && (
              <p className="mt-1.5 text-[10px] text-white/55">
                {reachBoundariesVisible ? t('home.globe.reachBoundary.hintVisible') : t('home.globe.reachBoundary.hintHidden')}
              </p>
            )}
          </div>
        </div>,
