...
// ?ö® [Fix] MessageSquare import Ï∂îÍ?
...
const TicketModal = ({ 
  ...
  scoutedPins, 
  savedTrips = [] // Home?êÏÑú ?ÑÎã¨Î∞õÏ? ?Ä???¥Î†•
}) => {
      ...
      <div className="bg-[#1a1a1a] border border-white/10 w-full max-w-4xl h-[600px] rounded-3xl shadow-2xl flex overflow-hidden">
        ...
        <div className="w-1/3 bg-black/30 border-r border-white/5 p-6 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
            {savedTrips.length === 0 ? (
                <div className="text-center text-gray-600 text-xs py-10">?Ä??Í∏∞Î°ù???ÜÏäµ?àÎã§.</div>
            ) : (
                savedTrips.map((trip) => (
                  <button 
                    key={trip.id}
        ...
        <div className="w-2/3 p-8 relative">
           ...
           <p className="text-sm text-gray-400 mb-8">?¨Ìñâ Í≥ÑÌöç???ïÏ†ï?òÍ≥† ?∞Ïºì??Î∞úÍ∂å?òÏÑ∏??</p>
           ...
           <button onClick={() => { onIssue({ text: `${destination} ?¨Ìñâ Í≥ÑÌöç ?∏ÏõåÏ§? }); onClose(); }} className="absolute bottom-8 right-8 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-4 rounded-full font-bold shadow-lg hover:scale-105 transition-transform flex items-center gap-2">
             <Ticket size={18} /> Î∞úÍ∂å?òÍ∏∞ (Issue Ticket)
           </button>
        </div>
      </div>
    </div>
