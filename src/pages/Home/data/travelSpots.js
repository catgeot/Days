/**
 * Travel Spots Data - Globe Display Rebalanced
 *
 * Total: 200개
 * Globe Display: 150개 (75.0%)
 *
 * Globe Display Strategy: "Special Destinations First"
 * - Tier 1 Common Cities: 36/50개 (72.0%)
 * - Tier 2 Special Places: 92/128개 (71.9%)
 * - Tier 3 Hidden Gems: 22/22개 (100.0%)
 *
 * Last Updated: 2026-03-31
 */

export const TRAVEL_SPOTS = [
  {
    "id": 101,
    "slug": "bora-bora",
    "name": "보라보라",
    "name_en": "Bora Bora",
    "country": "프랑스령 폴리네시아",
    "country_en": "French Polynesia",
    "lat": -16.5,
    "lng": -151.74,
    "tier": 2,
    "popularity": 75,
    "continent": "oceania",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남태평양의 진주라 불리는 지구상에서 가장 낭만적인 섬입니다. 맑고 투명한 라군 위에 지어진 오버워터 방갈로에서 완벽한 휴식을 취할 수 있습니다. 상어와 가오리와 함께 수영하는 경이로운 경험을 놓치지 마세요.",
    "keywords": [
      "오세아니아",
      "라군",
      "방갈로",
      "신혼여행"
    ]
  },
  {
    "id": 102,
    "slug": "santorini",
    "name": "산토리니",
    "name_en": "Santorini",
    "country": "그리스",
    "country_en": "Greece",
    "lat": 36.39,
    "lng": 25.46,
    "tier": 2,
    "popularity": 69,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "하얀 외벽과 파란 지붕이 코발트빛 지중해와 눈부신 대비를 이루는 낭만의 섬입니다. 절벽 위에서 바라보는 붉은 일몰은 전 세계 여행자들의 버킷리스트로 꼽힙니다. 골목골목 숨겨진 아름다운 카페와 상점들을 탐험해 보세요.",
    "keywords": [
      "유럽",
      "지중해",
      "일몰",
      "화이트"
    ]
  },
  {
    "id": 103,
    "slug": "cancun",
    "name": "칸쿤",
    "name_en": "Cancun",
    "country": "멕시코",
    "country_en": "Mexico",
    "lat": 21.16,
    "lng": -86.85,
    "tier": 2,
    "popularity": 67,
    "continent": "north_america",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "카리브해의 매혹적인 청록색 바다와 마야 문명의 신비로움이 공존하는 북미 최고의 휴양지입니다. 올인클루시브 리조트에서의 여유로운 시간은 물론, 천연 우물인 세노테에서의 신비로운 수영도 즐길 수 있습니다.",
    "keywords": [
      "북미",
      "카리브해",
      "세노테",
      "올인클루시브"
    ]
  },
  {
    "id": 104,
    "slug": "maldives",
    "name": "몰디브",
    "name_en": "Maldives",
    "country": "몰디브",
    "country_en": "Maldives",
    "lat": 3.2,
    "lng": 73.22,
    "tier": 2,
    "popularity": 78,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "인도양에 흩뿌려진 수천 개의 산호섬으로 이루어진 궁극의 럭셔리 휴양지입니다. 섬 하나에 리조트 하나가 자리하여 완벽한 프라이버시를 보장받을 수 있습니다. 객실에서 바로 투명한 바다로 뛰어드는 로망을 실현해 보세요.",
    "keywords": [
      "아시아",
      "인도양",
      "럭셔리",
      "수중환경"
    ]
  },
  {
    "id": 105,
    "slug": "seychelles",
    "name": "세이셸",
    "name_en": "Seychelles",
    "country": "세이셸",
    "country_en": "Seychelles",
    "lat": -4.67,
    "lng": 55.49,
    "tier": 2,
    "popularity": 79,
    "continent": "africa",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아프리카 인도양의 최후의 낙원이라 불리며, 거대한 화강암 바위들이 해변을 감싸고 있는 독특한 절경을 자랑합니다. 사람의 손길이 닿지 않은 원시림과 에메랄드빛 바다가 완벽한 조화를 이룹니다.",
    "keywords": [
      "아프리카",
      "원시림",
      "해변",
      "휴양"
    ]
  },
  {
    "id": 106,
    "slug": "fernando-de-noronha",
    "name": "페르난두지노로냐",
    "name_en": "Fernando de Noronha",
    "country": "브라질",
    "country_en": "Brazil",
    "lat": -3.84,
    "lng": -32.42,
    "tier": 2,
    "popularity": 74,
    "continent": "south_america",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남미 브라질 연안에 위치한 엄격히 보호받는 해양 국립공원입니다. 돌고래 떼와 바다거북을 일상처럼 마주할 수 있으며, 투명한 바다와 야생 그대로의 자연이 경이로움을 선사합니다.",
    "keywords": [
      "남미",
      "에코투어",
      "바다거북",
      "돌고래"
    ]
  },
  {
    "id": 107,
    "slug": "bermuda",
    "name": "버뮤다",
    "name_en": "Bermuda",
    "country": "버뮤다",
    "country_en": "Bermuda",
    "lat": 32.3078,
    "lng": -64.7505,
    "tier": 2,
    "popularity": 69,
    "continent": "north_america",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "북대서양의 미스터리한 전설을 품은 섬입니다. 실제로는 분홍빛 모래사장인 핑크 샌드 비치와 파스텔톤 주택들이 그림처럼 아름다운 곳입니다.",
    "keywords": [
      "대륙",
      "휴양",
      "낭만"
    ]
  },
  {
    "id": 108,
    "slug": "azores",
    "name": "아조레스 제도",
    "name_en": "Azores",
    "country": "포르투갈",
    "country_en": "Portugal",
    "lat": 37.7412,
    "lng": -25.6756,
    "tier": 2,
    "popularity": 67,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "대서양 한가운데 떠 있는 포르투갈령 화산섬입니다. 에메랄드빛 칼데라 호수와 푸른 초원이 빚어내는 유럽의 하와이입니다.",
    "keywords": [
      "대륙",
      "자연/오지",
      "휴양"
    ]
  },
  {
    "id": 109,
    "slug": "zanzibar",
    "name": "잔지바르",
    "name_en": "Zanzibar",
    "country": "탄자니아",
    "country_en": "Tanzania",
    "lat": -6.1659,
    "lng": 39.2026,
    "tier": 2,
    "popularity": 79,
    "continent": "africa",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아프리카 탄자니아 앞바다에 떠 있는 향신료의 섬입니다. 아랍풍의 구시가지 스톤타운과 새하얀 백사장이 묘한 조화를 이룹니다.",
    "keywords": [
      "대륙",
      "휴양",
      "역사/문화",
      "낭만"
    ]
  },
  {
    "id": 110,
    "slug": "rarotonga",
    "name": "라로통가",
    "name_en": "Rarotonga",
    "country": "쿡 제도",
    "country_en": "Cook Islands",
    "lat": -21.2292,
    "lng": -159.7764,
    "tier": 2,
    "popularity": 78,
    "continent": "oceania",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남태평양 쿡 제도의 심장부로, 화산 지형과 맑은 라군이 빚어내는 풍경이 압도적인 영감의 원천입니다.",
    "keywords": [
      "휴양",
      "자연/오지",
      "낭만"
    ]
  },
  {
    "id": 111,
    "slug": "samoa",
    "name": "사모아",
    "name_en": "Samoa",
    "country": "사모아",
    "country_en": "Samoa",
    "lat": -13.759,
    "lng": -172.1046,
    "tier": 2,
    "popularity": 73,
    "continent": "oceania",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "폴리네시아의 심장으로 불리며, 울창한 우림 속 숨겨진 폭포와 자연이 만든 천연 수영장 토 수아(To Sua)로 유명합니다.",
    "keywords": [
      "자연/오지",
      "휴양"
    ]
  },
  {
    "id": 112,
    "slug": "palawan",
    "name": "팔라완",
    "name_en": "Palawan",
    "country": "필리핀",
    "country_en": "Philippines",
    "lat": 9.8349,
    "lng": 118.7384,
    "tier": 2,
    "popularity": 70,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": "southeast-asia",
    "desc": "필리핀 최후의 비경으로, 석회암 암벽과 지하 강, 투명한 라군이 환상적인 절경을 이룹니다. 엘니도 투어가 특히 유명합니다.",
    "keywords": [
      "휴양",
      "자연/오지",
      "액티비티"
    ]
  },
  {
    "id": 113,
    "slug": "canary-islands",
    "name": "카나리아 제도",
    "name_en": "Canary Islands",
    "country": "스페인",
    "country_en": "Spain",
    "lat": 28.2916,
    "lng": -16.6291,
    "tier": 2,
    "popularity": 77,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "유럽인들의 따뜻한 겨울 피난처입니다. 달 표면 같은 화산 지형과 매혹적인 스페인 문화가 어우러진 매력적인 섬들입니다.",
    "keywords": [
      "휴양",
      "자연/오지"
    ]
  },
  {
    "id": 114,
    "slug": "bali",
    "name": "발리",
    "name_en": "Bali",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -8.4095,
    "lng": 115.1889,
    "tier": 2,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": "southeast-asia",
    "desc": "신들의 섬이라 불리는 전 세계 서퍼와 요기(Yogi)들의 안식처입니다. 힌두교 문화와 트렌디한 비치 클럽이 완벽하게 공존합니다.",
    "keywords": [
      "휴양",
      "액티비티",
      "낭만"
    ]
  },
  {
    "id": 115,
    "slug": "hawaii",
    "name": "하와이",
    "name_en": "Hawaii",
    "country": "하와이",
    "country_en": "Hawaii",
    "lat": 19.8968,
    "lng": -155.5828,
    "tier": 2,
    "popularity": 69,
    "continent": "unknown",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "알로하 정신이 살아 숨 쉬는 북태평양의 지상 낙원입니다. 와이키키 해변의 활기와 거대한 화산 국립공원의 경이로움이 공존합니다.",
    "keywords": [
      "휴양",
      "자연/오지",
      "액티비티"
    ]
  },
  {
    "id": 116,
    "slug": "guam",
    "name": "괌",
    "name_en": "Guam",
    "country": "괌",
    "country_en": "Guam",
    "lat": 13.4443,
    "lng": 144.7937,
    "tier": 2,
    "popularity": 75,
    "continent": "unknown",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "투몬 비치의 눈부신 에메랄드빛 바다와 면세 쇼핑의 즐거움이 가득한 서태평양의 대표적인 가족 휴양지입니다.",
    "keywords": [
      "휴양",
      "액티비티"
    ]
  },
  {
    "id": 117,
    "slug": "banff-national-park",
    "name": "밴프 국립공원",
    "name_en": "Banff National Park",
    "country": "캐나다",
    "country_en": "Canada",
    "lat": 51.17,
    "lng": -115.57,
    "tier": 2,
    "popularity": 74,
    "continent": "north_america",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "북미 로키산맥의 심장부로, 에메랄드빛 루이스 호수와 웅장한 빙하가 장관을 이룹니다. 대자연 속에서 엘크와 곰 같은 야생동물과 조우하며 하이킹, 카누 등 다양한 아웃도어 활동을 즐기기에 최적입니다.",
    "keywords": [
      "북미",
      "로키산맥",
      "호수",
      "하이킹"
    ]
  },
  {
    "id": 118,
    "slug": "iguazu-falls",
    "name": "이구아수 폭포",
    "name_en": "Iguazu Falls",
    "country": "아르헨티나",
    "country_en": "Argentina",
    "lat": -25.69,
    "lng": -54.43,
    "tier": 2,
    "popularity": 71,
    "continent": "south_america",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남미 대륙을 가로지르며 엄청난 굉음과 함께 쏟아져 내리는 세계 최대의 폭포입니다. '악마의 목구멍'이라 불리는 하이라이트 구간에 서면 대자연의 압도적인 힘과 경외감을 온몸으로 느낄 수 있습니다.",
    "keywords": [
      "남미",
      "폭포",
      "정글",
      "경이로움"
    ]
  },
  {
    "id": 119,
    "slug": "iceland",
    "name": "아이슬란드",
    "name_en": "Iceland",
    "country": "아이슬란드",
    "country_en": "Iceland",
    "lat": 64.96,
    "lng": -19.02,
    "tier": 2,
    "popularity": 71,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "불과 얼음의 나라로 불리며, 거대한 빙하와 활화산이 공존하는 유럽의 외딴섬입니다. 겨울에는 신비로운 오로라가 하늘을 수놓고, 여름에는 백야의 태양 아래 장엄한 폭포들이 빛을 발합니다.",
    "keywords": [
      "유럽",
      "오로라",
      "빙하",
      "폭포"
    ]
  },
  {
    "id": 120,
    "slug": "victoria-falls",
    "name": "빅토리아 폭포",
    "name_en": "Victoria Falls",
    "country": "잠비아",
    "country_en": "Zambia",
    "lat": -17.92,
    "lng": 25.85,
    "tier": 2,
    "popularity": 77,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아프리카 대륙을 가르는 잠베지 강이 빚어낸 거대한 물보라의 기적입니다. 현지어로 '천둥 치는 연기'라 불리며, 폭포를 내려다보는 헬기 투어나 악마의 수영장에서의 아찔한 경험을 즐길 수 있습니다.",
    "keywords": [
      "아프리카",
      "폭포",
      "무지개",
      "자연"
    ]
  },
  {
    "id": 121,
    "slug": "mount-fuji",
    "name": "후지산",
    "name_en": "Mount Fuji",
    "country": "일본",
    "country_en": "Japan",
    "lat": 35.36,
    "lng": 138.72,
    "tier": 2,
    "popularity": 79,
    "continent": "asia",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": "east-asia",
    "desc": "아시아를 대표하는 완벽한 대칭형의 웅장한 화산으로, 사계절 내내 각기 다른 아름다움을 뽐냅니다. 산봉우리에 내려앉은 만년설과 그 주변을 감싸는 평화로운 호수들이 빚어내는 풍경은 한 폭의 그림 같습니다.",
    "keywords": [
      "아시아",
      "화산",
      "하이킹",
      "풍경"
    ]
  },
  {
    "id": 122,
    "slug": "great-barrier-reef",
    "name": "그레이트 배리어 리프",
    "name_en": "Great Barrier Reef",
    "country": "호주",
    "country_en": "Australia",
    "lat": -18.28,
    "lng": 147.69,
    "tier": 2,
    "popularity": 74,
    "continent": "oceania",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "오세아니아 우주에서도 보인다는 세계 최대의 산호초 군락입니다. 수만 종의 해양 생물이 살아 숨 쉬는 바닷속 생태계의 보고로, 스쿠버 다이빙과 스노클링을 통해 신비로운 수중 세계를 탐험할 수 있습니다.",
    "keywords": [
      "오세아니아",
      "산호초",
      "다이빙",
      "바다"
    ]
  },
  {
    "id": 123,
    "slug": "andaman-islands",
    "name": "안다만 제도",
    "name_en": "Andaman Islands",
    "country": "인도",
    "country_en": "India",
    "lat": 11.7401,
    "lng": 92.6586,
    "tier": 2,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "인도양 한가운데 숨겨진 열대의 낙원으로, 문명의 때가 묻지 않은 순백의 해변과 울창한 맹그로브 숲을 자랑합니다.",
    "keywords": [
      "대륙",
      "휴양",
      "자연/오지"
    ]
  },
  {
    "id": 124,
    "slug": "palau",
    "name": "팔라우",
    "name_en": "Palau",
    "country": "팔라우",
    "country_en": "Palau",
    "lat": 7.515,
    "lng": 134.5825,
    "tier": 2,
    "popularity": 65,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "신들의 바다 정원이라 불리며, 세계 최고 수준의 수중 환경과 무독성 해파리 호수를 품고 있는 청정 국가입니다.",
    "keywords": [
      "대륙",
      "휴양",
      "액티비티",
      "자연/오지"
    ]
  },
  {
    "id": 125,
    "slug": "galapagos",
    "name": "갈라파고스",
    "name_en": "Galapagos",
    "country": "에콰도르",
    "country_en": "Ecuador",
    "lat": -0.9538,
    "lng": -90.9656,
    "tier": 2,
    "popularity": 73,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "찰스 다윈의 진화론을 탄생시킨 생물학적 기적의 섬들입니다. 사람을 피하지 않는 희귀 야생동물들과 함께 걷는 경이로운 경험을 제공합니다.",
    "keywords": [
      "자연/오지",
      "역사/문화",
      "액티비티"
    ]
  },
  {
    "id": 126,
    "slug": "mauritius",
    "name": "모리셔스",
    "name_en": "Mauritius",
    "country": "모리셔스",
    "country_en": "Mauritius",
    "lat": -20.3484,
    "lng": 57.5522,
    "tier": 2,
    "popularity": 66,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "톰 소여의 모험 작가 마크 트웨인이 신이 천국을 만들기 전 모델로 삼았다고 극찬한 인도양의 아름다운 화산섬입니다.",
    "keywords": [
      "휴양",
      "낭만",
      "자연/오지"
    ]
  },
  {
    "id": 127,
    "slug": "christmas-island",
    "name": "크리스마스 섬",
    "name_en": "Christmas Island",
    "country": "호주",
    "country_en": "Australia",
    "lat": -10.4475,
    "lng": 105.6904,
    "tier": 2,
    "popularity": 69,
    "continent": "oceania",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "매년 수천만 마리의 붉은 대게가 숲에서 바다로 대이동을 하는 장관을 연출하는 호주령의 고립된 생태 섬입니다.",
    "keywords": [
      "자연/오지"
    ]
  },
  {
    "id": 128,
    "slug": "diego-garcia",
    "name": "디에고 가르시아",
    "name_en": "Diego Garcia",
    "country": "영국령 인도양 지역",
    "country_en": "Indian Ocean",
    "lat": -7.3195,
    "lng": 72.4229,
    "tier": 2,
    "popularity": 73,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "인도양 중앙에 위치한 영국령 환초로, 거대한 V자 모양의 섬입니다. 군사 구역으로 지정되어 일반인의 출입이 제한된 비밀스러운 곳입니다.",
    "keywords": [
      "자연/오지"
    ]
  },
  {
    "id": 129,
    "slug": "lofoten",
    "name": "로포텐 제도",
    "name_en": "Lofoten",
    "country": "노르웨이",
    "country_en": "Norway",
    "lat": 68.2086,
    "lng": 13.552,
    "tier": 2,
    "popularity": 78,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "노르웨이 피오르드의 정점입니다. 날카로운 산봉우리 아래 그림처럼 자리 잡은 빨간 어촌 마을 위로 환상적인 오로라가 춤을 춥니다.",
    "keywords": [
      "자연/오지",
      "낭만"
    ]
  },
  {
    "id": 130,
    "slug": "sahara-desert",
    "name": "사하라 사막",
    "name_en": "Sahara Desert",
    "country": "사하라",
    "country_en": "Sahara",
    "lat": 23.4162,
    "lng": 25.6628,
    "tier": 2,
    "popularity": 65,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "끝을 알 수 없는 황금빛 모래 언덕이 파도처럼 일렁이는 세상에서 가장 넓은 사막입니다. 밤하늘에 쏟아지는 은하수는 숨을 멎게 합니다.",
    "keywords": [
      "자연/오지",
      "액티비티"
    ]
  },
  {
    "id": 131,
    "slug": "yellowknife",
    "name": "옐로나이프",
    "name_en": "Yellowknife",
    "country": "캐나다",
    "country_en": "Canada",
    "lat": 62.454,
    "lng": -114.3718,
    "tier": 2,
    "popularity": 72,
    "continent": "north_america",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "미항공우주국(NASA)이 선정한 세계 최고의 오로라 관측지입니다. 쏟아지는 빛의 커튼 아래서 잊지 못할 밤을 맞이합니다.",
    "keywords": [
      "자연/오지",
      "낭만",
      "액티비티"
    ]
  },
  {
    "id": 132,
    "slug": "uyuni-salt-flat",
    "name": "우유니 소금사막",
    "name_en": "Uyuni Salt Flat",
    "country": "볼리비아",
    "country_en": "Bolivia",
    "lat": -20.1338,
    "lng": -67.4891,
    "tier": 2,
    "popularity": 73,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "우기가 되면 세상에서 가장 거대한 거울로 변신하는 볼리비아의 소금 사막입니다. 하늘과 땅의 경계가 무너지는 기적을 경험합니다.",
    "keywords": [
      "자연/오지",
      "낭만",
      "액티비티"
    ]
  },
  {
    "id": 133,
    "slug": "new-york",
    "name": "뉴욕",
    "name_en": "New York",
    "country": "미국",
    "country_en": "USA",
    "lat": 40.71,
    "lng": -74,
    "tier": 1,
    "popularity": 88,
    "continent": "unknown",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "us-east-coast",
    "desc": "북미를 넘어 전 세계의 문화, 패션, 금융이 교차하는 진정한 세계의 수도입니다. 타임스퀘어의 화려한 네온사인과 브로드웨이의 열정, 그리고 엠파이어 스테이트 빌딩의 압도적인 스카이라인을 경험해 보세요.",
    "keywords": [
      "북미",
      "타임스퀘어",
      "야경",
      "쇼핑"
    ]
  },
  {
    "id": 134,
    "slug": "rio-de-janeiro",
    "name": "리우데자네이루",
    "name_en": "Rio de Janeiro",
    "country": "브라질",
    "country_en": "Brazil",
    "lat": -22.9,
    "lng": -43.17,
    "tier": 1,
    "popularity": 93,
    "continent": "south_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남미 특유의 열정과 눈부신 자연이 거대한 도시와 완벽하게 융합된 곳입니다. 코파카바나 해변의 여유로움과 코르코바도 산 꼭대기에서 도시를 굽어보는 거대한 구세주 그리스도상이 잊지 못할 인상을 남깁니다.",
    "keywords": [
      "남미",
      "예수상",
      "해변",
      "삼바"
    ]
  },
  {
    "id": 135,
    "slug": "london",
    "name": "런던",
    "name_en": "London",
    "country": "영국",
    "country_en": "UK",
    "lat": 51.5,
    "lng": -0.12,
    "tier": 1,
    "popularity": 85,
    "continent": "unknown",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "western-europe",
    "desc": "유럽의 고풍스러운 랜드마크와 현대적인 도시 풍경이 우아하게 어우러집니다. 무료로 개방되는 세계적 수준의 박물관들, 템스강 위를 가로지르는 타워브리지, 그리고 웨스트엔드의 감동적인 뮤지컬을 즐겨보세요.",
    "keywords": [
      "유럽",
      "빅벤",
      "박물관",
      "뮤지컬"
    ]
  },
  {
    "id": 136,
    "slug": "cape-town",
    "name": "케이프타운",
    "name_en": "Cape Town",
    "country": "남아프리카공화국",
    "country_en": "South Africa",
    "lat": -33.92,
    "lng": 18.42,
    "tier": 1,
    "popularity": 88,
    "continent": "africa",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아프리카 대륙의 끝단에서 세련된 도시 문화와 장엄한 자연을 동시에 만끽할 수 있습니다. 구름이 내려앉는 테이블 마운틴을 배경으로, 감각적인 카페와 갤러리들이 가득한 트렌디한 도심을 거닐어 보세요.",
    "keywords": [
      "아프리카",
      "항구도시",
      "테이블마운틴",
      "트렌디"
    ]
  },
  {
    "id": 137,
    "slug": "tokyo",
    "name": "도쿄",
    "name_en": "Tokyo",
    "country": "일본",
    "country_en": "Japan",
    "lat": 35.67,
    "lng": 139.76,
    "tier": 1,
    "popularity": 87,
    "continent": "asia",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "east-asia",
    "desc": "아시아 초현대적인 마천루와 전통 신사가 골목 하나를 사이에 두고 공존하는 메가시티입니다. 세계 최고의 미식부터 화려한 팝컬처와 쉴 새 없이 변하는 트렌드까지, 모든 것이 집약되어 있습니다.",
    "keywords": [
      "아시아",
      "쇼핑",
      "미식",
      "네온사인"
    ]
  },
  {
    "id": 138,
    "slug": "sydney",
    "name": "시드니",
    "name_en": "Sydney",
    "country": "호주",
    "country_en": "Australia",
    "lat": -33.86,
    "lng": 151.2,
    "tier": 1,
    "popularity": 92,
    "continent": "oceania",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "오세아니아를 대표하는 아름다운 항구 도시입니다. 조가비를 엎어 놓은 듯한 시드니 오페라 하우스와 웅장한 하버 브리지가 빚어내는 스카이라인은 전 세계에서 가장 상징적이고 활기찬 도시 풍경을 선사합니다.",
    "keywords": [
      "오세아니아",
      "오페라하우스",
      "하버브리지",
      "항구"
    ]
  },
  {
    "id": 139,
    "slug": "ushuaia",
    "name": "우수아이아",
    "name_en": "Ushuaia",
    "country": "아르헨티나",
    "country_en": "Argentina",
    "lat": -54.8019,
    "lng": -68.303,
    "tier": 1,
    "popularity": 91,
    "continent": "south_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아르헨티나 최남단(티에라델푸에고)의 세상 끝 도시입니다. 남극·남부 파타고니아 크루즈의 관문(USH)이며, 북부 파타고니아·토레스 델 파이네와는 별도 여행지입니다.",
    "keywords": [
      "대륙",
      "자연/오지",
      "대도시"
    ]
  },
  {
    "id": 140,
    "slug": "moscow",
    "name": "모스크바",
    "name_en": "Moscow",
    "country": "러시아",
    "country_en": "Russia",
    "lat": 55.7558,
    "lng": 37.6173,
    "tier": 1,
    "popularity": 86,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "테트리스 성으로 유명한 성 바실리 대성당과 붉은 광장의 심장부입니다. 러시아 제국의 웅장한 건축 예술을 목격할 수 있습니다.",
    "keywords": [
      "대륙",
      "대도시",
      "역사/문화"
    ]
  },
  {
    "id": 141,
    "slug": "cairo",
    "name": "카이로",
    "name_en": "Cairo",
    "country": "이집트",
    "country_en": "Egypt",
    "lat": 30.0444,
    "lng": 31.2357,
    "tier": 1,
    "popularity": 94,
    "continent": "africa",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "도심 한가운데서 피라미드와 스핑크스를 볼 수 있는 고대 문명의 요람입니다. 모래바람과 이슬람 문명의 신비로움이 가득합니다.",
    "keywords": [
      "대륙",
      "대도시",
      "역사/문화"
    ]
  },
  {
    "id": 142,
    "slug": "new-caledonia",
    "name": "뉴칼레도니아",
    "name_en": "New Caledonia",
    "country": "뉴칼레도니아",
    "country_en": "New Caledonia",
    "lat": -20.9043,
    "lng": 165.618,
    "tier": 2,
    "popularity": 75,
    "continent": "oceania",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "태평양의 프렌치 리비에라로 불립니다. 프랑스의 세련미와 멜라네시아의 자연이 혼합된 우아한 휴양지입니다.",
    "keywords": [
      "대륙",
      "휴양",
      "낭만",
      "대도시"
    ]
  },
  {
    "id": 143,
    "slug": "singapore",
    "name": "싱가포르",
    "name_en": "Singapore",
    "country": "싱가포르",
    "country_en": "Singapore",
    "lat": 1.3521,
    "lng": 103.8198,
    "tier": 1,
    "popularity": 85,
    "continent": "asia",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "southeast-asia",
    "desc": "완벽하게 정돈된 거리에 마리나 베이 샌즈가 우뚝 솟은 도시 국가입니다. 다국적 문화가 혼합된 세련된 쇼핑과 미식의 천국입니다.",
    "keywords": [
      "대도시",
      "액티비티",
      "휴양"
    ]
  },
  {
    "id": 144,
    "slug": "dubai",
    "name": "두바이",
    "name_en": "Dubai",
    "country": "아랍에미리트",
    "country_en": "UAE",
    "lat": 25.2048,
    "lng": 55.2707,
    "tier": 1,
    "popularity": 93,
    "continent": "middle_east",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "사막 위에 세워진 기적의 도시입니다. 세계 최고층 빌딩 부르즈 할리파와 럭셔리 쇼핑몰들이 상상을 초월하는 부유함을 보여줍니다.",
    "keywords": [
      "대도시",
      "액티비티",
      "휴양"
    ]
  },
  {
    "id": 145,
    "slug": "nairobi",
    "name": "나이로비",
    "name_en": "Nairobi",
    "country": "케냐",
    "country_en": "Kenya",
    "lat": -1.2864,
    "lng": 36.8172,
    "tier": 1,
    "popularity": 85,
    "continent": "africa",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "케냐의 활기찬 수도로, 도시의 마천루를 배경으로 기린과 사자가 뛰어노는 국립공원을 보유한 전 세계 유일의 사파리 관문 도시입니다.",
    "keywords": [
      "대도시",
      "자연/오지",
      "액티비티"
    ]
  },
  {
    "id": 146,
    "slug": "chicago",
    "name": "시카고",
    "name_en": "Chicago",
    "country": "미국",
    "country_en": "USA",
    "lat": 41.8781,
    "lng": -87.6298,
    "tier": 1,
    "popularity": 87,
    "continent": "unknown",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "마천루 건축의 발상지이자 거대한 미시간 호수를 품은 바람의 도시입니다. 깊은 맛의 딥디쉬 피자와 재즈의 선율이 흐릅니다.",
    "keywords": [
      "대도시",
      "역사/문화"
    ]
  },
  {
    "id": 147,
    "slug": "buenos-aires",
    "name": "부에노스아이레스",
    "name_en": "Buenos Aires",
    "country": "아르헨티나",
    "country_en": "Argentina",
    "lat": -34.6037,
    "lng": -58.3816,
    "tier": 1,
    "popularity": 88,
    "continent": "south_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남미의 파리라 불리는 아르헨티나의 수도입니다. 탱고의 고혹적인 스텝이 거리마다 울려 퍼지며 세계 최고의 소고기 스테이크를 맛볼 수 있습니다.",
    "keywords": [
      "대도시",
      "낭만",
      "역사/문화"
    ]
  },
  {
    "id": 148,
    "slug": "melbourne",
    "name": "멜버른",
    "name_en": "Melbourne",
    "country": "호주",
    "country_en": "Australia",
    "lat": -37.8136,
    "lng": 144.9631,
    "tier": 1,
    "popularity": 88,
    "continent": "oceania",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "향긋한 플랫화이트 한 잔과 함께 그래피티가 그려진 골목을 걷는 호주의 문화 수도입니다. 그레이트 오션 로드로 떠나는 환상적인 드라이브가 기다립니다.",
    "keywords": [
      "대도시",
      "문화/역사",
      "낭만"
    ]
  },
  {
    "id": 149,
    "slug": "chichen-itza",
    "name": "치첸이트사",
    "name_en": "Chichen Itza",
    "country": "멕시코",
    "country_en": "Mexico",
    "lat": 20.68,
    "lng": -88.56,
    "tier": 2,
    "popularity": 79,
    "continent": "north_america",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "북미 정글 한가운데 솟아오른 고대 마야 문명의 신비로운 피라미드입니다. 천문학과 수학에 능통했던 마야인들의 놀라운 지혜가 깃든 쿠쿨칸의 신전에서 수천 년 전의 신비로운 숨결을 느껴보세요.",
    "keywords": [
      "북미",
      "마야문명",
      "피라미드",
      "유적지"
    ]
  },
  {
    "id": 150,
    "slug": "machu-picchu",
    "name": "마추픽추",
    "name_en": "Machu Picchu",
    "country": "페루",
    "country_en": "Peru",
    "lat": -13.16,
    "lng": -72.54,
    "tier": 1,
    "popularity": 90,
    "continent": "south_america",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "남미 안데스 산맥의 가파른 봉우리 위에 숨겨진 잉카 제국의 공중 도시입니다. 구름이 걷히며 서서히 드러나는 거대한 돌로 쌓은 고대 도시의 웅장함은 여행자들에게 형언할 수 없는 전율을 안겨줍니다.",
    "keywords": [
      "남미",
      "잉카",
      "안데스",
      "잃어버린도시"
    ]
  },
  {
    "id": 151,
    "slug": "rome",
    "name": "로마",
    "name_en": "Rome",
    "country": "이탈리아",
    "country_en": "Italy",
    "lat": 41.9,
    "lng": 12.49,
    "tier": 1,
    "popularity": 85,
    "continent": "europe",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": "southern-europe",
    "desc": "유럽 발길 닿는 모든 곳이 유적인 거대한 야외 박물관 같은 영원의 도시입니다. 콜로세움과 포로 로마노의 장엄한 폐허 속을 거닐며, 한 시대를 호령했던 로마 제국의 위대한 역사를 직접 대면해 보세요.",
    "keywords": [
      "유럽",
      "콜로세움",
      "로마제국",
      "역사"
    ]
  },
  {
    "id": 152,
    "slug": "giza-pyramids",
    "name": "기자 피라미드",
    "name_en": "Giza Pyramids",
    "country": "이집트",
    "country_en": "Egypt",
    "lat": 29.97,
    "lng": 31.13,
    "tier": 1,
    "popularity": 88,
    "continent": "africa",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아프리카 대륙 나일강 변 사막 위에 우뚝 솟은 인류 역사상 가장 위대한 건축물입니다. 수천 년의 세월을 견뎌낸 스핑크스와 거대한 파라오의 무덤 앞에서 인간의 경이로운 한계를 체감하게 됩니다.",
    "keywords": [
      "아프리카",
      "스핑크스",
      "고대문명",
      "사막"
    ]
  },
  {
    "id": 153,
    "slug": "angkor-wat",
    "name": "앙코르 와트",
    "name_en": "Angkor Wat",
    "country": "캄보디아",
    "country_en": "Cambodia",
    "lat": 13.41,
    "lng": 103.86,
    "tier": 1,
    "popularity": 93,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": "southeast-asia",
    "desc": "아시아 캄보디아 밀림 속에 오랫동안 잠들어 있던 크메르 제국의 거대한 종교 건축물입니다. 정교한 석조 부조들과 거대한 뿌리로 사원을 뒤덮은 나무들이 얽힌 풍경은 신비롭고도 장엄합니다.",
    "keywords": [
      "아시아",
      "크메르",
      "불교/힌두교",
      "유적"
    ]
  },
  {
    "id": 154,
    "slug": "petra",
    "name": "페트라",
    "name_en": "Petra",
    "country": "요르단",
    "country_en": "Jordan",
    "lat": 30.32,
    "lng": 35.44,
    "tier": 1,
    "popularity": 86,
    "continent": "middle_east",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "중동의 붉은 사암 절벽을 깎아 만든 나바테아인들의 잃어버린 고대 도시입니다. 좁은 협곡을 지나 마주하는 장미빛 알 카즈네 신전은 영화 속 인디아나 존스가 된 듯한 강렬한 모험심을 불러일으킵니다.",
    "keywords": [
      "중동",
      "장미빛도시",
      "사막",
      "고대유적"
    ]
  },
  {
    "id": 155,
    "slug": "midway-atoll",
    "name": "미드웨이 환초",
    "name_en": "Midway Atoll",
    "country": "미드웨이 환초",
    "country_en": "Midway Atoll",
    "lat": 28.2072,
    "lng": -177.3735,
    "tier": 3,
    "popularity": 46,
    "continent": "unknown",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "알바트로스 새들의 최대 서식지이자 제2차 세계대전의 주요 격전지였던 태평양 한가운데의 고립된 환초입니다.",
    "keywords": [
      "대륙",
      "자연/오지",
      "역사/문화"
    ]
  },
  {
    "id": 156,
    "slug": "bagan",
    "name": "바간",
    "name_en": "Bagan",
    "country": "미얀마",
    "country_en": "Myanmar",
    "lat": 21.1717,
    "lng": 94.8585,
    "tier": 2,
    "popularity": 78,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "미얀마의 광활한 평원 위로 솟아오른 수천 개의 불탑들이 압도적인 곳입니다. 열기구를 타고 내려다보는 일출은 평생의 감동입니다.",
    "keywords": [
      "대륙",
      "역사/문화",
      "낭만"
    ]
  },
  {
    "id": 157,
    "slug": "fiji",
    "name": "피지",
    "name_en": "Fiji",
    "country": "피지",
    "country_en": "Fiji",
    "lat": -17.7134,
    "lng": 178.065,
    "tier": 2,
    "popularity": 77,
    "continent": "oceania",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "300개가 넘는 섬으로 이루어진 다채로운 국가입니다. 원주민들의 친절한 미소와 다이내믹한 해양 스포츠가 기다립니다.",
    "keywords": [
      "대륙",
      "휴양",
      "액티비티",
      "역사/문화"
    ]
  },
  {
    "id": 158,
    "slug": "easter-island",
    "name": "이스터 섬",
    "name_en": "Easter Island",
    "country": "칠레",
    "country_en": "Chile",
    "lat": -27.1127,
    "lng": -109.3497,
    "tier": 3,
    "popularity": 45,
    "continent": "south_america",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "거대한 모아이 석상들이 수수께끼처럼 해안가를 지키고 있는 칠레령의 외딴 섬입니다. 미지의 웅장함이 압도합니다.",
    "keywords": [
      "자연/오지",
      "역사/문화"
    ]
  },
  {
    "id": 159,
    "slug": "meteora",
    "name": "메테오라",
    "name_en": "Meteora",
    "country": "그리스",
    "country_en": "Greece",
    "lat": 39.7128,
    "lng": 21.6266,
    "tier": 2,
    "popularity": 70,
    "continent": "europe",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "그리스 중부, 하늘에 기둥처럼 솟아오른 거대한 암벽 꼭대기에 아슬아슬하게 세워진 수도원들이 초현실적인 경관을 연출합니다.",
    "keywords": [
      "역사/문화",
      "자연/오지"
    ]
  },
  {
    "id": 160,
    "slug": "st-helena",
    "name": "세인트 헬레나",
    "name_en": "St. Helena",
    "country": "영국령 세인트 헬레나",
    "country_en": "St. Helena",
    "lat": -15.965,
    "lng": -5.7089,
    "tier": 2,
    "popularity": 78,
    "continent": "unknown",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "나폴레옹이 유배되어 생을 마감한 남대서양의 고독한 섬입니다. 육지와 수천 킬로미터 떨어진 고립된 역사적 장소입니다.",
    "keywords": [
      "역사/문화",
      "자연/오지"
    ]
  },
  {
    "id": 161,
    "slug": "ulaanbaatar",
    "name": "울란바토르",
    "name_en": "Ulaanbaatar",
    "country": "몽골",
    "country_en": "Mongolia",
    "lat": 47.92,
    "lng": 106.92,
    "tier": 2,
    "popularity": 73,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "끝없는 초원과 게르가 떠오르는 유목민의 심장입니다. 밤하늘에 쏟아지는 별빛 아래 말들이 달리는 몽골의 중심지입니다.",
    "keywords": [
      "대도시",
      "자연/오지",
      "역사/문화"
    ]
  },
  {
    "id": 162,
    "slug": "istanbul",
    "name": "이스탄불",
    "name_en": "Istanbul",
    "country": "튀르키예",
    "country_en": "Turkey",
    "lat": 41.0082,
    "lng": 28.9783,
    "tier": 1,
    "popularity": 85,
    "continent": "europe",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "유럽과 아시아가 보스포루스 해협을 사이에 두고 입을 맞추는 매혹적인 교차로입니다. 웅장한 모스크의 기도 소리가 이국적인 정취를 극대화합니다.",
    "keywords": [
      "대도시",
      "역사/문화",
      "낭만"
    ]
  },
  {
    "id": 163,
    "slug": "prague",
    "name": "프라하",
    "name_en": "Prague",
    "country": "체코",
    "country_en": "Czech Republic",
    "lat": 50.0755,
    "lng": 14.4378,
    "tier": 1,
    "popularity": 93,
    "continent": "unknown",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "western-europe",
    "desc": "블타바 강 위로 까를교가 빛나는 동유럽 야경의 끝판왕입니다. 수많은 첨탑과 붉은 지붕이 로맨틱한 분위기를 자아냅니다.",
    "keywords": [
      "대도시",
      "역사/문화",
      "낭만"
    ]
  },
  {
    "id": 164,
    "slug": "marrakech",
    "name": "마라케시",
    "name_en": "Marrakech",
    "country": "모로코",
    "country_en": "Morocco",
    "lat": 31.6295,
    "lng": -7.9811,
    "tier": 2,
    "popularity": 69,
    "continent": "africa",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "붉은 성벽으로 둘러싸인 모로코의 천일야화 도시입니다. 제마 엘 프나 광장의 혼돈과 아름다운 리아드(전통 가옥)가 강렬하게 대비됩니다.",
    "keywords": [
      "역사/문화",
      "낭만",
      "대도시"
    ]
  },
  {
    "id": 165,
    "slug": "alaska",
    "name": "알래스카",
    "name_en": "Alaska",
    "country": "미국",
    "country_en": "USA",
    "lat": 64.2,
    "lng": -149.49,
    "tier": 2,
    "popularity": 73,
    "continent": "unknown",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "북미 최북단의 야생이 살아 숨 쉬는 거대한 얼음의 땅입니다. 빙하가 무너져 내리는 장관을 목격하고, 고래와 불곰 등 거대한 야생동물들의 생태계를 관찰하는 진정한 탐험가의 목적지입니다.",
    "keywords": [
      "북미",
      "빙하",
      "야생동물",
      "오지"
    ]
  },
  {
    "id": 166,
    "slug": "amazon-rainforest",
    "name": "아마존 열대우림",
    "name_en": "Amazon Rainforest",
    "country": "브라질",
    "country_en": "Brazil",
    "lat": -3.46,
    "lng": -62.21,
    "tier": 2,
    "popularity": 73,
    "continent": "south_america",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남미 지구의 허파라 불리는 세계에서 가장 크고 깊은 미지의 정글입니다. 거대한 아마존 강을 따라 보트를 타며 핑크 돌고래와 희귀한 조류들을 관찰하는 날것 그대로의 생태 탐험을 떠나보세요.",
    "keywords": [
      "남미",
      "정글",
      "생태탐험",
      "야생"
    ]
  },
  {
    "id": 167,
    "slug": "svalbard",
    "name": "스발바르 제도",
    "name_en": "Svalbard",
    "country": "노르웨이",
    "country_en": "Norway",
    "lat": 78.22,
    "lng": 15.65,
    "tier": 3,
    "popularity": 52,
    "continent": "unknown",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "유럽 대륙 북쪽, 북극점과 가장 가까운 인간의 거주지입니다. 끝없는 설원 위를 걷는 북극곰의 발자취를 쫓고, 영구 동토층의 빙동굴을 탐험하며 혹한이 빚어낸 순백의 고립을 경험할 수 있습니다.",
    "keywords": [
      "북유럽",
      "북극곰",
      "설원",
      "오로라"
    ]
  },
  {
    "id": 168,
    "slug": "serengeti",
    "name": "세렝게티",
    "name_en": "Serengeti",
    "country": "탄자니아",
    "country_en": "Tanzania",
    "lat": -2.33,
    "lng": 34.83,
    "tier": 2,
    "popularity": 74,
    "continent": "africa",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "아프리카의 끝없는 초원 위에서 펼쳐지는 야생동물들의 서사시를 직접 목격할 수 있습니다. 수백만 마리의 누떼가 이동하는 대자연의 스펙터클 한가운데로 사파리 지프를 타고 뛰어들어 보세요.",
    "keywords": [
      "아프리카",
      "사파리",
      "대자연",
      "초원"
    ]
  },
  {
    "id": 169,
    "slug": "uluru",
    "name": "울루루",
    "name_en": "Uluru",
    "country": "호주",
    "country_en": "Australia",
    "lat": -25.34,
    "lng": 131.03,
    "tier": 2,
    "popularity": 68,
    "continent": "oceania",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "오세아니아 대륙의 붉은 심장, 아웃백 한가운데 솟아오른 거대한 단일 바위입니다. 시간과 햇빛에 따라 붉게 타오르는 듯 변하는 바위의 색조와 원주민의 신성한 에너지를 오프로드를 달리며 느껴보세요.",
    "keywords": [
      "오세아니아",
      "아웃백",
      "사막",
      "원주민"
    ]
  },
  {
    "id": 170,
    "slug": "antarctica",
    "name": "남극 대륙",
    "name_en": "Antarctica",
    "country": "남극",
    "country_en": "Antarctica",
    "lat": -80,
    "lng": 0,
    "tier": 3,
    "popularity": 58,
    "continent": "unknown",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "지구상에 남은 마지막 진정한 오지이자 빙하로 뒤덮인 극한의 대륙입니다. 쇄빙선을 타고 얼어붙은 바다를 가르며 펭귄 군락과 거대한 빙산들이 떠다니는 초현실적인 세계로 들어가는 궁극의 모험입니다.",
    "keywords": [
      "남극",
      "펭귄",
      "빙산",
      "극한의탐험"
    ]
  },
  {
    "id": 171,
    "slug": "los-angeles",
    "name": "로스앤젤레스",
    "name_en": "Los Angeles",
    "country": "미국",
    "country_en": "USA",
    "lat": 34.0522,
    "lng": -118.2437,
    "tier": 2,
    "popularity": 69,
    "continent": "unknown",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": false,
    "denseRegion": "us-west-coast",
    "desc": "할리우드의 글래머러스한 매력과 연중 따뜻한 캘리포니아의 햇살이 가득한 천사의 도시입니다. 해변에서 롤러스케이트를 타는 여유가 있습니다.",
    "keywords": [
      "대륙",
      "대도시",
      "휴양",
      "액티비티"
    ]
  },
  {
    "id": 172,
    "slug": "tonga",
    "name": "통가",
    "name_en": "Tonga",
    "country": "통가",
    "country_en": "Tonga",
    "lat": -21.179,
    "lng": -175.1982,
    "tier": 2,
    "popularity": 66,
    "continent": "oceania",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남태평양 유일의 왕국입니다. 매년 혹등고래가 짝짓기를 위해 찾아오는 경이로운 생태의 현장을 목격할 수 있습니다.",
    "keywords": [
      "대륙",
      "자연/오지",
      "액티비티",
      "역사/문화"
    ]
  },
  {
    "id": 174,
    "slug": "la-reunion",
    "name": "레위니옹",
    "name_en": "La Reunion",
    "country": "프랑스령 레위니옹",
    "country_en": "La Reunion",
    "lat": -21.1151,
    "lng": 55.5364,
    "tier": 2,
    "popularity": 74,
    "continent": "unknown",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "인도양의 알프스라 불리는 프랑스령 화산섬입니다. 활화산 등반과 헬기 투어 등 스릴 넘치는 대자연의 경관을 뽐냅니다.",
    "keywords": [
      "대륙",
      "자연/오지",
      "액티비티"
    ]
  },
  {
    "id": 175,
    "slug": "seoul",
    "name": "서울",
    "name_en": "Seoul",
    "country": "한국",
    "country_en": "South Korea",
    "lat": 37.5665,
    "lng": 126.978,
    "tier": 2,
    "popularity": 73,
    "continent": "asia",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "잠들지 않는 역동성의 도시입니다. 600년 역사의 고궁 주변으로 화려한 스카이라인과 최첨단 트렌드가 쉴 새 없이 교차합니다.",
    "keywords": [
      "대륙",
      "대도시",
      "역사/문화",
      "액티비티"
    ]
  },
  {
    "id": 176,
    "slug": "vanuatu",
    "name": "바누아투",
    "name_en": "Vanuatu",
    "country": "바누아투",
    "country_en": "Vanuatu",
    "lat": -15.3767,
    "lng": 166.9592,
    "tier": 2,
    "popularity": 65,
    "continent": "oceania",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "활화산을 가까이서 볼 수 있는 모험의 섬입니다. 원시 부족의 문화와 자연 그대로의 생태계가 매력적입니다.",
    "keywords": [
      "대륙",
      "자연/오지",
      "액티비티",
      "역사/문화"
    ]
  },
  {
    "id": 177,
    "slug": "patagonia",
    "name": "파타고니아",
    "name_en": "Patagonia (Northern)",
    "country": "아르헨티나",
    "country_en": "Argentina",
    "lat": -41.1335,
    "lng": -71.3103,
    "tier": 2,
    "popularity": 72,
    "continent": "south_america",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아르헨티나 북부 파타고니아(호수·안데스·바릴로체 권)입니다. 남부(우수아이아·토레스 델 파이네)와는 별도 여행지로, 보통 부에노스아이레스 경유 후 바릴로체(BRC) 등으로 들어옵니다.",
    "keywords": [
      "바릴로체",
      "호수",
      "안데스",
      "트레킹",
      "자연/오지"
    ]
  },
  {
    "id": 178,
    "slug": "kerguelen-islands",
    "name": "케르겔렌 제도",
    "name_en": "Kerguelen Islands",
    "country": "프랑스 남부 연방 영토",
    "country_en": "French Southern and Antarctic Lands",
    "lat": -49.3958,
    "lng": 70.2073,
    "tier": 3,
    "popularity": 56,
    "continent": "unknown",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남인도양에 위치한 황량한 프랑스령 섬입니다. 황량함(Desolation)의 섬이라 불릴 만큼 극도로 고립되어 바람이 멈추지 않는 곳입니다.",
    "keywords": [
      "자연/오지"
    ]
  },
  {
    "id": 179,
    "slug": "yakutsk",
    "name": "야쿠츠크",
    "name_en": "Yakutsk",
    "country": "러시아",
    "country_en": "Russia",
    "lat": 62.0355,
    "lng": 129.6755,
    "tier": 3,
    "popularity": 53,
    "continent": "europe",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "겨울이면 영하 50도까지 떨어지는 세계에서 가장 추운 도시입니다. 영구동토층 위에 세워진 극한 환경 속 인류의 인내를 보여줍니다.",
    "keywords": [
      "대도시",
      "자연/오지"
    ]
  },
  {
    "id": 180,
    "slug": "kamchatka-peninsula",
    "name": "캄차카 반도",
    "name_en": "Kamchatka Peninsula",
    "country": "러시아",
    "country_en": "Russia",
    "lat": 56.1327,
    "lng": 159.5314,
    "tier": 3,
    "popularity": 59,
    "continent": "europe",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "수백 개의 화산이 연기를 뿜어내고 곰들이 연어를 사냥하는 야생의 땅입니다. 불과 얼음이 공존하는 원시 자연을 간직하고 있습니다.",
    "keywords": [
      "자연/오지",
      "액티비티"
    ]
  },
  {
    "id": 181,
    "slug": "paris",
    "name": "파리",
    "name_en": "Paris",
    "country": "프랑스",
    "country_en": "France",
    "lat": 48.8566,
    "lng": 2.3522,
    "tier": 1,
    "popularity": 98,
    "continent": "europe",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "western-europe",
    "desc": "에펠탑과 루브르 박물관이 있는 예술과 낭만의 도시입니다. 센강을 따라 거닐며 노트르담 대성당과 샹젤리제 거리를 감상하고, 세계 최고의 미술관과 카페 문화를 경험할 수 있습니다.",
    "keywords": [
      "에펠탑",
      "루브르",
      "센강",
      "샹젤리제",
      "예술"
    ]
  },
  {
    "id": 182,
    "slug": "barcelona",
    "name": "바르셀로나",
    "name_en": "Barcelona",
    "country": "스페인",
    "country_en": "Spain",
    "lat": 41.3851,
    "lng": 2.1734,
    "tier": 1,
    "popularity": 95,
    "continent": "europe",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "western-europe",
    "desc": "가우디의 사그라다 파밀리아와 구엘 공원이 있는 건축의 도시입니다. 고딕 지구의 좁은 골목과 람블라스 거리의 활기, 그리고 지중해 해변의 여유를 모두 즐길 수 있습니다.",
    "keywords": [
      "사그라다파밀리아",
      "가우디",
      "람블라스",
      "고딕지구",
      "건축"
    ]
  },
  {
    "id": 183,
    "slug": "bangkok",
    "name": "방콕",
    "name_en": "Bangkok",
    "country": "태국",
    "country_en": "Thailand",
    "lat": 13.7563,
    "lng": 100.5018,
    "tier": 1,
    "popularity": 96,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "southeast-asia",
    "desc": "왓 아룬과 왕궁이 있는 동남아시아의 활기찬 수도입니다. 화려한 불교 사원과 현대적인 쇼핑몰, 그리고 세계적인 태국 음식과 나이트 마켓이 공존하는 매력적인 도시입니다.",
    "keywords": [
      "왓아룬",
      "왕궁",
      "카오산로드",
      "태국음식",
      "나이트마켓"
    ]
  },
  {
    "id": 184,
    "slug": "amsterdam",
    "name": "암스테르담",
    "name_en": "Amsterdam",
    "country": "네덜란드",
    "country_en": "Netherlands",
    "lat": 52.3676,
    "lng": 4.9041,
    "tier": 1,
    "popularity": 90,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "western-europe",
    "desc": "운하를 따라 펼쳐진 그림 같은 도시입니다. 안네 프랑크의 집과 반 고흐 미술관, 그리고 자유로운 분위기와 자전거 문화가 어우러진 독특한 매력을 지닌 곳입니다.",
    "keywords": [
      "운하",
      "안네의집",
      "반고흐",
      "자전거",
      "튤립"
    ]
  },
  {
    "id": 185,
    "slug": "venice",
    "name": "베니스",
    "name_en": "Venice",
    "country": "이탈리아",
    "country_en": "Italy",
    "lat": 45.4408,
    "lng": 12.3155,
    "tier": 1,
    "popularity": 92,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "southern-europe",
    "desc": "물 위에 떠 있는 낭만적인 도시입니다. 곤돌라를 타고 대운하를 따라 이동하며 산마르코 광장과 리알토 다리를 감상하고, 베네치아 가면 축제의 신비로움을 경험할 수 있습니다.",
    "keywords": [
      "곤돌라",
      "대운하",
      "산마르코광장",
      "수상도시",
      "가면축제"
    ]
  },
  {
    "id": 186,
    "slug": "athens",
    "name": "아테네",
    "name_en": "Athens",
    "country": "그리스",
    "country_en": "Greece",
    "lat": 37.9838,
    "lng": 23.7275,
    "tier": 1,
    "popularity": 88,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "서양 문명의 요람이자 민주주의의 발상지입니다. 아크로폴리스 언덕의 파르테논 신전과 고대 아고라를 탐험하며, 수천 년 역사가 살아 숨 쉬는 도시를 경험할 수 있습니다.",
    "keywords": [
      "파르테논",
      "아크로폴리스",
      "고대그리스",
      "신전",
      "민주주의"
    ]
  },
  {
    "id": 187,
    "slug": "berlin",
    "name": "베를린",
    "name_en": "Berlin",
    "country": "독일",
    "country_en": "Germany",
    "lat": 52.52,
    "lng": 13.405,
    "tier": 1,
    "popularity": 87,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "central-europe",
    "desc": "분단의 역사를 극복하고 예술과 자유의 도시로 거듭난 독일의 수도입니다. 베를린 장벽 기념관과 브란덴부르크 문, 그리고 세계적인 클럽과 갤러리가 공존하는 창의적인 도시입니다.",
    "keywords": [
      "베를린장벽",
      "브란덴부르크문",
      "역사",
      "클럽",
      "예술"
    ]
  },
  {
    "id": 188,
    "slug": "vienna",
    "name": "빈",
    "name_en": "Vienna",
    "country": "오스트리아",
    "country_en": "Austria",
    "lat": 48.2082,
    "lng": 16.3738,
    "tier": 1,
    "popularity": 86,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "central-europe",
    "desc": "음악의 도시이자 합스부르크 제국의 화려한 수도였던 곳입니다. 쇤브룬 궁전과 오페라 하우스, 그리고 전통 카페 문화와 비엔나 왈츠의 우아함을 경험할 수 있습니다.",
    "keywords": [
      "쇤브룬궁전",
      "오페라",
      "모차르트",
      "커피하우스",
      "왈츠"
    ]
  },
  {
    "id": 189,
    "slug": "florence",
    "name": "피렌체",
    "name_en": "Florence",
    "country": "이탈리아",
    "country_en": "Italy",
    "lat": 43.7696,
    "lng": 11.2558,
    "tier": 1,
    "popularity": 89,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "southern-europe",
    "desc": "르네상스의 발상지이자 미켈란젤로와 다빈치의 도시입니다. 두오모 대성당과 우피치 미술관, 베키오 다리를 거닐며 예술과 역사가 살아 숨 쉬는 토스카나의 보석을 경험할 수 있습니다.",
    "keywords": [
      "두오모",
      "우피치",
      "르네상스",
      "미켈란젤로",
      "예술"
    ]
  },
  {
    "id": 190,
    "slug": "hong-kong",
    "name": "홍콩",
    "name_en": "Hong Kong",
    "country": "중국",
    "country_en": "China",
    "lat": 22.3193,
    "lng": 114.1694,
    "tier": 1,
    "popularity": 94,
    "continent": "asia",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "동서양이 만나는 역동적인 금융 도시입니다. 빅토리아 피크에서 내려다보는 환상적인 야경과 침사추이의 쇼핑, 그리고 딤섬과 길거리 음식의 천국을 경험할 수 있습니다.",
    "keywords": [
      "빅토리아피크",
      "침사추이",
      "야경",
      "딤섬",
      "쇼핑"
    ]
  },
  {
    "id": 191,
    "slug": "taipei",
    "name": "타이베이",
    "name_en": "Taipei",
    "country": "대만",
    "country_en": "Taiwan",
    "lat": 25.033,
    "lng": 121.5654,
    "tier": 1,
    "popularity": 85,
    "continent": "asia",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "east-asia",
    "desc": "타이베이 101과 야시장의 도시입니다. 현대적인 마천루와 전통 사원이 어우러지고, 지우펀의 아름다운 야경과 맛있는 타이완 음식을 즐길 수 있습니다.",
    "keywords": [
      "타이베이101",
      "야시장",
      "지우펀",
      "타이완음식",
      "버블티"
    ]
  },
  {
    "id": 192,
    "slug": "shanghai",
    "name": "상하이",
    "name_en": "Shanghai",
    "country": "중국",
    "country_en": "China",
    "lat": 31.2304,
    "lng": 121.4737,
    "tier": 1,
    "popularity": 91,
    "continent": "asia",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "east-asia",
    "desc": "중국 최대의 경제 도시이자 미래 도시의 모습을 보여주는 곳입니다. 와이탄의 역사적인 건물들과 동방명주타워, 그리고 현대적인 푸동 지구의 마천루가 대비를 이룹니다.",
    "keywords": [
      "와이탄",
      "동방명주",
      "푸동",
      "마천루",
      "야경"
    ]
  },
  {
    "id": 193,
    "slug": "kyoto",
    "name": "교토",
    "name_en": "Kyoto",
    "country": "일본",
    "country_en": "Japan",
    "lat": 35.0116,
    "lng": 135.7681,
    "tier": 1,
    "popularity": 93,
    "continent": "asia",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": "east-asia",
    "desc": "일본 전통 문화의 심장이자 옛 수도입니다. 금각사와 후시미 이나리 신사의 붉은 도리이, 기온 게이샤 거리와 대나무 숲을 거닐며 일본의 고즈넉한 아름다움을 경험할 수 있습니다.",
    "keywords": [
      "금각사",
      "후시미이나리",
      "기온",
      "게이샤",
      "대나무숲"
    ]
  },
  {
    "id": 194,
    "slug": "delhi",
    "name": "델리",
    "name_en": "Delhi",
    "country": "인도",
    "country_en": "India",
    "lat": 28.7041,
    "lng": 77.1025,
    "tier": 1,
    "popularity": 84,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "인도의 수도이자 역사와 현대가 공존하는 거대 도시입니다. 타지마할로 가는 관문이며, 인도 문과 후마윤의 무덤, 그리고 활기찬 시장과 인도 음식의 진수를 경험할 수 있습니다.",
    "keywords": [
      "인도문",
      "후마윤무덤",
      "타지마할관문",
      "인도음식",
      "역사"
    ]
  },
  {
    "id": 195,
    "slug": "san-francisco",
    "name": "샌프란시스코",
    "name_en": "San Francisco",
    "country": "미국",
    "country_en": "United States",
    "lat": 37.7749,
    "lng": -122.4194,
    "tier": 1,
    "popularity": 90,
    "continent": "north_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "us-west-coast",
    "desc": "금문교와 케이블카의 도시입니다. 가파른 언덕과 빅토리아 양식의 주택, 피셔맨스 워프와 알카트라즈 섬, 그리고 실리콘밸리의 혁신 문화가 어우러진 캘리포니아의 보석입니다.",
    "keywords": [
      "금문교",
      "케이블카",
      "알카트라즈",
      "피셔맨스워프",
      "실리콘밸리"
    ]
  },
  {
    "id": 196,
    "slug": "las-vegas",
    "name": "라스베가스",
    "name_en": "Las Vegas",
    "country": "미국",
    "country_en": "United States",
    "lat": 36.1699,
    "lng": -115.1398,
    "tier": 1,
    "popularity": 89,
    "continent": "north_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "사막 한가운데의 불야성 엔터테인먼트 도시입니다. 화려한 카지노와 세계적인 쇼, 벨라지오 분수와 스트립 거리의 네온사인이 24시간 잠들지 않는 도시의 에너지를 보여줍니다.",
    "keywords": [
      "카지노",
      "스트립",
      "쇼",
      "벨라지오",
      "엔터테인먼트"
    ]
  },
  {
    "id": 197,
    "slug": "toronto",
    "name": "토론토",
    "name_en": "Toronto",
    "country": "캐나다",
    "country_en": "Canada",
    "lat": 43.6532,
    "lng": -79.3832,
    "tier": 1,
    "popularity": 83,
    "continent": "north_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "us-east-coast",
    "desc": "캐나다 최대의 다문화 도시입니다. CN 타워의 전망대에서 내려다보는 도시 풍경과 나이아가라 폭포로 가는 관문, 그리고 다양한 문화가 어우러진 활기찬 도시를 경험할 수 있습니다.",
    "keywords": [
      "CN타워",
      "나이아가라",
      "다문화",
      "스카이라인",
      "도시"
    ]
  },
  {
    "id": 198,
    "slug": "vancouver",
    "name": "밴쿠버",
    "name_en": "Vancouver",
    "country": "캐나다",
    "country_en": "Canada",
    "lat": 49.2827,
    "lng": -123.1207,
    "tier": 1,
    "popularity": 82,
    "continent": "north_america",
    "categories": [
      "urban",
      "nature"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "us-west-coast",
    "desc": "산과 바다가 만나는 아름다운 도시입니다. 스탠리 파크의 숲과 그랜빌 아일랜드 마켓, 그리고 주변의 웅장한 록키 산맥까지, 자연과 도시가 완벽하게 조화를 이루는 곳입니다.",
    "keywords": [
      "스탠리파크",
      "그랜빌",
      "산과바다",
      "자연",
      "록키"
    ]
  },
  {
    "id": 199,
    "slug": "auckland",
    "name": "오클랜드",
    "name_en": "Auckland",
    "country": "뉴질랜드",
    "country_en": "New Zealand",
    "lat": -36.8485,
    "lng": 174.7633,
    "tier": 1,
    "popularity": 80,
    "continent": "oceania",
    "categories": [
      "urban",
      "nature"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "뉴질랜드 최대 도시이자 두 개의 항구를 가진 도시입니다. 스카이 타워와 와이헤케 섬의 와이너리, 그리고 주변의 아름다운 해변과 화산 지형을 탐험할 수 있습니다.",
    "keywords": [
      "스카이타워",
      "와이헤케섬",
      "항구",
      "와인",
      "해변"
    ]
  },
  {
    "id": 200,
    "slug": "jerusalem",
    "name": "예루살렘",
    "name_en": "Jerusalem",
    "country": "이스라엘",
    "country_en": "Israel",
    "lat": 31.7683,
    "lng": 35.2137,
    "tier": 1,
    "popularity": 85,
    "continent": "middle_east",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "세 종교의 성지이자 수천 년 역사가 살아있는 도시입니다. 통곡의 벽과 성묘 교회, 알 아크사 모스크를 방문하며 인류 역사에서 가장 중요한 도시 중 하나를 경험할 수 있습니다.",
    "keywords": [
      "통곡의벽",
      "성지",
      "구시가지",
      "종교",
      "역사"
    ]
  },
  {
    "id": 201,
    "slug": "gili-meno",
    "name": "길리 메노",
    "name_en": "Gili Meno",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -8.3597,
    "lng": 116.0808,
    "tier": 2,
    "popularity": 82,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": "southeast-asia",
    "desc": "롬복 인근의 작은 섬으로, 길리 3형제 중 가장 조용하고 한적한 천국입니다. 형광 플랑크톤이 빛나는 밤바다와 거북이와 함께 수영하는 경험을 선사합니다. 상업화되지 않은 순수한 자연 그대로의 모습이 매력적입니다.",
    "keywords": [
      "인도네시아",
      "스노클링",
      "거북이",
      "한적한",
      "천국"
    ]
  },
  {
    "id": 202,
    "slug": "el-nido",
    "name": "엘니도",
    "name_en": "El Nido",
    "country": "필리핀",
    "country_en": "Philippines",
    "lat": 11.195,
    "lng": 119.4019,
    "tier": 2,
    "popularity": 84,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "필리핀 팔라완 북쪽 끝에 위치한 숨겨진 보석입니다. 석회암 절벽으로 둘러싸인 비밀 해변과 에메랄드빛 라군은 지구상에서 가장 아름다운 풍경을 자랑합니다. 아일랜드 호핑 투어는 필수 체험입니다.",
    "keywords": [
      "필리핀",
      "라군",
      "아일랜드호핑",
      "석회암절벽",
      "비밀해변"
    ]
  },
  {
    "id": 203,
    "slug": "boracay",
    "name": "보라카이",
    "name_en": "Boracay",
    "country": "필리핀",
    "country_en": "Philippines",
    "lat": 11.9674,
    "lng": 121.9248,
    "tier": 2,
    "popularity": 86,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": "southeast-asia",
    "desc": "4km에 달하는 파우더처럼 고운 화이트 비치가 유명한 필리핀 최고의 휴양지입니다. 낮에는 카약과 카이트서핑을, 밤에는 해변의 파이어 댄스를 즐길 수 있습니다. 완벽히 복구된 자연환경이 더욱 빛납니다.",
    "keywords": [
      "필리핀",
      "화이트비치",
      "카이트서핑",
      "선셋",
      "파티"
    ]
  },
  {
    "id": 204,
    "slug": "phuket",
    "name": "푸켓",
    "name_en": "Phuket",
    "country": "태국",
    "country_en": "Thailand",
    "lat": 7.8804,
    "lng": 98.3923,
    "tier": 2,
    "popularity": 88,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": "southeast-asia",
    "desc": "안다만해에 위치한 태국 최대의 섬 휴양지입니다. 파통, 카론, 카타 등 다양한 테마의 해변과 피피 섬으로 가는 관문 역할을 합니다. 스노클링, 다이빙, 태국 전통 마사지까지 모든 것을 즐길 수 있습니다.",
    "keywords": [
      "태국",
      "안다만해",
      "다이빙",
      "나이트라이프",
      "리조트"
    ]
  },
  {
    "id": 205,
    "slug": "phi-phi-islands",
    "name": "피피 섬",
    "name_en": "Phi Phi Islands",
    "country": "태국",
    "country_en": "Thailand",
    "lat": 7.7407,
    "lng": 98.7784,
    "tier": 2,
    "popularity": 83,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "영화 '더 비치'의 촬영지로 유명해진 천혜의 비경입니다. 마야 베이의 투명한 바다와 수직으로 솟은 석회암 절벽은 숨이 멎을 듯 아름답습니다. 스노클링하면 형형색색의 열대어 떼를 만날 수 있습니다.",
    "keywords": [
      "태국",
      "더비치",
      "마야베이",
      "스노클링",
      "석회암"
    ]
  },
  {
    "id": 206,
    "slug": "koh-samui",
    "name": "코사무이",
    "name_en": "Koh Samui",
    "country": "태국",
    "country_en": "Thailand",
    "lat": 9.5076,
    "lng": 100.0145,
    "tier": 2,
    "popularity": 81,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "태국만에 위치한 코코넛 나무 가득한 열대 낙원입니다. 차웽 비치와 라마이 비치는 각각 다른 매력을 선사하며, 인근의 앙통 국립 해양공원은 카약 투어의 명소입니다. 럭셔리 리조트와 저렴한 방갈로가 공존합니다.",
    "keywords": [
      "태국",
      "열대섬",
      "럭셔리",
      "방갈로",
      "카약"
    ]
  },
  {
    "id": 207,
    "slug": "langkawi",
    "name": "랑카위",
    "name_en": "Langkawi",
    "country": "말레이시아",
    "country_en": "Malaysia",
    "lat": 6.35,
    "lng": 99.8,
    "tier": 2,
    "popularity": 79,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "99개의 섬으로 이루어진 말레이시아의 면세 천국입니다. 스카이 브리지에서 바라보는 열대우림과 바다의 조화는 장관입니다. 맹그로브 숲 카약 투어와 독수리 먹이주기 체험이 인기입니다.",
    "keywords": [
      "말레이시아",
      "면세",
      "스카이브리지",
      "맹그로브",
      "독수리"
    ]
  },
  {
    "id": 208,
    "slug": "krabi",
    "name": "크라비",
    "name_en": "Krabi",
    "country": "태국",
    "country_en": "Thailand",
    "lat": 8.0863,
    "lng": 98.9063,
    "tier": 2,
    "popularity": 80,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "석회암 카르스트 지형이 만들어낸 자연의 걸작입니다. 라일레이 비치는 육로로 접근 불가능한 비밀 해변으로, 암벽 등반의 성지이기도 합니다. 에메랄드 풀과 온천이 있는 정글 트레킹도 즐길 수 있습니다.",
    "keywords": [
      "태국",
      "석회암",
      "라일레이",
      "암벽등반",
      "비밀해변"
    ]
  },
  {
    "id": 209,
    "slug": "cebu",
    "name": "세부",
    "name_en": "Cebu",
    "country": "필리핀",
    "country_en": "Philippines",
    "lat": 10.3157,
    "lng": 123.8854,
    "tier": 2,
    "popularity": 77,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": "southeast-asia",
    "desc": "필리핀 중부의 섬으로, 고래상어와 함께 수영할 수 있는 오슬롭이 유명합니다. 가와산 폭포의 청록색 물과 캐녀닝 액티비티는 모험을 원하는 이들에게 완벽합니다. 막탄 섬의 리조트에서 편안한 휴식도 가능합니다.",
    "keywords": [
      "필리핀",
      "고래상어",
      "폭포",
      "캐녀닝",
      "다이빙"
    ]
  },
  {
    "id": 210,
    "slug": "nha-trang",
    "name": "나트랑",
    "name_en": "Nha Trang",
    "country": "베트남",
    "country_en": "Vietnam",
    "lat": 12.2388,
    "lng": 109.1967,
    "tier": 2,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "베트남 남부의 숨은 보석으로, 6km에 이르는 아름다운 해변을 자랑합니다. 빈펄랜드 테마파크와 머드 스파는 가족 여행객에게 인기입니다. 인근 섬으로의 스노클링 투어에서는 산호초와 다채로운 해양 생물을 만날 수 있습니다.",
    "keywords": [
      "베트남",
      "해변",
      "스파",
      "스노클링",
      "빈펄랜드"
    ]
  },
  {
    "id": 211,
    "slug": "yosemite",
    "name": "요세미티 국립공원",
    "name_en": "Yosemite National Park",
    "country": "미국",
    "country_en": "USA",
    "lat": 37.8651,
    "lng": -119.5383,
    "tier": 2,
    "popularity": 85,
    "continent": "north_america",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "캘리포니아의 대표적인 국립공원으로, 높이 739m의 엘 캐피탄 화강암 절벽과 요세미티 폭포가 장관을 이룹니다. 자이언트 세쿼이아 나무 숲과 하프 돔 등반은 자연 애호가들의 버킷리스트입니다. 존 뮤어가 사랑한 미국 자연보호의 상징입니다.",
    "keywords": [
      "미국",
      "국립공원",
      "엘캐피탄",
      "요세미티폭포",
      "등반"
    ]
  },
  {
    "id": 212,
    "slug": "grand-canyon",
    "name": "그랜드 캐니언",
    "name_en": "Grand Canyon",
    "country": "미국",
    "country_en": "USA",
    "lat": 36.1069,
    "lng": -112.1129,
    "tier": 2,
    "popularity": 90,
    "continent": "north_america",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "콜로라도 강이 수백만 년에 걸쳐 깎아낸 거대한 협곡입니다. 깊이 1,800m, 길이 446km에 달하는 압도적인 스케일은 인간을 겸손하게 만듭니다. 사우스 림에서 바라보는 일출과 일몰은 평생 잊지 못할 순간입니다.",
    "keywords": [
      "미국",
      "협곡",
      "콜로라도강",
      "일출",
      "트레킹"
    ]
  },
  {
    "id": 213,
    "slug": "torres-del-paine",
    "name": "토레스 델 파이네",
    "name_en": "Torres del Paine",
    "country": "칠레",
    "country_en": "Chile",
    "lat": -51,
    "lng": -73,
    "tier": 2,
    "popularity": 78,
    "continent": "south_america",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "칠레 남부 파타고니아의 토레스 델 파이네 국립공원입니다. W·O 트레킹·화강암 봉우리·빙하 호수로 유명하며, 보통 푼타아레나스(PUQ) 관문을 거칩니다. (아르헨티나 북부 파타고니아·우수아이아와 별도 여행지)",
    "keywords": [
      "칠레",
      "파타고니아",
      "트레킹",
      "봉우리",
      "빙하"
    ]
  },
  {
    "id": 214,
    "slug": "milford-sound",
    "name": "밀포드 사운드",
    "name_en": "Milford Sound",
    "country": "뉴질랜드",
    "country_en": "New Zealand",
    "lat": -44.6717,
    "lng": 167.9261,
    "tier": 2,
    "popularity": 82,
    "continent": "oceania",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "러들어드 키플링이 '세계 8번째 불가사의'라고 극찬한 피오르드입니다. 1,200m 높이의 수직 절벽에서 폭포가 쏟아지고, 돌고래와 물개가 유유히 헤엄칩니다. 비 오는 날의 밀포드 사운드가 가장 아름답습니다.",
    "keywords": [
      "뉴질랜드",
      "피오르드",
      "폭포",
      "크루즈",
      "절벽"
    ]
  },
  {
    "id": 215,
    "slug": "plitvice-lakes",
    "name": "플리트비체 호수",
    "name_en": "Plitvice Lakes",
    "country": "크로아티아",
    "country_en": "Croatia",
    "lat": 44.88,
    "lng": 15.62,
    "tier": 2,
    "popularity": 75,
    "continent": "europe",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "16개의 호수가 계단식 폭포로 연결된 동화 같은 국립공원입니다. 에메랄드빛과 터콰이즈색이 교차하는 물빛은 석회암 지형이 만든 자연의 마술입니다. 나무 산책로를 따라 걷는 4시간은 마법 같은 경험입니다.",
    "keywords": [
      "크로아티아",
      "호수",
      "폭포",
      "에메랄드",
      "산책로"
    ]
  },
  {
    "id": 216,
    "slug": "zhangjiajie",
    "name": "장가계",
    "name_en": "Zhangjiajie",
    "country": "중국",
    "country_en": "China",
    "lat": 29.1167,
    "lng": 110.479,
    "tier": 2,
    "popularity": 80,
    "continent": "asia",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "영화 '아바타'의 배경이 된 기암괴석의 숲입니다. 1,000개가 넘는 석영 사암 기둥이 하늘을 향해 솟아있는 모습은 외계 행성 같습니다. 세계 최장 유리 다리에서 즐기는 스릴도 놓칠 수 없습니다.",
    "keywords": [
      "중국",
      "아바타",
      "기암괴석",
      "유리다리",
      "케이블카"
    ]
  },
  {
    "id": 217,
    "slug": "halong-bay",
    "name": "하롱베이",
    "name_en": "Halong Bay",
    "country": "베트남",
    "country_en": "Vietnam",
    "lat": 20.9101,
    "lng": 107.1839,
    "tier": 2,
    "popularity": 83,
    "continent": "asia",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "'하늘에서 내려온 용'이라는 이름처럼 신비로운 바다 위의 석회암 섬들입니다. 1,600개가 넘는 크고 작은 섬과 바위가 만드는 풍경은 수묵화 같습니다. 크루즈를 타고 동굴을 탐험하며 에메랄드빛 바다를 누빕니다.",
    "keywords": [
      "베트남",
      "석회암",
      "크루즈",
      "동굴",
      "UNESCO"
    ]
  },
  {
    "id": 218,
    "slug": "dolomites",
    "name": "돌로미티",
    "name_en": "Dolomites",
    "country": "이탈리아",
    "country_en": "Italy",
    "lat": 46.4102,
    "lng": 11.844,
    "tier": 2,
    "popularity": 77,
    "continent": "europe",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": false,
    "denseRegion": "southern-europe",
    "desc": "북부 이탈리아의 알프스 산맥으로, 독특한 담홍색 석회암이 만드는 '알펜글로우' 현상이 유명합니다. 세첼다와 트레치메 같은 봉우리들은 등반가들의 천국입니다. 겨울에는 스키, 여름에는 하이킹을 즐길 수 있습니다.",
    "keywords": [
      "이탈리아",
      "알프스",
      "등반",
      "스키",
      "알펜글로우"
    ]
  },
  {
    "id": 219,
    "slug": "zermatt",
    "name": "체르마트",
    "name_en": "Zermatt",
    "country": "스위스",
    "country_en": "Switzerland",
    "lat": 46.0207,
    "lng": 7.7491,
    "tier": 2,
    "popularity": 81,
    "continent": "europe",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "마테호른 산 기슭의 작은 마을로, 차 없는 청정 지역입니다. 해발 3,883m 고르너그라트에서 바라보는 29개 4,000m 급 봉우리는 숨이 멎을 듯 장엄합니다. 빙하 특급 열차로 오는 여정도 그 자체로 환상적입니다.",
    "keywords": [
      "스위스",
      "마테호른",
      "알프스",
      "빙하특급",
      "스키"
    ]
  },
  {
    "id": 220,
    "slug": "dublin",
    "name": "더블린",
    "name_en": "Dublin",
    "country": "아일랜드",
    "country_en": "Ireland",
    "lat": 53.3498,
    "lng": -6.2603,
    "tier": 1,
    "popularity": 79,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "아일랜드의 수도로, 문학과 음악이 살아 숨 쉬는 도시입니다. 제임스 조이스와 오스카 와일드의 흔적을 따라 걷고, 템플 바 거리에서 전통 아이리시 음악을 즐길 수 있습니다. 기네스 맥주 공장 투어는 필수 코스입니다.",
    "keywords": [
      "아일랜드",
      "문학",
      "펍",
      "기네스",
      "템플바"
    ]
  },
  {
    "id": 221,
    "slug": "edinburgh",
    "name": "에딘버러",
    "name_en": "Edinburgh",
    "country": "스코틀랜드",
    "country_en": "Scotland",
    "lat": 55.9533,
    "lng": -3.1883,
    "tier": 1,
    "popularity": 81,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "화산 위에 지어진 에딘버러 성과 중세 거리가 조화를 이룬 스코틀랜드의 보석입니다. 해리 포터가 탄생한 카페와 로열 마일의 역사적 건축물들이 매력적입니다. 여름의 프린지 페스티벌은 세계 최대 예술 축제입니다.",
    "keywords": [
      "스코틀랜드",
      "성",
      "해리포터",
      "프린지",
      "중세"
    ]
  },
  {
    "id": 222,
    "slug": "copenhagen",
    "name": "코펜하겐",
    "name_en": "Copenhagen",
    "country": "덴마크",
    "country_en": "Denmark",
    "lat": 55.6761,
    "lng": 12.5683,
    "tier": 1,
    "popularity": 80,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "western-europe",
    "desc": "세계에서 가장 행복한 나라의 수도로, 자전거 천국으로 유명합니다. 뉘하운의 알록달록한 건물과 인어공주 동상, 티볼리 놀이공원이 동화 같은 분위기를 연출합니다. 북유럽 디자인과 미식의 중심지이기도 합니다.",
    "keywords": [
      "덴마크",
      "자전거",
      "뉘하운",
      "인어공주",
      "북유럽디자인"
    ]
  },
  {
    "id": 223,
    "slug": "stockholm",
    "name": "스톡홀름",
    "name_en": "Stockholm",
    "country": "스웨덴",
    "country_en": "Sweden",
    "lat": 59.3293,
    "lng": 18.0686,
    "tier": 1,
    "popularity": 78,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "14개의 섬으로 이루어진 물 위의 도시입니다. 감라스탄 구시가지의 좁은 골목과 왕궁, 바사 박물관의 17세기 전함이 인상적입니다. 노벨상 시상식이 열리는 도시답게 교육과 혁신의 중심지입니다.",
    "keywords": [
      "스웨덴",
      "섬",
      "감라스탄",
      "바사박물관",
      "노벨상"
    ]
  },
  {
    "id": 224,
    "slug": "jakarta",
    "name": "자카르타",
    "name_en": "Jakarta",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -6.2088,
    "lng": 106.8456,
    "tier": 1,
    "popularity": 75,
    "continent": "asia",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": "southeast-asia",
    "desc": "인도네시아의 수도이자 동남아시아 최대 도시 중 하나입니다. 네덜란드 식민지 시대의 코타 투아 구시가지와 현대적인 마천루가 공존합니다. 나시고렝과 사테를 맛보며 다양한 인도네시아 문화를 경험할 수 있습니다.",
    "keywords": [
      "인도네시아",
      "메가시티",
      "코타투아",
      "나시고렝",
      "쇼핑"
    ]
  },
  {
    "id": 225,
    "slug": "kuala-lumpur",
    "name": "쿠알라룸푸르",
    "name_en": "Kuala Lumpur",
    "country": "말레이시아",
    "country_en": "Malaysia",
    "lat": 3.139,
    "lng": 101.6869,
    "tier": 1,
    "popularity": 77,
    "continent": "asia",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "southeast-asia",
    "desc": "페트로나스 트윈 타워로 상징되는 말레이시아의 수도입니다. 바투 동굴의 힌두 사원과 이슬람 사원, 중국 사원이 조화롭게 공존하는 다문화 도시입니다. 저렴한 쇼핑과 다양한 스트리트 푸드가 매력적입니다.",
    "keywords": [
      "말레이시아",
      "페트로나스",
      "바투동굴",
      "다문화",
      "쇼핑"
    ]
  },
  {
    "id": 226,
    "slug": "hanoi",
    "name": "하노이",
    "name_en": "Hanoi",
    "country": "베트남",
    "country_en": "Vietnam",
    "lat": 21.0285,
    "lng": 105.8542,
    "tier": 1,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "1,000년 역사를 자랑하는 베트남의 수도입니다. 구시가지 36개 거리의 활기찬 분위기와 호안끼엠 호수의 평화로운 풍경이 대비를 이룹니다. 포(쌀국수)와 분짜를 맛보며 프랑스 식민지 시대의 건축물을 감상할 수 있습니다.",
    "keywords": [
      "베트남",
      "구시가지",
      "호안끼엠",
      "포",
      "오토바이"
    ]
  },
  {
    "id": 227,
    "slug": "seattle",
    "name": "시애틀",
    "name_en": "Seattle",
    "country": "미국",
    "country_en": "USA",
    "lat": 47.6062,
    "lng": -122.3321,
    "tier": 1,
    "popularity": 74,
    "continent": "north_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "스페이스 니들과 파이크 플레이스 마켓으로 유명한 태평양 연안의 도시입니다. 스타벅스와 아마존의 본사가 있는 혁신의 중심지이며, 그런지 록의 발상지이기도 합니다. 레이니어 산을 배경으로 한 도시 스카이라인이 아름답습니다.",
    "keywords": [
      "미국",
      "스페이스니들",
      "스타벅스",
      "시애틀",
      "IT"
    ]
  },
  {
    "id": 228,
    "slug": "munich",
    "name": "뮌헨",
    "name_en": "Munich",
    "country": "독일",
    "country_en": "Germany",
    "lat": 49.4521,
    "lng": 11.0767,
    "tier": 2,
    "popularity": 77,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "central-europe",
    "desc": "바이에른의 수도로, 맥주와 옥토버페스트의 도시입니다. 마리엔 광장의 신시청사와 님펜부르크 궁전이 화려한 바로크 양식을 자랑합니다. BMW 박물관과 알리안츠 아레나는 현대 독일의 기술력을 보여줍니다.",
    "keywords": [
      "독일",
      "옥토버페스트",
      "맥주",
      "바이에른",
      "BMW"
    ]
  },
  {
    "id": 229,
    "slug": "milan",
    "name": "밀라노",
    "name_en": "Milan",
    "country": "이탈리아",
    "country_en": "Italy",
    "lat": 45.4642,
    "lng": 9.19,
    "tier": 2,
    "popularity": 79,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "southern-europe",
    "desc": "세계 패션의 수도이자 이탈리아 경제의 중심지입니다. 두오모 대성당의 섬세한 고딕 양식과 레오나르도 다 빈치의 '최후의 만찬'이 있는 산타 마리아 델레 그라치에 성당이 유명합니다. 스칼라 극장에서 오페라를 감상하는 것도 특별한 경험입니다.",
    "keywords": [
      "이탈리아",
      "패션",
      "두오모",
      "최후의만찬",
      "쇼핑"
    ]
  },
  {
    "id": 366,
    "slug": "madeira",
    "name": "마데이라",
    "name_en": "Madeira",
    "country": "포르투갈",
    "country_en": "Portugal",
    "lat": 32.7607,
    "lng": -16.9595,
    "tier": 2,
    "popularity": 78,
    "continent": "europe",
    "categories": [
      "nature",
      "adventure"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "대서양의 진주로 불리며, 일 년 내내 봄날 같은 날씨를 자랑합니다. 울창한 숲을 따라 걷는 레바다(Levada) 하이킹이 유명합니다. 푼샬(Funchal)을 거점으로 해안 드라이브·왜인 투어를 즐기기 좋습니다.",
    "keywords": [
      "포르투갈",
      "레바다",
      "하이킹",
      "대서양",
      "푼샬"
    ]
  },
  {
    "id": 230,
    "slug": "lisbon",
    "name": "리스본",
    "name_en": "Lisbon",
    "country": "포르투갈",
    "country_en": "Portugal",
    "lat": 38.7223,
    "lng": -9.1393,
    "tier": 2,
    "popularity": 80,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "7개의 언덕 위에 지어진 포르투갈의 수도로, 노란 트램 28번이 구시가지를 누빕니다. 벨렝 탑과 제로니무스 수도원은 대항해시대의 영광을 보여줍니다. 파스텔톤 건물과 아줄레주 타일이 만드는 풍경이 로맨틱합니다.",
    "keywords": [
      "포르투갈",
      "트램",
      "벨렝탑",
      "파스텔",
      "대항해시대"
    ]
  },
  {
    "id": 231,
    "slug": "porto",
    "name": "포르투",
    "name_en": "Porto",
    "country": "포르투갈",
    "country_en": "Portugal",
    "lat": 41.1579,
    "lng": -8.6291,
    "tier": 2,
    "popularity": 76,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "도루 강변의 알록달록한 건물들이 인상적인 포르투갈 제2의 도시입니다. 포트 와인의 본고장으로, 강 건너편 빌라노바 드 가이아의 와이너리 투어가 필수입니다. 동 루이스 1세 다리에서 바라보는 석양이 환상적입니다.",
    "keywords": [
      "포르투갈",
      "포트와인",
      "도루강",
      "다리",
      "와이너리"
    ]
  },
  {
    "id": 232,
    "slug": "madrid",
    "name": "마드리드",
    "name_en": "Madrid",
    "country": "스페인",
    "country_en": "Spain",
    "lat": 40.4168,
    "lng": -3.7038,
    "tier": 2,
    "popularity": 81,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "southern-europe",
    "desc": "스페인의 열정적인 수도로, 프라도 미술관에는 고야와 벨라스케스의 명작이 가득합니다. 레티로 공원에서 여유를 즐기고, 밤늦게까지 타파스와 와인을 즐기는 마드리드의 밤문화는 특별합니다. 레알 마드리드의 산티아고 베르나베우 경기장도 방문할 가치가 있습니다.",
    "keywords": [
      "스페인",
      "프라도미술관",
      "타파스",
      "레알마드리드",
      "레티로공원"
    ]
  },
  {
    "id": 233,
    "slug": "seville",
    "name": "세비야",
    "name_en": "Seville",
    "country": "스페인",
    "country_en": "Spain",
    "lat": 37.3886,
    "lng": -5.9823,
    "tier": 2,
    "popularity": 75,
    "continent": "europe",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "southern-europe",
    "desc": "플라멩코와 투우의 본고장으로, 안달루시아 지방의 정열이 살아있는 도시입니다. 세비야 대성당은 세계에서 세 번째로 큰 성당이며, 알카사르 궁전은 무데하르 양식의 걸작입니다. 스페인 광장의 타일 벤치에서 각 지방의 역사를 배울 수 있습니다.",
    "keywords": [
      "스페인",
      "플라멩코",
      "알카사르",
      "세비야대성당",
      "타파스"
    ]
  },
  {
    "id": 234,
    "slug": "brussels",
    "name": "브뤼셀",
    "name_en": "Brussels",
    "country": "벨기에",
    "country_en": "Belgium",
    "lat": 50.8503,
    "lng": 4.3517,
    "tier": 2,
    "popularity": 73,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "western-europe",
    "desc": "유럽연합의 수도로 불리는 국제도시입니다. 그랑 플라스의 황금빛 길드 하우스는 유네스코 세계유산이며, 오줌싸개 소년 동상과 벨기에 와플, 초콜릿이 유명합니다. 르네 마그리트 박물관에서 초현실주의 예술을 감상할 수 있습니다.",
    "keywords": [
      "벨기에",
      "그랑플라스",
      "와플",
      "초콜릿",
      "EU"
    ]
  },
  {
    "id": 235,
    "slug": "oslo",
    "name": "오슬로",
    "name_en": "Oslo",
    "country": "노르웨이",
    "country_en": "Norway",
    "lat": 59.9139,
    "lng": 10.7522,
    "tier": 2,
    "popularity": 72,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "피오르드와 숲으로 둘러싸인 노르웨이의 수도입니다. 비겔란 조각 공원의 독특한 인체 조각상과 오페라 하우스의 현대적인 건축이 인상적입니다. 바이킹 박물관에서는 실제 바이킹 선박을 볼 수 있습니다. 겨울에는 오슬로 인근에서 스키를 즐길 수 있습니다.",
    "keywords": [
      "노르웨이",
      "비겔란공원",
      "오페라하우스",
      "바이킹",
      "피오르드"
    ]
  },
  {
    "id": 236,
    "slug": "zurich",
    "name": "취리히",
    "name_en": "Zurich",
    "country": "스위스",
    "country_en": "Switzerland",
    "lat": 47.3769,
    "lng": 8.5417,
    "tier": 2,
    "popularity": 74,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "central-europe",
    "desc": "취리히 호수와 알프스 산맥 사이에 위치한 스위스 최대 도시입니다. 반호프 거리는 세계적인 쇼핑 명소이며, 구시가지의 중세 건축물과 현대적인 금융 중심지가 조화를 이룹니다. 스위스 초콜릿과 시계 박물관도 놓칠 수 없습니다.",
    "keywords": [
      "스위스",
      "취리히호수",
      "금융",
      "초콜릿",
      "시계"
    ]
  },
  {
    "id": 237,
    "slug": "geneva",
    "name": "제네바",
    "name_en": "Geneva",
    "country": "스위스",
    "country_en": "Switzerland",
    "lat": 46.2044,
    "lng": 6.1432,
    "tier": 2,
    "popularity": 73,
    "continent": "europe",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "central-europe",
    "desc": "레만 호수 기슭의 국제도시로, 유엔과 국제적십자사 본부가 있습니다. 140m 높이의 제트 도(분수)와 꽃시계가 랜드마크입니다. 프랑스 국경과 가까워 프랑스 문화의 영향을 받은 미식과 와인을 즐길 수 있습니다.",
    "keywords": [
      "스위스",
      "레만호수",
      "UN",
      "국제도시",
      "분수"
    ]
  },
  {
    "id": 238,
    "slug": "washington-dc",
    "name": "워싱턴 DC",
    "name_en": "Washington DC",
    "country": "미국",
    "country_en": "USA",
    "lat": 38.9072,
    "lng": -77.0369,
    "tier": 2,
    "popularity": 76,
    "continent": "north_america",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "미국의 수도로, 백악관과 국회의사당, 링컨 기념관 등 상징적인 건물들이 모여 있습니다. 스미소니언 박물관군은 모두 무료로 입장할 수 있으며, 국립항공우주박물관과 자연사박물관이 특히 인기입니다. 벚꽃 시즌의 타이달 베이신이 아름답습니다.",
    "keywords": [
      "미국",
      "백악관",
      "스미소니언",
      "벚꽃",
      "수도"
    ]
  },
  {
    "id": 239,
    "slug": "boston",
    "name": "보스턴",
    "name_en": "Boston",
    "country": "미국",
    "country_en": "USA",
    "lat": 42.3601,
    "lng": -71.0589,
    "tier": 2,
    "popularity": 75,
    "continent": "north_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "us-east-coast",
    "desc": "미국에서 가장 오래된 도시 중 하나로, 하버드와 MIT가 있는 교육의 중심지입니다. 프리덤 트레일을 따라 미국 독립혁명의 역사를 배우고, 펜웨이 파크에서 레드삭스 야구를 관람할 수 있습니다. 보스턴 크림 파이와 클램 차우더가 유명합니다.",
    "keywords": [
      "미국",
      "하버드",
      "MIT",
      "프리덤트레일",
      "야구"
    ]
  },
  {
    "id": 240,
    "slug": "miami",
    "name": "마이애미",
    "name_en": "Miami",
    "country": "미국",
    "country_en": "USA",
    "lat": 25.7617,
    "lng": -80.1918,
    "tier": 2,
    "popularity": 79,
    "continent": "north_america",
    "categories": [
      "urban",
      "paradise"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "플로리다 남부의 열대 도시로, 아르데코 건축이 늘어선 사우스 비치가 유명합니다. 라틴 아메리카 문화가 살아있는 리틀 하바나에서 쿠바 커피와 시가를 즐기고, 와인우드 월스에서 스트리트 아트를 감상할 수 있습니다. 크루즈의 출발지로도 인기입니다.",
    "keywords": [
      "미국",
      "사우스비치",
      "아르데코",
      "쿠바",
      "크루즈"
    ]
  },
  {
    "id": 241,
    "slug": "montreal",
    "name": "몬트리올",
    "name_en": "Montreal",
    "country": "캐나다",
    "country_en": "Canada",
    "lat": 45.5017,
    "lng": -73.5673,
    "tier": 2,
    "popularity": 74,
    "continent": "north_america",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "북미의 파리로 불리는 프랑스어권 도시입니다. 올드 몬트리올의 자갈길과 노트르담 대성당이 유럽 같은 분위기를 자아냅니다. 몬트리올 재즈 페스티벌과 퓨티느(감자튀김에 그레이비 소스를 얹은 요리)가 유명합니다. 겨울에는 지하도시에서 쇼핑을 즐길 수 있습니다.",
    "keywords": [
      "캐나다",
      "프랑스어",
      "재즈",
      "퓨티느",
      "올드몬트리올"
    ]
  },
  {
    "id": 242,
    "slug": "angkor-thom",
    "name": "앙코르 톰",
    "name_en": "Angkor Thom",
    "country": "캄보디아",
    "country_en": "Cambodia",
    "lat": 13.4412,
    "lng": 103.8586,
    "tier": 2,
    "popularity": 82,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "southeast-asia",
    "desc": "앙코르 와트 북쪽의 거대한 성곽 도시로, 바이욘 사원의 미소 짓는 얼굴 탑이 유명합니다. 12세기 크메르 제국의 마지막 수도로, 남대문의 거대한 얼굴 조각이 방문객을 맞이합니다. 타 프롬 사원은 나무뿌리가 유적을 감싸 신비로운 분위기를 자아냅니다.",
    "keywords": [
      "캄보디아",
      "크메르",
      "바이욘",
      "타프롬",
      "유적"
    ]
  },
  {
    "id": 243,
    "slug": "borobudur",
    "name": "보로부두르",
    "name_en": "Borobudur",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -7.6079,
    "lng": 110.2038,
    "tier": 2,
    "popularity": 80,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계에서 가장 큰 불교 사원이자 9세기 자바 왕국의 걸작입니다. 9개 층으로 이루어진 피라미드형 구조에 2,672개의 부조와 504개의 불상이 새겨져 있습니다. 일출을 보기 위해 새벽에 방문하면 주변 화산과 안개가 만드는 신비로운 풍경을 볼 수 있습니다.",
    "keywords": [
      "인도네시아",
      "불교",
      "자바",
      "일출",
      "UNESCO"
    ]
  },
  {
    "id": 244,
    "slug": "ayutthaya",
    "name": "아유타야",
    "name_en": "Ayutthaya",
    "country": "태국",
    "country_en": "Thailand",
    "lat": 14.3532,
    "lng": 100.5676,
    "tier": 2,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "southeast-asia",
    "desc": "14-18세기 시암 왕국의 수도로, 400년간 번영했던 도시의 유적이 남아 있습니다. 와트 마하탓의 나무뿌리에 감싸인 불상 머리가 가장 유명한 포토 스팟입니다. 자전거를 타고 유적지를 돌아보는 것이 인기 있는 여행 방식입니다.",
    "keywords": [
      "태국",
      "시암",
      "불교유적",
      "자전거",
      "방콕근교"
    ]
  },
  {
    "id": 245,
    "slug": "luang-prabang",
    "name": "루앙프라방",
    "name_en": "Luang Prabang",
    "country": "라오스",
    "country_en": "Laos",
    "lat": 19.8845,
    "lng": 102.135,
    "tier": 2,
    "popularity": 74,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "메콩 강변의 고대 수도로, 프랑스 식민지 시대 건축과 불교 사원이 조화를 이룹니다. 새벽 탁발 의식은 루앙프라방의 상징이며, 푸시 언덕에서 바라보는 일몰이 아름답습니다. 도시 전체가 유네스코 세계문화유산으로 지정되어 있습니다.",
    "keywords": [
      "라오스",
      "메콩강",
      "불교",
      "탁발",
      "UNESCO"
    ]
  },
  {
    "id": 246,
    "slug": "hue",
    "name": "후에",
    "name_en": "Hue",
    "country": "베트남",
    "country_en": "Vietnam",
    "lat": 16.4637,
    "lng": 107.5909,
    "tier": 2,
    "popularity": 73,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "southeast-asia",
    "desc": "응우옌 왕조의 수도로, 자금성을 본뜬 황제의 궁궐이 있습니다. 향강을 따라 티엔무 사원과 황제 능묘가 자리하고 있으며, 베트남 중부 궁중 요리의 본고장입니다. 향강에서 드래곤 보트를 타고 일몰을 감상하는 것이 낭만적입니다.",
    "keywords": [
      "베트남",
      "황궁",
      "응우옌왕조",
      "향강",
      "궁중요리"
    ]
  },
  {
    "id": 247,
    "slug": "hoi-an",
    "name": "호이안",
    "name_en": "Hoi An",
    "country": "베트남",
    "country_en": "Vietnam",
    "lat": 15.8801,
    "lng": 108.338,
    "tier": 2,
    "popularity": 79,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "15-19세기 동남아시아 무역항으로 번성했던 고대 도시입니다. 노란색 건물과 등불이 어우러진 구시가지는 저녁이 되면 환상적인 분위기를 연출합니다. 맞춤 의류 제작이 유명하며, 24시간 만에 고품질 옷을 제작할 수 있습니다.",
    "keywords": [
      "베트남",
      "등불",
      "구시가지",
      "맞춤의류",
      "UNESCO"
    ]
  },
  {
    "id": 248,
    "slug": "taj-mahal",
    "name": "타지마할",
    "name_en": "Taj Mahal",
    "country": "인도",
    "country_en": "India",
    "lat": 27.1751,
    "lng": 78.0421,
    "tier": 2,
    "popularity": 92,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "무굴 제국 황제가 사랑하는 아내를 위해 지은 세계에서 가장 아름다운 영묘입니다. 흰 대리석에 새겨진 섬세한 꽃무늬와 보석 장식은 22년에 걸쳐 완성되었습니다. 일출과 일몰 때 바라보는 타지마할이 가장 아름다우며, 세계 7대 불가사의 중 하나입니다.",
    "keywords": [
      "인도",
      "무굴제국",
      "대리석",
      "영묘",
      "사랑"
    ]
  },
  {
    "id": 249,
    "slug": "jaipur",
    "name": "자이푸르",
    "name_en": "Jaipur",
    "country": "인도",
    "country_en": "India",
    "lat": 26.9124,
    "lng": 75.7873,
    "tier": 2,
    "popularity": 77,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "핑크빛 건물로 유명한 '핑크 시티'입니다. 언덕 위의 암베르 성에서 코끼리를 타고 올라가는 경험이 특별하며, 하와 마할(바람의 궁전)의 953개 창문이 인상적입니다. 보석과 직물 시장에서 전통 라자스탄 공예품을 구경할 수 있습니다.",
    "keywords": [
      "인도",
      "핑크시티",
      "암베르성",
      "하와마할",
      "코끼리"
    ]
  },
  {
    "id": 250,
    "slug": "varanasi",
    "name": "바라나시",
    "name_en": "Varanasi",
    "country": "인도",
    "country_en": "India",
    "lat": 25.3176,
    "lng": 82.9739,
    "tier": 2,
    "popularity": 75,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "3,000년 역사를 자랑하는 힌두교의 성지입니다. 갠지스 강변의 가트(계단)에서 매일 수천 명이 목욕하고 기도하며, 화장 의식도 이루어집니다. 새벽 보트 투어로 일출을 보며 성스러운 의식을 관찰하는 것은 평생 잊지 못할 경험입니다.",
    "keywords": [
      "인도",
      "갠지스강",
      "힌두교",
      "가트",
      "성지"
    ]
  },
  {
    "id": 251,
    "slug": "hampi",
    "name": "함피",
    "name_en": "Hampi",
    "country": "인도",
    "country_en": "India",
    "lat": 15.335,
    "lng": 76.46,
    "tier": 3,
    "popularity": 70,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "14세기 비자야나가르 제국의 수도 유적지로, 거대한 바위 사이에 흩어진 사원과 궁전이 장관입니다. 비투말라 사원의 음악 기둥을 두드리면 각기 다른 음이 울립니다. 바위산 정상에서 바라보는 일몰 풍경이 환상적입니다.",
    "keywords": [
      "인도",
      "유적지",
      "비자야나가르",
      "바위",
      "일몰"
    ]
  },
  {
    "id": 252,
    "slug": "great-wall",
    "name": "만리장성",
    "name_en": "Great Wall of China",
    "country": "중국",
    "country_en": "China",
    "lat": 40.4319,
    "lng": 116.5704,
    "tier": 2,
    "popularity": 90,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "총 길이 21,196km에 달하는 인류 역사상 가장 거대한 건축물입니다. 베이징 근처의 바다링과 무티안위 구간이 가장 인기 있으며, 산맥을 따라 구불구불 이어지는 성벽의 모습은 경이롭습니다. 가을 단풍 시즌이 가장 아름답습니다.",
    "keywords": [
      "중국",
      "장성",
      "바다링",
      "세계7대불가사의",
      "등산"
    ]
  },
  {
    "id": 253,
    "slug": "forbidden-city",
    "name": "자금성",
    "name_en": "Forbidden City",
    "country": "중국",
    "country_en": "China",
    "lat": 39.9163,
    "lng": 116.3972,
    "tier": 2,
    "popularity": 88,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "명나라와 청나라 24명의 황제가 살았던 거대한 궁궐 복합체입니다. 980개의 건물과 9,000개가 넘는 방으로 이루어져 있으며, 붉은색과 황금색의 화려한 장식이 압도적입니다. 영화 '마지막 황제'의 배경이 된 곳으로, 중국 건축의 정수를 보여줍니다.",
    "keywords": [
      "중국",
      "베이징",
      "황궁",
      "명청",
      "UNESCO"
    ]
  },
  {
    "id": 254,
    "slug": "terracotta-army",
    "name": "병마용",
    "name_en": "Terracotta Army",
    "country": "중국",
    "country_en": "China",
    "lat": 34.3848,
    "lng": 109.2789,
    "tier": 2,
    "popularity": 85,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "진시황의 무덤을 지키는 8,000명 이상의 실물 크기 병사 조각상입니다. 1974년 농부에 의해 우연히 발견된 이 고고학적 보물은 각 병사마다 얼굴과 표정이 다릅니다. 1호 갱의 거대한 규모는 숨이 멎을 정도로 장관입니다.",
    "keywords": [
      "중국",
      "시안",
      "진시황",
      "고고학",
      "병사"
    ]
  },
  {
    "id": 255,
    "slug": "lhasa",
    "name": "라싸",
    "name_en": "Lhasa",
    "country": "중국",
    "country_en": "China",
    "lat": 29.652,
    "lng": 91.172,
    "tier": 3,
    "popularity": 72,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "해발 3,650m에 위치한 티베트의 성지입니다. 포탈라 궁전은 달라이 라마의 겨울 궁전으로, 1,000개가 넘는 방과 13층 높이의 웅장한 건축물입니다. 조캉 사원 주변을 시계방향으로 도는 코라(순례길)에서 티베트 불교의 깊은 신앙심을 느낄 수 있습니다.",
    "keywords": [
      "티베트",
      "포탈라궁",
      "불교",
      "고산",
      "순례"
    ]
  },
  {
    "id": 256,
    "slug": "dunhuang",
    "name": "둔황",
    "name_en": "Dunhuang",
    "country": "중국",
    "country_en": "China",
    "lat": 40.1424,
    "lng": 94.6619,
    "tier": 3,
    "popularity": 68,
    "continent": "asia",
    "categories": [
      "culture",
      "adventure"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "실크로드의 중요한 오아시스 도시로, 막고굴의 불교 벽화가 유명합니다. 492개 석굴에 그려진 1,000년에 걸친 벽화는 불교 예술의 보고입니다. 명사산(울음 모래 언덕)에서 낙타를 타고 월아천(초승달 샘)을 방문하는 사막 체험도 할 수 있습니다.",
    "keywords": [
      "중국",
      "실크로드",
      "막고굴",
      "사막",
      "불교벽화"
    ]
  },
  {
    "id": 257,
    "slug": "persepolis",
    "name": "페르세폴리스",
    "name_en": "Persepolis",
    "country": "이란",
    "country_en": "Iran",
    "lat": 29.9352,
    "lng": 52.8916,
    "tier": 2,
    "popularity": 74,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "기원전 5세기 페르시아 제국의 수도 유적입니다. 아파다나 궁전의 계단에 새겨진 각국 사절단의 부조는 고대 페르시아의 위대함을 보여줍니다. 알렉산더 대왕이 불태웠지만, 여전히 남아있는 거대한 기둥과 문은 당시의 영광을 증언합니다.",
    "keywords": [
      "이란",
      "페르시아",
      "고대유적",
      "아케메네스",
      "UNESCO"
    ]
  },
  {
    "id": 258,
    "slug": "cappadocia",
    "name": "카파도키아",
    "name_en": "Cappadocia",
    "country": "튀르키예",
    "country_en": "Turkey",
    "lat": 38.6431,
    "lng": 34.8286,
    "tier": 2,
    "popularity": 83,
    "continent": "asia",
    "categories": [
      "culture",
      "nature"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "화산재가 굳어 만들어진 기암괴석 지대로, 요정의 굴뚝이라 불리는 독특한 바위 기둥이 펼쳐집니다. 초기 기독교인들이 파낸 동굴 교회와 지하 도시가 남아있으며, 열기구를 타고 일출을 보는 경험은 평생 잊지 못할 순간입니다.",
    "keywords": [
      "튀르키예",
      "열기구",
      "동굴",
      "요정의굴뚝",
      "일출"
    ]
  },
  {
    "id": 259,
    "slug": "ephesus",
    "name": "에페소스",
    "name_en": "Ephesus",
    "country": "튀르키예",
    "country_en": "Turkey",
    "lat": 37.9395,
    "lng": 27.3408,
    "tier": 2,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "그리스-로마 시대 소아시아 최대 도시의 유적입니다. 셀수스 도서관의 정교한 파사드와 2만 5천 명을 수용하던 대극장이 보존 상태가 좋습니다. 세계 7대 불가사의 중 하나였던 아르테미스 신전 터도 근처에 있습니다.",
    "keywords": [
      "튀르키예",
      "고대로마",
      "도서관",
      "유적",
      "아르테미스"
    ]
  },
  {
    "id": 260,
    "slug": "luxor",
    "name": "룩소르",
    "name_en": "Luxor",
    "country": "이집트",
    "country_en": "Egypt",
    "lat": 25.6872,
    "lng": 32.6396,
    "tier": 2,
    "popularity": 81,
    "continent": "africa",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "고대 이집트 테베의 수도로, 세계에서 가장 큰 야외 박물관이라 불립니다. 카르나크 신전의 거대한 기둥과 룩소르 신전이 나일 강 동안에 있고, 서안에는 왕들의 계곡과 하트셉수트 여왕의 장제전이 있습니다. 열기구를 타고 유적지를 내려다보는 것이 인기입니다.",
    "keywords": [
      "이집트",
      "나일강",
      "카르나크",
      "왕들의계곡",
      "열기구"
    ]
  },
  {
    "id": 261,
    "slug": "abu-simbel",
    "name": "아부 심벨",
    "name_en": "Abu Simbel",
    "country": "이집트",
    "country_en": "Egypt",
    "lat": 22.3372,
    "lng": 31.6258,
    "tier": 3,
    "popularity": 73,
    "continent": "africa",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "람세스 2세가 세운 거대한 암굴 신전으로, 20m 높이의 파라오 좌상 4개가 입구를 지키고 있습니다. 아스완 댐 건설 시 유네스코가 신전 전체를 60m 위로 이동시킨 놀라운 역사가 있습니다. 연 2회 태양빛이 내부 성소까지 비추는 태양 축제가 유명합니다.",
    "keywords": [
      "이집트",
      "람세스",
      "암굴신전",
      "이전",
      "태양축제"
    ]
  },
  {
    "id": 262,
    "slug": "fez",
    "name": "페즈",
    "name_en": "Fez",
    "country": "모로코",
    "country_en": "Morocco",
    "lat": 34.0331,
    "lng": -5.0003,
    "tier": 2,
    "popularity": 75,
    "continent": "africa",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "9,000개가 넘는 골목이 미로처럼 얽힌 세계에서 가장 큰 차 없는 중세 도시입니다. 가죽 염색장의 형형색색 물감통과 독특한 냄새가 인상적이며, 카라위인 대학은 세계에서 가장 오래된 대학입니다. 메디나를 헤매며 장인들의 작업을 구경하는 것이 매력적입니다.",
    "keywords": [
      "모로코",
      "메디나",
      "가죽염색",
      "미로",
      "전통공예"
    ]
  },
  {
    "id": 263,
    "slug": "timbuktu",
    "name": "팀북투",
    "name_en": "Timbuktu",
    "country": "말리",
    "country_en": "Mali",
    "lat": 16.7666,
    "lng": -3.0026,
    "tier": 3,
    "popularity": 65,
    "continent": "africa",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "사하라 사막의 전설적인 도시로, 중세 시대 이슬람 학문과 금무역의 중심지였습니다. 흙 벽돌로 지어진 모스크와 수만 권의 고대 필사본이 보관된 도서관이 있습니다. '팀북투로 간다'는 표현이 '세상 끝으로 간다'는 의미로 쓰일 만큼 외진 곳입니다.",
    "keywords": [
      "말리",
      "사하라",
      "고대도시",
      "필사본",
      "사막"
    ]
  },
  {
    "id": 264,
    "slug": "lalibela",
    "name": "랄리벨라",
    "name_en": "Lalibela",
    "country": "에티오피아",
    "country_en": "Ethiopia",
    "lat": 12.0308,
    "lng": 39.0409,
    "tier": 3,
    "popularity": 69,
    "continent": "africa",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "12세기에 암반을 깎아 만든 11개의 암굴 교회가 있는 에티오피아 정교회의 성지입니다. 가장 유명한 성 게오르기오스 교회는 십자가 모양으로 바위 속에 새겨져 있습니다. '아프리카의 페트라'로 불리며, 순례자들이 지금도 찾아오는 살아있는 유적지입니다.",
    "keywords": [
      "에티오피아",
      "암굴교회",
      "기독교",
      "순례",
      "십자가"
    ]
  },
  {
    "id": 265,
    "slug": "cusco",
    "name": "쿠스코",
    "name_en": "Cusco",
    "country": "페루",
    "country_en": "Peru",
    "lat": -13.5319,
    "lng": -71.9675,
    "tier": 2,
    "popularity": 80,
    "continent": "south_america",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "잉카 제국의 수도로, 정교하게 맞춘 돌담이 도시 곳곳에 남아있습니다. 사크사이와만 요새의 거대한 석축은 현대 기술로도 재현하기 어려운 수준입니다. 마추픽추로 가는 관문이지만, 쿠스코 자체도 매력적인 역사 도시입니다. 해발 3,400m의 고산 지대이므로 고산병 주의가 필요합니다.",
    "keywords": [
      "페루",
      "잉카",
      "마추픽추관문",
      "돌담",
      "고산"
    ]
  },
  {
    "id": 266,
    "slug": "nazca-lines",
    "name": "나스카 라인",
    "name_en": "Nazca Lines",
    "country": "페루",
    "country_en": "Peru",
    "lat": -14.739,
    "lng": -75.13,
    "tier": 3,
    "popularity": 71,
    "continent": "south_america",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "사막에 그려진 거대한 지상화로, 벌새, 원숭이, 거미 등 70개 이상의 그림이 있습니다. 가장 큰 것은 길이 370m에 달하며, 기원전 500년~서기 500년 사이에 제작되었습니다. 경비행기를 타고 상공에서 내려다봐야 전체 모습을 볼 수 있는 미스터리한 유적입니다.",
    "keywords": [
      "페루",
      "지상화",
      "경비행기",
      "미스터리",
      "UNESCO"
    ]
  },
  {
    "id": 267,
    "slug": "tikal",
    "name": "티칼",
    "name_en": "Tikal",
    "country": "과테말라",
    "country_en": "Guatemala",
    "lat": 17.2221,
    "lng": -89.6236,
    "tier": 3,
    "popularity": 72,
    "continent": "north_america",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "정글 속에 숨겨진 마야 문명 최대 도시 유적입니다. 6개의 거대한 피라미드 신전이 열대우림 위로 솟아있으며, 4호 신전 꼭대기에서 바라보는 일출이 장관입니다. 원숭이와 앵무새의 울음소리가 들리는 가운데 고대 마야의 영광을 느낄 수 있습니다.",
    "keywords": [
      "과테말라",
      "마야",
      "정글",
      "피라미드",
      "일출"
    ]
  },
  {
    "id": 268,
    "slug": "santiago-de-compostela",
    "name": "산티아고 데 콤포스텔라",
    "name_en": "Santiago de Compostela",
    "country": "스페인",
    "country_en": "Spain",
    "lat": 42.8805,
    "lng": -8.5456,
    "tier": 2,
    "popularity": 77,
    "continent": "europe",
    "categories": [
      "culture"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "순례자의 길(카미노)의 종착지로, 사도 야고보의 유해가 안치된 대성당이 있습니다. 중세부터 지금까지 수백만 명의 순례자가 걸어온 유서 깊은 성지입니다. 구시가지 전체가 유네스코 세계문화유산이며, 순례자 여권에 찍는 도장이 특별한 기념이 됩니다.",
    "keywords": [
      "스페인",
      "순례자의길",
      "카미노",
      "대성당",
      "순례"
    ]
  },
  {
    "id": 269,
    "slug": "everest-base-camp",
    "name": "에베레스트 베이스캠프",
    "name_en": "Everest Base Camp",
    "country": "네팔",
    "country_en": "Nepal",
    "lat": 28.0026,
    "lng": 86.8528,
    "tier": 2,
    "popularity": 85,
    "continent": "asia",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계 최고봉 에베레스트(8,849m) 기슭까지 가는 트레킹의 성지입니다. 해발 5,364m의 베이스캠프까지 12-14일간의 여정은 고산병과의 싸움이지만, 쿰부 빙하와 히말라야 봉우리들의 장엄한 모습이 모든 고통을 보상합니다. 셰르파 마을과 불교 사원이 있는 남체 바자르가 중간 거점입니다.",
    "keywords": [
      "네팔",
      "에베레스트",
      "트레킹",
      "고산",
      "히말라야"
    ]
  },
  {
    "id": 270,
    "slug": "annapurna-circuit",
    "name": "안나푸르나 서킷",
    "name_en": "Annapurna Circuit",
    "country": "네팔",
    "country_en": "Nepal",
    "lat": 28.5967,
    "lng": 83.8206,
    "tier": 2,
    "popularity": 82,
    "continent": "asia",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "세계 10대 트레킹 코스로 꼽히는 160km의 순환 루트입니다. 5,416m의 토롱 라 고갯길을 넘어 열대부터 고산까지 다양한 기후대를 경험하며, 안나푸르나 봉우리들의 360도 파노라마를 감상할 수 있습니다. 15-20일이 소요되는 본격적인 고산 트레킹입니다.",
    "keywords": [
      "네팔",
      "안나푸르나",
      "트레킹",
      "고갯길",
      "히말라야"
    ]
  },
  {
    "id": 271,
    "slug": "kilimanjaro",
    "name": "킬리만자로",
    "name_en": "Mount Kilimanjaro",
    "country": "탄자니아",
    "country_en": "Tanzania",
    "lat": -3.0674,
    "lng": 37.3556,
    "tier": 2,
    "popularity": 80,
    "continent": "africa",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아프리카 대륙 최고봉(5,895m)으로, 기술 등반 없이 오를 수 있는 세계에서 가장 높은 산입니다. 적도 지역이지만 정상에는 만년설이 있으며, 5-7일간의 등반 과정에서 열대우림부터 빙하까지 5개 기후대를 통과합니다. 우후루 피크 정상에서 보는 일출은 평생 잊지 못할 순간입니다.",
    "keywords": [
      "탄자니아",
      "등반",
      "최고봉",
      "만년설",
      "일출"
    ]
  },
  {
    "id": 272,
    "slug": "mont-blanc",
    "name": "몽블랑",
    "name_en": "Mont Blanc",
    "country": "프랑스",
    "country_en": "France",
    "lat": 45.8326,
    "lng": 6.8652,
    "tier": 2,
    "popularity": 78,
    "continent": "europe",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "서유럽 최고봉(4,810m)으로, 샤모니 계곡을 중심으로 한 알프스 최고의 등산 명소입니다. 투르 뒤 몽블랑(TMB) 트레킹은 프랑스, 이탈리아, 스위스를 거치는 170km의 환상적인 코스입니다. 에귀유 뒤 미디 케이블카로 해발 3,842m까지 올라가 빙하와 바위 봉우리를 가까이서 볼 수 있습니다.",
    "keywords": [
      "프랑스",
      "알프스",
      "등반",
      "샤모니",
      "빙하"
    ]
  },
  {
    "id": 273,
    "slug": "komodo-island",
    "name": "코모도 섬",
    "name_en": "Komodo Island",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -8.55,
    "lng": 119.45,
    "tier": 2,
    "popularity": 77,
    "continent": "asia",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계 최대 도마뱀인 코모도 왕도마뱀(최대 3m)의 서식지입니다. 레인저와 함께 트레킹하며 야생 코모도 드래곤을 관찰하는 스릴 넘치는 경험을 할 수 있습니다. 핑크 비치의 독특한 분홍빛 모래와 주변 섬의 다이빙 포인트도 세계적입니다. 리브어보드 크루즈로 여러 섬을 탐험하는 것이 인기입니다.",
    "keywords": [
      "인도네시아",
      "코모도드래곤",
      "트레킹",
      "핑크비치",
      "다이빙"
    ]
  },
  {
    "id": 274,
    "slug": "raja-ampat",
    "name": "라자 암팟",
    "name_en": "Raja Ampat",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -0.2297,
    "lng": 130.5186,
    "tier": 3,
    "popularity": 75,
    "continent": "asia",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계에서 가장 생물 다양성이 높은 바다로, 지구상 산호초 종의 75%가 서식합니다. 1,500개가 넘는 섬으로 이루어진 군도로, 스쿠버 다이빙과 스노클링 천국입니다. 카르스트 지형의 섬들이 만드는 풍경이 장관이며, 접근성이 떨어져 순수한 자연이 보존되어 있습니다.",
    "keywords": [
      "인도네시아",
      "다이빙",
      "산호초",
      "섬",
      "생물다양성"
    ]
  },
  {
    "id": 275,
    "slug": "borneo",
    "name": "보르네오",
    "name_en": "Borneo",
    "country": "말레이시아",
    "country_en": "Malaysia",
    "lat": 0.9619,
    "lng": 114.5548,
    "tier": 2,
    "popularity": 74,
    "continent": "asia",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계에서 세 번째로 큰 섬으로, 1억 4천만 년 된 열대우림이 펼쳐집니다. 오랑우탄, 피그미코끼리, 코뿔새 등 희귀 동물의 보고이며, 키나발루 산(4,095m) 등반과 정글 트레킹을 즐길 수 있습니다. 세피록 오랑우탄 센터와 키나바탕안 강의 반딧불이 투어가 유명합니다.",
    "keywords": [
      "말레이시아",
      "열대우림",
      "오랑우탄",
      "키나발루",
      "정글"
    ]
  },
  {
    "id": 276,
    "slug": "madagascar",
    "name": "마다가스카르",
    "name_en": "Madagascar",
    "country": "마다가스카르",
    "country_en": "Madagascar",
    "lat": -18.7669,
    "lng": 46.8691,
    "tier": 3,
    "popularity": 72,
    "continent": "africa",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "8,800만 년 전 아프리카 대륙에서 분리된 섬으로, 동식물의 90%가 고유종입니다. 여우원숭이, 바오밥 나무, 카멜레온 등 독특한 생태계를 볼 수 있습니다. 칭기 데 베마라하의 바위 숲과 이살로 국립공원의 기암괴석이 장관입니다. 미개척 오지 탐험의 매력이 있습니다.",
    "keywords": [
      "마다가스카르",
      "여우원숭이",
      "바오밥",
      "고유종",
      "오지"
    ]
  },
  {
    "id": 277,
    "slug": "costa-rica",
    "name": "코스타리카",
    "name_en": "Costa Rica",
    "country": "코스타리카",
    "country_en": "Costa Rica",
    "lat": 9.7489,
    "lng": -83.7534,
    "tier": 2,
    "popularity": 79,
    "continent": "north_america",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "중미의 에코 투어리즘 천국으로, 국토의 25%가 국립공원입니다. 집라인으로 열대우림 숲을 가로지르고, 화산 하이킹, 급류 래프팅, 서핑을 즐길 수 있습니다. 몬테베르데 구름 숲에서는 케찰새를, 토르투게로에서는 바다거북 산란을 볼 수 있습니다. '푸라 비다(순수한 삶)'의 정신이 살아있는 곳입니다.",
    "keywords": [
      "코스타리카",
      "에코투어",
      "집라인",
      "화산",
      "열대우림"
    ]
  },
  {
    "id": 278,
    "slug": "inca-trail",
    "name": "잉카 트레일",
    "name_en": "Inca Trail",
    "country": "페루",
    "country_en": "Peru",
    "lat": -13.1631,
    "lng": -72.545,
    "tier": 2,
    "popularity": 81,
    "continent": "south_america",
    "categories": [
      "adventure",
      "culture"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "마추픽추로 가는 고대 잉카의 돌길을 따라 걷는 4일 트레킹입니다. 해발 4,200m의 와르미와누스카(죽은 여인의 고개)를 넘고, 윈야이와이나 유적을 거쳐 새벽에 태양의 문으로 마추픽추에 입성하는 감동적인 여정입니다. 하루 500명으로 인원 제한되어 사전 예약이 필수입니다.",
    "keywords": [
      "페루",
      "마추픽추",
      "트레킹",
      "잉카",
      "고산"
    ]
  },
  {
    "id": 279,
    "slug": "peninsula-valdes",
    "name": "발데스 반도",
    "name_en": "Peninsula Valdes",
    "country": "아르헨티나",
    "country_en": "Argentina",
    "lat": -42.5,
    "lng": -63.6,
    "tier": 3,
    "popularity": 71,
    "continent": "south_america",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남극 고래, 코끼리 물개, 펭귄, 범고래 등을 관찰할 수 있는 해양 생물의 천국입니다. 특히 9-11월에는 남극 고래가 새끼를 낳으러 오며, 범고래가 해변으로 돌진해 물개를 사냥하는 장면을 볼 수 있습니다. 푼타 톰보의 펭귄 서식지는 남미 최대 규모입니다.",
    "keywords": [
      "아르헨티나",
      "고래",
      "물개",
      "펭귄",
      "야생동물"
    ]
  },
  {
    "id": 300,
    "slug": "ibiza",
    "name": "이비사",
    "name_en": "Ibiza",
    "country": "스페인",
    "country_en": "Spain",
    "lat": 38.98,
    "lng": 1.43,
    "tier": 2,
    "popularity": 78,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "지중해의 파티 아일랜드로 유명하지만 아름다운 해변과 고요한 만도 많습니다. 클럽 문화와 해변 휴양이 공존하는 독특한 섬입니다.",
    "keywords": [
      "파티",
      "해변",
      "클럽",
      "지중해"
    ]
  },
  {
    "id": 301,
    "slug": "crete",
    "name": "크레타",
    "name_en": "Crete",
    "country": "그리스",
    "country_en": "Greece",
    "lat": 35.24,
    "lng": 24.81,
    "tier": 2,
    "popularity": 76,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "그리스 최대의 섬으로 에게해의 아름다운 해변, 고대 미노아 문명 유적, 사마리아 협곡 등 다양한 볼거리를 자랑합니다.",
    "keywords": [
      "그리스",
      "해변",
      "역사",
      "협곡"
    ]
  },
  {
    "id": 302,
    "slug": "corsica",
    "name": "코르시카",
    "name_en": "Corsica",
    "country": "프랑스",
    "country_en": "France",
    "lat": 42.04,
    "lng": 9.01,
    "tier": 2,
    "popularity": 72,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "프랑스령 지중해 섬으로 험준한 산과 아름다운 해변이 조화를 이룹니다. GR20 트레킹 코스로 유명하며, 나폴레옹의 고향이기도 합니다.",
    "keywords": [
      "지중해",
      "트레킹",
      "해변",
      "프랑스"
    ]
  },
  {
    "id": 303,
    "slug": "sicily",
    "name": "시칠리아",
    "name_en": "Sicily",
    "country": "이탈리아",
    "country_en": "Italy",
    "lat": 37.6,
    "lng": 14.01,
    "tier": 2,
    "popularity": 80,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "이탈리아 최대의 섬으로 에트나 화산, 바로크 건축, 그리스 신전 유적이 공존합니다. 지중해 요리의 정수를 맛볼 수 있는 미식의 섬입니다.",
    "keywords": [
      "화산",
      "역사",
      "미식",
      "지중해"
    ]
  },
  {
    "id": 304,
    "slug": "hvar",
    "name": "흐바르",
    "name_en": "Hvar",
    "country": "크로아티아",
    "country_en": "Croatia",
    "lat": 43.17,
    "lng": 16.44,
    "tier": 2,
    "popularity": 74,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "크로아티아의 보석 같은 섬으로 라벤더 향기와 맑은 아드리아해가 어우러집니다. 유럽 부호들의 휴양지로 알려진 고급스러운 섬입니다.",
    "keywords": [
      "아드리아해",
      "라벤더",
      "럭셔리",
      "항구"
    ]
  },
  {
    "id": 305,
    "slug": "bodrum",
    "name": "보드룸",
    "name_en": "Bodrum",
    "country": "터키",
    "country_en": "Turkey",
    "lat": 37.03,
    "lng": 27.43,
    "tier": 2,
    "popularity": 75,
    "continent": "europe",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "에게해 연안의 터키 휴양지로 하얀 건물과 파란 바다가 아름답습니다. 고대 7대 불가사의 중 하나인 마우솔로스 영묘가 있던 곳입니다.",
    "keywords": [
      "에게해",
      "리조트",
      "역사",
      "요트"
    ]
  },
  {
    "id": 306,
    "slug": "bohol",
    "name": "보홀",
    "name_en": "Bohol",
    "country": "필리핀",
    "country_en": "Philippines",
    "lat": 9.85,
    "lng": 124.14,
    "tier": 2,
    "popularity": 73,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "필리핀의 숨겨진 보석으로 초콜릿 힐스, 세계에서 가장 작은 원숭이 안경원숭이, 맑은 바닷속 다이빙 명소를 자랑합니다.",
    "keywords": [
      "초콜릿힐스",
      "다이빙",
      "원숭이",
      "해변"
    ]
  },
  {
    "id": 307,
    "slug": "rocky-mountains",
    "name": "로키 산맥",
    "name_en": "Rocky Mountains",
    "country": "미국",
    "country_en": "United States",
    "lat": 40.34,
    "lng": -105.69,
    "tier": 2,
    "popularity": 80,
    "continent": "north_america",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "북미 서부를 관통하는 거대한 산맥으로 웅장한 봉우리, 빙하호, 야생동물이 공존합니다. 로키 마운틴 국립공원은 하이킹의 천국입니다.",
    "keywords": [
      "국립공원",
      "하이킹",
      "야생동물",
      "산맥"
    ]
  },
  {
    "id": 308,
    "slug": "wadi-rum",
    "name": "요르단 와디 럼",
    "name_en": "Wadi Rum",
    "country": "요르단",
    "country_en": "Jordan",
    "lat": 29.58,
    "lng": 35.42,
    "tier": 2,
    "popularity": 72,
    "continent": "middle_east",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "영화 마션과 듄의 촬영지로 화성 같은 붉은 사막 풍경이 펼쳐집니다. 베두인 캠프에서의 사막 캠핑이 인기입니다.",
    "keywords": [
      "사막",
      "캠핑",
      "영화",
      "베두인"
    ]
  },
  {
    "id": 309,
    "slug": "kamchatka",
    "name": "캄차카",
    "name_en": "Kamchatka",
    "country": "러시아",
    "country_en": "Russia",
    "lat": 56,
    "lng": 160,
    "tier": 3,
    "popularity": 58,
    "continent": "unknown",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "러시아 극동의 화산 반도로 300개 이상의 화산과 간헐천, 온천이 있습니다. 곰과 연어가 공존하는 원시 자연의 보고입니다.",
    "keywords": [
      "화산",
      "온천",
      "곰",
      "원시자연"
    ]
  },
  {
    "id": 310,
    "slug": "fiordland",
    "name": "피오르드랜드",
    "name_en": "Fiordland",
    "country": "뉴질랜드",
    "country_en": "New Zealand",
    "lat": -45.42,
    "lng": 167.72,
    "tier": 2,
    "popularity": 75,
    "continent": "oceania",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "뉴질랜드 남섬 남서부의 피오르드 지역으로 밀포드 사운드를 포함합니다. 세계에서 가장 습한 지역 중 하나로 장엄한 폭포가 많습니다.",
    "keywords": [
      "피오르드",
      "폭포",
      "트레킹",
      "자연"
    ]
  },
  {
    "id": 311,
    "slug": "tahiti",
    "name": "타히티",
    "name_en": "Tahiti",
    "country": "프랑스령 폴리네시아",
    "country_en": "French Polynesia",
    "lat": -17.68,
    "lng": -149.41,
    "tier": 2,
    "popularity": 78,
    "continent": "oceania",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "프랑스령 폴리네시아의 중심 섬으로 화산 봉우리와 검은 모래 해변이 특징입니다. 폴 고갱이 사랑한 예술가들의 영감의 섬입니다.",
    "keywords": [
      "폴리네시아",
      "화산",
      "해변",
      "예술"
    ]
  },
  {
    "id": 312,
    "slug": "sapa",
    "name": "사파",
    "name_en": "Sapa",
    "country": "베트남",
    "country_en": "Vietnam",
    "lat": 22.34,
    "lng": 103.84,
    "tier": 2,
    "popularity": 74,
    "continent": "asia",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "베트남 북부 고산지대의 계단식 논밭이 장관을 이룹니다. 소수민족 마을을 방문하며 트레킹할 수 있는 베트남의 스위스입니다.",
    "keywords": [
      "계단식논",
      "트레킹",
      "소수민족",
      "고산"
    ]
  },
  {
    "id": 313,
    "slug": "aconcagua",
    "name": "아콩카과",
    "name_en": "Aconcagua",
    "country": "아르헨티나",
    "country_en": "Argentina",
    "lat": -32.65,
    "lng": -70.01,
    "tier": 3,
    "popularity": 62,
    "continent": "unknown",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남미 최고봉(6,961m)이자 히말라야 밖 세계 최고봉입니다. 7대륙 최고봉 등반가들의 필수 코스로 기술 등반 없이 정상에 도전할 수 있습니다.",
    "keywords": [
      "등반",
      "최고봉",
      "7대륙",
      "안데스"
    ]
  },
  {
    "id": 314,
    "slug": "denali",
    "name": "데날리",
    "name_en": "Denali",
    "country": "미국",
    "country_en": "United States",
    "lat": 63.07,
    "lng": -151.01,
    "tier": 3,
    "popularity": 64,
    "continent": "north_america",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "북미 최고봉(6,190m)으로 매킨리 산으로도 불립니다. 극한의 추위와 고도로 에베레스트보다 등반 성공률이 낮은 험난한 산입니다.",
    "keywords": [
      "등반",
      "최고봉",
      "극한",
      "알래스카"
    ]
  },
  {
    "id": 315,
    "slug": "carstensz-pyramid",
    "name": "카르스텐츠 피라미드",
    "name_en": "Carstensz Pyramid",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -4.08,
    "lng": 137.18,
    "tier": 3,
    "popularity": 55,
    "continent": "asia",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "오세아니아 최고봉(4,884m)이자 7대륙 최고봉 중 하나입니다. 적도 부근의 빙하와 기술 등반이 필요한 도전적인 산입니다.",
    "keywords": [
      "등반",
      "최고봉",
      "7대륙",
      "빙하"
    ]
  },
  {
    "id": 316,
    "slug": "sequoia-national-park",
    "name": "시퀘이아 국립공원",
    "name_en": "Sequoia National Park",
    "country": "미국",
    "country_en": "United States",
    "lat": 36.49,
    "lng": -118.57,
    "tier": 2,
    "popularity": 76,
    "continent": "north_america",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계에서 가장 거대한 나무인 자이언트 세쿼이아가 숲을 이루는 국립공원입니다. 3,000년 된 거대 나무 아래서 자연의 경이를 느낄 수 있습니다.",
    "keywords": [
      "세쿼이아",
      "거대나무",
      "국립공원",
      "하이킹"
    ]
  },
  {
    "id": 317,
    "slug": "arequipa",
    "name": "아레키파",
    "name_en": "Arequipa",
    "country": "페루",
    "country_en": "Peru",
    "lat": -16.41,
    "lng": -71.54,
    "tier": 2,
    "popularity": 68,
    "continent": "unknown",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "페루 남부의 화산 도시로 콜카 캐니언 트레킹의 출발점입니다. 흰색 화산암으로 지은 건축물로 '하얀 도시'로 불립니다.",
    "keywords": [
      "화산",
      "협곡",
      "트레킹",
      "콘도르"
    ]
  },
  {
    "id": 318,
    "slug": "similan-islands",
    "name": "시미란 제도",
    "name_en": "Similan Islands",
    "country": "태국",
    "country_en": "Thailand",
    "lat": 8.65,
    "lng": 97.64,
    "tier": 2,
    "popularity": 72,
    "continent": "asia",
    "categories": [
      "adventure"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "태국의 다이빙 천국으로 세계 10대 다이빙 포인트 중 하나입니다. 만타 가오리와 고래상어를 볼 수 있는 맑고 투명한 바다를 자랑합니다.",
    "keywords": [
      "다이빙",
      "스쿠버",
      "해양",
      "국립공원"
    ]
  },
  {
    "id": 319,
    "slug": "cinque-terre",
    "name": "친퀘테레",
    "name_en": "Cinque Terre",
    "country": "이탈리아",
    "country_en": "Italy",
    "lat": 44.12,
    "lng": 9.72,
    "tier": 2,
    "popularity": 79,
    "continent": "europe",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "이탈리아 리구리아 해안의 다섯 마을로 절벽 위에 세워진 형형색색의 집들이 지중해와 어우러져 그림 같은 풍경을 만듭니다. 해안 트레킹 코스가 유명합니다.",
    "keywords": [
      "해안",
      "마을",
      "트레킹",
      "UNESCO"
    ]
  },
  {
    "id": 320,
    "slug": "mount-bromo",
    "name": "브로모 화산",
    "name_en": "Mount Bromo",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -7.94,
    "lng": 112.95,
    "tier": 2,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "nature"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "인도네시아 자바 섬의 활화산으로 거대한 분화구 안에 있는 신비로운 풍경을 자랑합니다. 일출 시 바다 같은 화산재 평원 위로 솟은 분화구가 장관을 이룹니다.",
    "keywords": [
      "화산",
      "일출",
      "분화구",
      "트레킹"
    ]
  }
,
  {
    "id": 321,
    "slug": "da-nang",
    "name": "다낭",
    "name_en": "Da Nang",
    "country": "베트남",
    "country_en": "Vietnam",
    "lat": 16.0544,
    "lng": 108.2022,
    "tier": 2,
    "popularity": 85,
    "continent": "asia",
    "categories": [
      "paradise",
      "urban"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "아름다운 해변과 저렴한 물가로 한국인에게 가장 사랑받는 베트남 대표 휴양지입니다. 인근 호이안과 연계하여 가족 여행으로 완벽한 코스를 자랑합니다.",
    "keywords": [
      "아시아",
      "가족여행",
      "가성비",
      "해변",
      "효도여행"
    ]
  },
  {
    "id": 322,
    "slug": "sapporo",
    "name": "삿포로",
    "name_en": "Sapporo",
    "country": "일본",
    "country_en": "Japan",
    "lat": 43.0618,
    "lng": 141.3545,
    "tier": 2,
    "popularity": 80,
    "continent": "asia",
    "categories": [
      "urban",
      "nature"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "눈의 왕국 홋카이도의 중심지입니다. 겨울에는 아름다운 설경과 눈축제를, 여름에는 시원하고 쾌적한 날씨 속에서 라벤더 축제를 즐길 수 있습니다.",
    "keywords": [
      "아시아",
      "겨울여행",
      "설경",
      "미식",
      "가족여행"
    ]
  },
  {
    "id": 323,
    "slug": "osaka",
    "name": "오사카",
    "name_en": "Osaka",
    "country": "일본",
    "country_en": "Japan",
    "lat": 34.6937,
    "lng": 135.5023,
    "tier": 1,
    "popularity": 92,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "'먹다가 망한다'는 말이 있을 정도로 식도락 여행의 성지입니다. 도톤보리의 화려한 네온사인과 유니버설 스튜디오 재팬, 그리고 인근 교토와의 접근성이 뛰어납니다.",
    "keywords": [
      "아시아",
      "미식",
      "유니버설스튜디오",
      "도톤보리",
      "가족여행"
    ]
  },
  {
    "id": 324,
    "slug": "fukuoka",
    "name": "후쿠오카",
    "name_en": "Fukuoka",
    "country": "일본",
    "country_en": "Japan",
    "lat": 33.5902,
    "lng": 130.4017,
    "tier": 2,
    "popularity": 88,
    "continent": "asia",
    "categories": [
      "urban"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "한국에서 가장 가깝고 가볍게 떠날 수 있는 일본의 미식 도시입니다. 돈코츠 라멘의 본고장이자, 나카스 포장마차 거리에서 현지의 밤 문화를 만끽할 수 있습니다.",
    "keywords": [
      "아시아",
      "단거리",
      "미식",
      "온천",
      "효도여행"
    ]
  },
  {
    "id": 325,
    "slug": "qingdao",
    "name": "칭다오",
    "name_en": "Qingdao",
    "country": "중국",
    "country_en": "China",
    "lat": 36.0671,
    "lng": 120.3826,
    "tier": 2,
    "popularity": 70,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "붉은 지붕과 푸른 바다가 어우러져 '동양의 나폴리'라 불립니다. 세계적인 명성의 칭다오 맥주와 함께 유럽풍 건축물이 가득한 구시가지를 산책해보세요.",
    "keywords": [
      "아시아",
      "맥주",
      "청도",
      "유럽풍",
      "가족여행"
    ]
  },
  {
    "id": 326,
    "slug": "dubrovnik",
    "name": "두브로브니크",
    "name_en": "Dubrovnik",
    "country": "크로아티아",
    "country_en": "Croatia",
    "lat": 42.6507,
    "lng": 18.0944,
    "tier": 1,
    "popularity": 86,
    "continent": "europe",
    "categories": [
      "culture",
      "paradise"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": "europe",
    "desc": "아드리아 해의 진주라 불리는 아름다운 성벽 도시입니다. 붉은 지붕들 사이로 펼쳐지는 짙푸른 바다의 풍경은 '왕좌의 게임' 촬영지로도 유명합니다.",
    "keywords": [
      "유럽",
      "아드리아해",
      "성벽투어",
      "왕좌의게임",
      "유럽소도시"
    ]
  },
  {
    "id": 327,
    "slug": "saipan",
    "name": "사이판",
    "name_en": "Saipan",
    "country": "북마리아나 제도",
    "country_en": "Northern Mariana Islands",
    "lat": 15.1909,
    "lng": 145.7467,
    "tier": 2,
    "popularity": 82,
    "continent": "oceania",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "때묻지 않은 자연과 맑은 바다를 간직한 가족 휴양의 성지입니다. 마나가하 섬에서의 스노클링과 별빛 투어는 잊지 못할 추억을 선사합니다.",
    "keywords": [
      "오세아니아",
      "마나가하섬",
      "휴양지",
      "스노클링",
      "가족여행"
    ]
  },
  {
    "id": 328,
    "slug": "phu-quoc",
    "name": "푸꾸옥",
    "name_en": "Phu Quoc",
    "country": "베트남",
    "country_en": "Vietnam",
    "lat": 10.2289,
    "lng": 103.9573,
    "tier": 2,
    "popularity": 84,
    "continent": "asia",
    "categories": [
      "paradise"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "베트남의 몰디브라 불리는 청정 섬입니다. 세계에서 가장 긴 해상 케이블카를 타고 환상적인 일몰을 감상하며 고급 리조트에서 여유를 만끽하세요.",
    "keywords": [
      "아시아",
      "휴양지",
      "일몰",
      "케이블카",
      "신혼여행"
    ]
  },
  {
    "id": 329,
    "slug": "kota-kinabalu",
    "name": "코타키나발루",
    "name_en": "Kota Kinabalu",
    "country": "말레이시아",
    "country_en": "Malaysia",
    "lat": 5.9804,
    "lng": 116.0735,
    "tier": 2,
    "popularity": 81,
    "continent": "asia",
    "categories": [
      "paradise",
      "nature"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": "east-asia",
    "desc": "세계 3대 석양으로 꼽히는 황홀한 일몰과 함께, 반딧불이 투어로 신비로운 대자연을 체험할 수 있는 완벽한 힐링 여행지입니다.",
    "keywords": [
      "아시아",
      "세계3대석양",
      "반딧불이",
      "휴양지",
      "가족여행"
    ]
  },
  {
    "id": 330,
    "slug": "honolulu",
    "name": "호놀룰루",
    "name_en": "Honolulu",
    "country": "하와이",
    "country_en": "Hawaii",
    "lat": 21.3099,
    "lng": -157.8581,
    "tier": 1,
    "popularity": 89,
    "continent": "north_america",
    "categories": [
      "paradise",
      "urban"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "와이키키 해변의 활기와 다이아몬드 헤드의 장관이 어우러진 하와이의 심장입니다. 세계 최고의 휴양과 쇼핑, 대자연의 경이로움을 동시에 누릴 수 있습니다.",
    "keywords": [
      "북미",
      "와이키키",
      "하와이",
      "쇼핑",
      "신혼여행"
    ]
  }
,
  {
    "id": 331,
    "slug": "hokkaido",
    "name": "북해도",
    "name_en": "Hokkaido",
    "country": "일본",
    "country_en": "Japan",
    "lat": 43.2203,
    "lng": 142.8635,
    "tier": 2,
    "popularity": 85,
    "continent": "asia",
    "categories": [
      "nature",
      "adventure"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "사계절 내내 각기 다른 매력을 뽐내는 대자연의 보고입니다. 겨울에는 세계 최고 수준의 파우더 스노우를, 여름에는 끝없이 펼쳐진 라벤더 밭과 시원한 드라이브 코스를 만끽할 수 있습니다.",
    "keywords": [
      "아시아",
      "대자연",
      "설경",
      "라벤더",
      "온천"
    ]
  },
  {
    "id": 332,
    "slug": "sant-joan",
    "name": "Sant Joan",
    "name_en": "Sant Joan de Labritja",
    "country": "스페인",
    "country_en": "Spain",
    "lat": 39.0775,
    "lng": 1.5134,
    "tier": 3,
    "popularity": 60,
    "continent": "europe",
    "categories": [
      "paradise",
      "culture"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": "europe",
    "desc": "이비자 섬 북부의 평화롭고 조용한 마을입니다. 섬 남부의 화려한 파티 문화와는 대조적으로 때묻지 않은 자연 해변과 보헤미안 감성의 로컬 마켓, 그리고 여유로운 휴식을 즐길 수 있습니다.",
    "keywords": [
      "유럽",
      "이비자",
      "히피마켓",
      "조용한휴양지",
      "지중해"
    ]
  },
  {
    "id": 333,
    "slug": "miyakojima",
    "name": "미야코지마",
    "name_en": "Miyakojima",
    "country": "일본",
    "country_en": "Japan",
    "lat": 24.8055,
    "lng": 125.2811,
    "tier": 2,
    "popularity": 72,
    "continent": "asia",
    "categories": [
      "paradise",
      "nature"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": "east-asia",
    "desc": "'미야코 블루'라 불리는 숨막히게 아름다운 청록색 바다를 품은 산호섬입니다. 일본 최고의 해변으로 꼽히는 요나하 마에하마 비치와 스쿠버 다이빙 포인트가 넘쳐납니다.",
    "keywords": [
      "아시아",
      "오키나와",
      "산호초",
      "스쿠버다이빙",
      "미야코블루"
    ]
  },
  {
    "id": 334,
    "slug": "kala-patthar",
    "name": "Kala Patthar",
    "name_en": "Kala Patthar",
    "country": "네팔",
    "country_en": "Nepal",
    "lat": 27.9959,
    "lng": 86.8284,
    "tier": 3,
    "popularity": 55,
    "continent": "asia",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": false,
    "denseRegion": "south-asia",
    "desc": "해발 5,545m에 위치한 세계 최고의 에베레스트 전망대입니다. 세계 최고봉을 가장 가까이서, 그리고 가장 웅장하게 감상할 수 있는 트레커들의 궁극적인 목적지입니다.",
    "keywords": [
      "아시아",
      "히말라야",
      "트레킹",
      "에베레스트전망대",
      "고산지대"
    ]
  },
  {
    "id": 335,
    "slug": "faroe-islands",
    "name": "페로 제도",
    "name_en": "Faroe Islands",
    "country": "덴마크",
    "country_en": "Denmark",
    "lat": 61.8926,
    "lng": -6.9118,
    "tier": 2,
    "popularity": 65,
    "continent": "europe",
    "categories": [
      "nature",
      "adventure"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "북대서양 한가운데 솟아오른 18개의 화산섬으로 이루어진 미지의 영토입니다. 깎아지른 절벽, 초록빛 지붕의 집들, 그리고 퍼핀 새들이 어우러진 동화 같은 풍경을 선사합니다.",
    "keywords": [
      "유럽",
      "북대서양",
      "절벽",
      "트레킹",
      "신비로운자연"
    ]
  },
  {
    "id": 336,
    "slug": "kiribati",
    "name": "키리바시",
    "name_en": "Kiribati",
    "country": "키리바시",
    "country_en": "Kiribati",
    "lat": -1.3333,
    "lng": 173,
    "tier": 3,
    "popularity": 45,
    "continent": "oceania",
    "categories": [
      "paradise",
      "nature"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "태평양 한가운데 적도를 가로지르는 산호초 섬나라입니다. 기후 변화로 인해 사라질 위기에 처해 있어, 원시적인 산호 라군과 고요한 열대의 아름다움을 만날 수 있는 마지막 기회일지도 모릅니다.",
    "keywords": [
      "오세아니아",
      "산호섬",
      "라군",
      "기후변화",
      "미지의섬"
    ]
  },
  {
    "id": 337,
    "slug": "cape-verde",
    "name": "카보베르데",
    "name_en": "Cape Verde",
    "country": "카보베르데",
    "country_en": "Cape Verde",
    "lat": 16.5388,
    "lng": -23.0418,
    "tier": 3,
    "popularity": 50,
    "continent": "africa",
    "categories": [
      "paradise",
      "culture"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "아프리카 대륙 서쪽 대서양에 위치한 군도로, 아프리카와 포르투갈, 브라질의 문화가 혼합된 독특한 매력을 발산합니다. 연중 따뜻한 기후와 화산 풍경, 흥겨운 로컬 음악이 여행자를 반깁니다.",
    "keywords": [
      "아프리카",
      "대서양군도",
      "모르나음악",
      "화산섬",
      "다문화"
    ]
  },
  {
    "id": 338,
    "slug": "bled",
    "name": "블레드",
    "name_en": "Bled",
    "country": "슬로베니아",
    "country_en": "Slovenia",
    "lat": 46.3683,
    "lng": 14.1146,
    "tier": 2,
    "popularity": 78,
    "continent": "europe",
    "categories": [
      "nature",
      "culture"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": "europe",
    "desc": "알프스 산맥의 만년설이 녹아 만들어진 에메랄드빛 블레드 호수가 있는 동화 속 마을입니다. 호수 한가운데 떠 있는 작은 섬과 그 위의 성당, 그리고 절벽 위의 블레드 성이 완벽한 절경을 만들어냅니다.",
    "keywords": [
      "유럽",
      "알프스",
      "블레드호수",
      "소원종",
      "동화마을"
    ]
  },
  {
    "id": 339,
    "slug": "la-spezia",
    "name": "라스페치아",
    "name_en": "La Spezia",
    "country": "이탈리아",
    "country_en": "Italy",
    "lat": 44.1025,
    "lng": 9.8241,
    "tier": 2,
    "popularity": 68,
    "continent": "europe",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "europe",
    "desc": "세계적인 절경을 자랑하는 해안 마을 '친퀘테레'로 향하는 관문 도시입니다. 리구리아 해의 아름다운 항구 풍경과 해산물 요리, 그리고 여유로운 이탈리아의 일상을 엿볼 수 있습니다.",
    "keywords": [
      "유럽",
      "친퀘테레",
      "항구도시",
      "리구리아해",
      "해산물"
    ]
  },
  {
    "id": 340,
    "slug": "kotor",
    "name": "코토르",
    "name_en": "Kotor",
    "country": "몬테네그로",
    "country_en": "Montenegro",
    "lat": 42.4247,
    "lng": 18.7712,
    "tier": 2,
    "popularity": 75,
    "continent": "europe",
    "categories": [
      "culture",
      "nature"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": "europe",
    "desc": "유럽 최남단의 피오르드라 불리는 코토르 만 깊숙한 곳에 자리한 중세 도시입니다. 아드리아 해안의 아름다운 자연과 잘 보존된 베네치아 양식의 성벽 구시가지가 세계문화유산으로 지정되어 있습니다.",
    "keywords": [
      "유럽",
      "피오르드",
      "발칸반도",
      "성벽도시",
      "유네스코"
    ]
  },
  {
    "id": 341,
    "slug": "jeju",
    "name": "제주",
    "name_en": "Jeju",
    "country": "한국",
    "country_en": "South Korea",
    "lat": 33.4996,
    "lng": 126.5312,
    "tier": 1,
    "popularity": 95,
    "continent": "asia",
    "categories": [
      "nature",
      "paradise"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "한국이 자랑하는 세계자연유산의 섬으로 화산이 빚어낸 경이로운 풍경과 에메랄드빛 바다가 어우러진 천혜의 자연을 간직하고 있습니다. 한라산 등반, 만장굴 탐험, 올레길 트레킹, 성산일출봉의 장엄한 일출까지 사계절 내내 특별한 경험을 선사합니다.",
    "keywords": [
      "아시아",
      "국내여행",
      "한라산",
      "올레길",
      "성산일출봉"
    ]
  },
  {
    "id": 342,
    "slug": "seogwipo",
    "name": "서귀포",
    "name_en": "Seogwipo",
    "country": "한국",
    "country_en": "South Korea",
    "lat": 33.2541,
    "lng": 126.5601,
    "tier": 2,
    "popularity": 82,
    "continent": "asia",
    "categories": [
      "nature",
      "paradise"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "제주도 남부의 중심 도시로 천지연폭포, 정방폭포 등 아름다운 폭포와 주상절리대, 섭지코지 등 화산이 만든 독특한 지형을 감상할 수 있습니다. 따뜻한 기후 덕분에 감귤 농장이 가득하며, 올레길의 여러 코스가 이곳을 지나갑니다.",
    "keywords": [
      "아시아",
      "국내여행",
      "폭포",
      "주상절리",
      "감귤"
    ]
  },
  {
    "id": 343,
    "slug": "manila",
    "name": "마닐라",
    "name_en": "Manila",
    "country": "필리핀",
    "country_en": "Philippines",
    "lat": 14.5995,
    "lng": 120.9842,
    "tier": 1,
    "popularity": 85,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "필리핀의 수도이자 정치·경제·문화의 중심지로 스페인 식민지 시대의 흔적이 남아있는 역사적인 도시입니다. 인트라무로스 성곽 도시, 마닐라 대성당, 산티아고 요새 등 문화유산과 함께 현대적인 쇼핑몰과 활기찬 나이트라이프를 즐길 수 있습니다.",
    "keywords": [
      "아시아",
      "동남아",
      "인트라무로스",
      "역사도시",
      "쇼핑"
    ]
  },
  {
    "id": 344,
    "slug": "chiang-mai",
    "name": "치앙마이",
    "name_en": "Chiang Mai",
    "country": "태국",
    "country_en": "Thailand",
    "lat": 18.7883,
    "lng": 98.9853,
    "tier": 1,
    "popularity": 90,
    "continent": "asia",
    "categories": [
      "culture",
      "nature"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "태국 북부의 문화 수도로 300개 이상의 아름다운 불교 사원과 전통 공예가 살아 숨쉬는 고즈넉한 도시입니다. 코끼리 보호 센터 방문, 정글 트레킹, 이산족 마을 체험 등 다양한 액티비티와 함께 매력적인 나이트 바자와 요리 교실로 유명합니다.",
    "keywords": [
      "아시아",
      "동남아",
      "사원",
      "코끼리",
      "전통문화"
    ]
  },
  {
    "id": 345,
    "slug": "macau",
    "name": "마카오",
    "name_en": "Macau",
    "country": "중국",
    "country_en": "China",
    "lat": 22.1987,
    "lng": 113.5439,
    "tier": 1,
    "popularity": 88,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "동양의 라스베가스로 불리는 마카오는 포르투갈 식민지 시대의 유산과 현대적인 카지노 리조트가 공존하는 독특한 매력의 도시입니다. 세계문화유산으로 지정된 역사 지구를 산책하며 에그타르트와 포르투갈-중국 퓨전 요리를 즐겨보세요.",
    "keywords": [
      "아시아",
      "카지노",
      "세계유산",
      "에그타르트",
      "포르투갈문화"
    ]
  },
  {
    "id": 346,
    "slug": "budapest",
    "name": "부다페스트",
    "name_en": "Budapest",
    "country": "헝가리",
    "country_en": "Hungary",
    "lat": 47.4979,
    "lng": 19.0402,
    "tier": 1,
    "popularity": 92,
    "continent": "europe",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "europe",
    "desc": "다뉴브 강을 사이에 두고 부다와 페스트로 나뉜 '다뉴브의 진주' 부다페스트는 웅장한 국회의사당과 세체니 다리의 야경, 그리고 100년 넘은 온천 문화로 유명합니다. 어부의 요새에서 바라보는 도시 전경과 루인 바의 독특한 분위기는 잊을 수 없는 추억을 선사합니다.",
    "keywords": [
      "유럽",
      "동유럽",
      "다뉴브강",
      "온천",
      "야경"
    ]
  },
  {
    "id": 347,
    "slug": "helsinki",
    "name": "헬싱키",
    "name_en": "Helsinki",
    "country": "핀란드",
    "country_en": "Finland",
    "lat": 60.1699,
    "lng": 24.9384,
    "tier": 1,
    "popularity": 80,
    "continent": "europe",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "발트해 연안에 자리한 북유럽 디자인의 수도 헬싱키는 깔끔하고 세련된 건축물과 아늑한 카페 문화로 유명합니다. 템펠리아우키오 암석 교회, 수오멘린나 해상 요새 등 독특한 명소와 함께 사우나 문화를 체험하고 현지인들의 여유로운 라이프스타일을 느껴보세요.",
    "keywords": [
      "유럽",
      "북유럽",
      "디자인",
      "사우나",
      "발트해"
    ]
  },
  {
    "id": 348,
    "slug": "reykjavik",
    "name": "레이캬비크",
    "name_en": "Reykjavik",
    "country": "아이슬란드",
    "country_en": "Iceland",
    "lat": 64.1466,
    "lng": -21.9426,
    "tier": 1,
    "popularity": 85,
    "continent": "europe",
    "categories": [
      "nature",
      "adventure"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계 최북단의 수도 레이캬비크는 오로라, 빙하, 온천, 화산 등 극적인 자연 경관으로 향하는 관문입니다. 컬러풀한 북유럽 건축물이 늘어선 구시가지를 거닐고, 블루라군 온천에서 휴식을 취한 뒤, 골든 서클 투어로 아이슬란드의 경이로운 자연을 만나보세요.",
    "keywords": [
      "유럽",
      "북유럽",
      "오로라",
      "온천",
      "빙하"
    ]
  },
  {
    "id": 349,
    "slug": "brisbane",
    "name": "브리즈번",
    "name_en": "Brisbane",
    "country": "호주",
    "country_en": "Australia",
    "lat": -27.4698,
    "lng": 153.0251,
    "tier": 1,
    "popularity": 78,
    "continent": "oceania",
    "categories": [
      "urban",
      "nature"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "호주 제3의 도시이자 퀸즐랜드 주의 주도인 브리즈번은 연중 따뜻한 기후와 여유로운 분위기를 자랑합니다. 브리즈번 강변의 사우스뱅크 파크랜드에서 휴식을 취하고, 론 파인 코알라 보호구역에서 코알라를 안아보며, 골드코스트와 선샤인 코스트로 향하는 관문 역할을 합니다.",
    "keywords": [
      "오세아니아",
      "호주",
      "코알라",
      "강변",
      "따뜻한기후"
    ]
  },
  {
    "id": 350,
    "slug": "nara",
    "name": "나라",
    "name_en": "Nara",
    "country": "일본",
    "country_en": "Japan",
    "lat": 34.6851,
    "lng": 135.8048,
    "tier": 2,
    "popularity": 82,
    "continent": "asia",
    "categories": [
      "culture",
      "nature"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "asia",
    "desc": "일본 고대 문화의 중심지로 1,000마리가 넘는 사슴이 자유롭게 노니는 나라 공원과 세계 최대의 목조 건축물인 도다이지 대불전이 유명합니다. 8세기 나라 시대의 문화재가 잘 보존되어 있으며, 사슴과 교감하며 고즈넉한 일본 전통 문화를 체험할 수 있습니다.",
    "keywords": [
      "아시아",
      "일본",
      "사슴",
      "세계유산",
      "대불"
    ]
  },
  {
    "id": 351,
    "slug": "kobe",
    "name": "고베",
    "name_en": "Kobe",
    "country": "일본",
    "country_en": "Japan",
    "lat": 34.6901,
    "lng": 135.1955,
    "tier": 2,
    "popularity": 80,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "asia",
    "desc": "세계적으로 유명한 고베규의 본고장이자 일본 3대 야경을 자랑하는 항구 도시입니다. 로코산에서 바라보는 반짝이는 야경, 이국적인 분위기의 기타노 이진관 거리, 난킨마치 차이나타운의 활기찬 먹거리까지 다채로운 매력을 간직하고 있습니다.",
    "keywords": [
      "아시아",
      "일본",
      "고베규",
      "야경",
      "항구도시"
    ]
  },
  {
    "id": 352,
    "slug": "nagasaki",
    "name": "나가사키",
    "name_en": "Nagasaki",
    "country": "일본",
    "country_en": "Japan",
    "lat": 32.7503,
    "lng": 129.8779,
    "tier": 2,
    "popularity": 78,
    "continent": "asia",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "asia",
    "desc": "일본의 서쪽 끝 규슈에 위치한 역사와 평화의 도시입니다. 평화공원과 원폭 자료관에서 평화의 메시지를 되새기고, 네덜란드 풍의 하우스텐보스 테마파크와 이나사야마 전망대에서 일본 3대 야경을 감상할 수 있습니다. 짬뽕과 카스테라 등 독특한 나가사키 음식도 놓치지 마세요.",
    "keywords": [
      "아시아",
      "일본",
      "평화공원",
      "야경",
      "하우스텐보스"
    ]
  },
  {
    "id": 353,
    "slug": "kanazawa",
    "name": "가나자와",
    "name_en": "Kanazawa",
    "country": "일본",
    "country_en": "Japan",
    "lat": 36.5619,
    "lng": 136.6562,
    "tier": 2,
    "popularity": 75,
    "continent": "asia",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "asia",
    "desc": "전통과 예술이 살아 숨쉬는 작은 교토로 불리는 도시입니다. 일본 3대 정원 중 하나인 겐로쿠엔, 전통 게이샤 거리인 히가시차야가이, 금박 공예로 유명한 가나자와는 에도 시대의 정취를 고스란히 간직하고 있습니다. 신선한 해산물이 가득한 오미초 시장도 필수 코스입니다.",
    "keywords": [
      "아시아",
      "일본",
      "겐로쿠엔",
      "금박",
      "전통문화"
    ]
  },
  {
    "id": 354,
    "slug": "yokohama",
    "name": "요코하마",
    "name_en": "Yokohama",
    "country": "일본",
    "country_en": "Japan",
    "lat": 35.4437,
    "lng": 139.638,
    "tier": 2,
    "popularity": 77,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "asia",
    "desc": "도쿄에서 30분 거리에 위치한 일본 제2의 도시이자 최대 항구 도시입니다. 미나토미라이 21의 미래적인 스카이라인과 역사 깊은 야마시타 공원, 일본 최대 차이나타운의 활기찬 분위기가 어우러져 있습니다. 컵라면 박물관과 산케이엔 정원도 인기 명소입니다.",
    "keywords": [
      "아시아",
      "일본",
      "미나토미라이",
      "차이나타운",
      "항구도시"
    ]
  },
  {
    "id": 355,
    "slug": "tsushima",
    "name": "대마도",
    "name_en": "Tsushima (Daemado)",
    "country": "일본",
    "country_en": "Japan",
    "lat": 34.2048,
    "lng": 129.2887,
    "tier": 2,
    "popularity": 72,
    "continent": "asia",
    "categories": [
      "nature",
      "culture"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "일본 対馬島(쓰시마)로, 한국에서는 대마도라 부릅니다. 부산국제여객터미널에서 히타카츠항까지 쾌속페리(약 1시간)가 주 경로이며, 인천 직항 항공은 없습니다. 맑은 바다·숲길 트레킹·면세 쇼핑·온천을 즐기기 좋습니다.",
    "keywords": [
      "아시아",
      "일본",
      "쓰시마",
      "쓰시마섬",
      "국경의섬",
      "부산페리",
      "히타카츠",
      "면세쇼핑"
    ]
  },
  {
    "id": 356,
    "slug": "kumamoto",
    "name": "구마모토",
    "name_en": "Kumamoto",
    "country": "일본",
    "country_en": "Japan",
    "lat": 32.8031,
    "lng": 130.7079,
    "tier": 2,
    "popularity": 70,
    "continent": "asia",
    "categories": [
      "culture",
      "nature"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "asia",
    "desc": "규슈 중앙부에 위치한 역사와 자연이 어우러진 도시입니다. 일본 3대 성 중 하나인 구마모토성과 세계 최대급 칼데라 화산인 아소산, 검은색 온천수가 특징인 구로카와 온천이 유명합니다. 마스코트 캐릭터 구마몬의 고향으로도 잘 알려져 있습니다.",
    "keywords": [
      "아시아",
      "일본",
      "구마모토성",
      "아소산",
      "구마몬"
    ]
  },
  {
    "id": 357,
    "slug": "ishigaki",
    "name": "이시가키",
    "name_en": "Ishigaki",
    "country": "일본",
    "country_en": "Japan",
    "lat": 24.3364,
    "lng": 124.1559,
    "tier": 3,
    "popularity": 68,
    "continent": "asia",
    "categories": [
      "paradise",
      "nature"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "오키나와 현 남서부 야에야마 제도의 중심 섬으로 에메랄드빛 바다와 산호초로 둘러싸인 열대 낙원입니다. 가비라 만의 투명한 바다에서 스노클링과 다이빙을 즐기고, 별이 쏟아지는 남십자성 관측으로 유명합니다. 인근의 다케토미 섬과 이리오모테 섬으로의 당일 여행도 가능합니다.",
    "keywords": [
      "아시아",
      "일본",
      "오키나와",
      "스노클링",
      "산호초"
    ]
  },
  {
    "id": 358,
    "slug": "lombok",
    "name": "롬복",
    "name_en": "Lombok",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -8.65,
    "lng": 116.3242,
    "tier": 2,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "paradise",
      "adventure"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "발리 동쪽에 위치한 순수하고 평화로운 열대 섬으로 과도한 개발을 피해 조용한 휴양을 원하는 여행자들에게 인기입니다. 3,726m 높이의 린자니 화산 트레킹, 핑크 비치에서의 스노클링, 길리 3섬의 맑은 바다와 파티 분위기까지 다채로운 즐거움을 선사합니다.",
    "keywords": [
      "아시아",
      "동남아",
      "린자니화산",
      "길리섬",
      "서핑"
    ]
  },
  {
    "id": 359,
    "slug": "vientiane",
    "name": "비엔티안",
    "name_en": "Vientiane",
    "country": "라오스",
    "country_en": "Laos",
    "lat": 17.9757,
    "lng": 102.6331,
    "tier": 2,
    "popularity": 73,
    "continent": "asia",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "메콩강변에 자리한 라오스의 수도로 동남아에서 가장 여유롭고 조용한 수도 중 하나입니다. 황금빛 파툭사이 개선문, 타트루앙 불탑 등 불교 사원과 프랑스 식민지 시대의 건축물이 공존하며, 메콩강 석양을 바라보며 비어라오 맥주를 즐기는 여유로운 시간이 매력적입니다.",
    "keywords": [
      "아시아",
      "동남아",
      "메콩강",
      "불교사원",
      "여유"
    ]
  },
  {
    "id": 360,
    "slug": "vang-vieng",
    "name": "방비엥",
    "name_en": "Vang Vieng",
    "country": "라오스",
    "country_en": "Laos",
    "lat": 18.9333,
    "lng": 102.4481,
    "tier": 3,
    "popularity": 70,
    "continent": "asia",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": false,
    "denseRegion": null,
    "desc": "석회암 카르스트 지형이 빚어낸 장엄한 풍경 속에 자리한 배낭여행자들의 천국입니다. 송강에서 튜빙을 즐기고, 블루 라군과 탐 창 동굴을 탐험하며, 열기구를 타고 하늘에서 바라보는 일몰 풍경은 잊을 수 없는 추억을 선사합니다. 최근에는 편안한 리조트도 많이 생겨났습니다.",
    "keywords": [
      "아시아",
      "동남아",
      "튜빙",
      "동굴",
      "배낭여행"
    ]
  },
  {
    "id": 361,
    "slug": "zagreb",
    "name": "자그레브",
    "name_en": "Zagreb",
    "country": "크로아티아",
    "country_en": "Croatia",
    "lat": 45.815,
    "lng": 15.9819,
    "tier": 2,
    "popularity": 74,
    "continent": "europe",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": false,
    "denseRegion": "europe",
    "desc": "크로아티아의 수도이자 문화의 중심지로 오스트리아-헝가리 제국 시대의 우아한 건축물과 활기찬 카페 문화가 어우러진 매력적인 도시입니다. 고르니그라드 구시가지의 돌로츠 시장, 성 마르코 교회의 화려한 타일 지붕, 그리고 독특한 실연 박물관이 인기 명소입니다.",
    "keywords": [
      "유럽",
      "동유럽",
      "구시가지",
      "카페문화",
      "발칸"
    ]
  },
  {
    "id": 362,
    "slug": "warsaw",
    "name": "바르샤바",
    "name_en": "Warsaw",
    "country": "폴란드",
    "country_en": "Poland",
    "lat": 52.2297,
    "lng": 21.0122,
    "tier": 1,
    "popularity": 77,
    "continent": "europe",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": false,
    "denseRegion": "europe",
    "desc": "폴란드의 수도로 제2차 세계대전 당시 85%가 파괴되었지만 시민들의 노력으로 완벽하게 재건된 불사조의 도시입니다. 세계문화유산으로 지정된 구시가지 광장, 쇼팽의 유산이 살아 숨쉬는 와지엔키 공원, 그리고 바르샤바 봉기 박물관에서 역사의 교훈을 되새길 수 있습니다.",
    "keywords": [
      "유럽",
      "동유럽",
      "구시가지",
      "쇼팽",
      "역사"
    ]
  },
  {
    "id": 363,
    "slug": "philadelphia",
    "name": "필라델피아",
    "name_en": "Philadelphia",
    "country": "미국",
    "country_en": "United States",
    "lat": 39.9526,
    "lng": -75.1652,
    "tier": 1,
    "popularity": 75,
    "continent": "north_america",
    "categories": [
      "culture",
      "urban"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "미국 독립의 요람이자 역사적으로 가장 중요한 도시 중 하나입니다. 자유의 종과 독립기념관에서 미국 건국의 역사를 체험하고, 필라델피아 미술관의 계단을 뛰어 올라 록키의 기분을 느껴보세요. 유명한 필리 치즈스테이크와 다양한 문화 명소도 매력적입니다.",
    "keywords": [
      "북미",
      "미국",
      "독립기념관",
      "자유의종",
      "역사도시"
    ]
  },
  {
    "id": 364,
    "slug": "san-diego",
    "name": "샌디에이고",
    "name_en": "San Diego",
    "country": "미국",
    "country_en": "United States",
    "lat": 32.7157,
    "lng": -117.1611,
    "tier": 1,
    "popularity": 80,
    "continent": "north_america",
    "categories": [
      "paradise",
      "urban"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "캘리포니아 남부의 햇살 가득한 해변 도시로 연중 완벽한 날씨와 70마일에 달하는 아름다운 해안선을 자랑합니다. 세계적인 샌디에이고 동물원, 발보아 공원의 박물관들, 역사적인 개스램프 쿼터와 라호야 코브의 물개들, 그리고 맥주 양조장 투어까지 다채로운 즐길거리가 가득합니다.",
    "keywords": [
      "북미",
      "미국",
      "해변",
      "동물원",
      "서핑"
    ]
  },
  {
    "id": 365,
    "slug": "gold-coast",
    "name": "골드코스트",
    "name_en": "Gold Coast",
    "country": "호주",
    "country_en": "Australia",
    "lat": -28.0167,
    "lng": 153.4,
    "tier": 2,
    "popularity": 82,
    "continent": "oceania",
    "categories": [
      "paradise",
      "adventure"
    ],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "호주 동부 해안의 대표적인 휴양 도시로 57km에 달하는 황금빛 해변과 세계적인 서핑 포인트로 유명합니다. 서퍼스 파라다이스의 활기찬 분위기, 드림월드와 무비월드 같은 테마파크, 그리고 내륙의 열대우림 국립공원까지 가족 여행객과 서퍼들에게 완벽한 목적지입니다.",
    "keywords": [
      "오세아니아",
      "호주",
      "서핑",
      "해변",
      "테마파크"
    ]
  },
  {
    "id": 367,
    "slug": "vladivostok",
    "name": "블라디보스토크",
    "name_en": "Vladivostok",
    "country": "러시아",
    "country_en": "Russia",
    "lat": 43.1198,
    "lng": 131.8869,
    "tier": 2,
    "popularity": 76,
    "continent": "asia",
    "categories": [
      "urban",
      "culture"
    ],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "가장 가까운 유럽이라 불리는 극동 러시아의 항구입니다. 시베리아 횡단 열차의 장대한 출발점이자 종착역으로, 골든혼만과 러스키 다리 야경·해산물 시장이 유명합니다.",
    "keywords": [
      "아시아",
      "러시아",
      "극동",
      "시베리아횡단열차",
      "항구"
    ]
  },
  {
    "id": 368,
    "slug": "irkutsk",
    "name": "이르쿠츠크",
    "name_en": "Irkutsk",
    "country": "러시아",
    "country_en": "Russia",
    "lat": 52.287,
    "lng": 104.305,
    "tier": 2,
    "popularity": 74,
    "continent": "asia",
    "categories": [
      "nature",
      "culture"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "시베리아의 파리라 불리는 목재 건축 도시입니다. 세계에서 가장 깊은 민물 호수인 바이칼 호수로 향하는 관문으로, 리스트비양카·올호노섬 일정의 출발점입니다.",
    "keywords": [
      "아시아",
      "러시아",
      "바이칼",
      "시베리아",
      "목조건축"
    ]
  },
  {
    "id": 369,
    "slug": "alice-springs",
    "name": "앨리스스프링스",
    "name_en": "Alice Springs",
    "country": "호주",
    "country_en": "Australia",
    "lat": -23.698,
    "lng": 133.8807,
    "tier": 2,
    "popularity": 72,
    "continent": "oceania",
    "categories": [
      "adventure",
      "nature"
    ],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "호주 대륙 정중앙 아웃백의 거점 도시입니다. 울루루·킹스캐니언·맥도널드 산맥 일정의 출발점으로, 사막 일출·별 관측·원주민 문화 체험이 인기입니다.",
    "keywords": [
      "오세아니아",
      "호주",
      "아웃백",
      "울루루",
      "레드센터"
    ]
  },
  {
    "id": 370,
    "slug": "brunei",
    "name": "브루나이",
    "name_en": "Brunei",
    "country": "브루나이",
    "country_en": "Brunei",
    "lat": 4.9031,
    "lng": 114.9398,
    "tier": 2,
    "popularity": 70,
    "continent": "asia",
    "categories": [
      "culture",
      "nature"
    ],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "보르네오섬 북부에 있는 독립 왕국으로, 황금 돔 모스크·수상 마을·열대우림이 어우러진 소규모 목적지입니다. 말레이시아 사바(보르네오)와 입국·관문(BWN)이 다르며, 반다르스리브가완을 거점으로 프로보시스 원숭이·왕실 박물관·정글 투어를 즐기기 좋습니다.",
    "keywords": [
      "아시아",
      "브루나이",
      "보르네오섬",
      "BWN",
      "술탄국"
    ]
  },
  {
    "id": 371,
    "slug": "el-calafate",
    "name": "엘칼라파테",
    "name_en": "El Calafate",
    "country": "아르헨티나",
    "country_en": "Argentina",
    "lat": -50.3378,
    "lng": -72.2648,
    "tier": 2,
    "popularity": 78,
    "continent": "south_america",
    "categories": [
      "nature",
      "adventure"
    ],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남부 아르헨티나 파타고니아의 빙하 관광 거점입니다. 페리토 모레노 빙하·로스글라시아레스 국립공원 관문(FTE)이며, 북부 파타고니아(바릴로체)·우수아이아·토레스 델 파이네와는 별도 여행지입니다.",
    "keywords": [
      "파타고니아",
      "페리토모레노",
      "빙하",
      "FTE",
      "자연/오지"
    ]
  },
  {
    "id": 372,
    "slug": "hamburg",
    "name": "함부르크",
    "name_en": "Hamburg",
    "country": "독일",
    "country_en": "Germany",
    "lat": 53.5511,
    "lng": 9.9937,
    "tier": 2,
    "popularity": 76,
    "continent": "europe",
    "categories": ["urban", "culture"],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "북독일 최대 항구 도시로, 엘베 강변 창고지·미니어트 우란·레커버거 거리가 어우러진 북유럽 문화 거점입니다. 함부르크(HAM) 직항·인근 프랑크푸르트(FRA) 경유 일정이 흔합니다.",
    "keywords": ["독일", "함부르크", "HAM", "항구", "도심"]
  },
  {
    "id": 373,
    "slug": "cocos-islands",
    "name": "코코스 제도",
    "name_en": "Cocos (Keeling) Islands",
    "country": "호주",
    "country_en": "Australia",
    "lat": -12.1642,
    "lng": 96.873,
    "tier": 2,
    "popularity": 62,
    "continent": "oceania",
    "categories": ["nature"],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "인도양 남부에 있는 호주령 산호섬으로, 투명한 라군과 조용한 해변이 특징인 오지 목적지입니다. 퍼스(PER) 등 호주 본토에서 정기편·경유로 이어지는 일정이 일반적입니다.",
    "keywords": ["코코스", "호주령", "산호섬", "PER", "자연/오지"]
  },
  {
    "id": 374,
    "slug": "pitcairn-islands",
    "name": "핏케언 제도",
    "name_en": "Pitcairn Islands",
    "country": "핏케언 제도",
    "country_en": "Pitcairn Islands",
    "lat": -25.0667,
    "lng": -130.1,
    "tier": 3,
    "popularity": 48,
    "continent": "oceania",
    "categories": ["nature", "culture"],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "바운티호 반란 선원 후손이 정착한 세계에서 가장 고립된 영토 중 하나입니다. 상용 노선은 타히티(PPT) 등 남태평양 관문 경유·페리·전용선으로 이어지는 일정이 일반적입니다.",
    "keywords": ["핏케언", "PPT", "바운티", "자연/오지"]
  },
  {
    "id": 375,
    "slug": "greenland",
    "name": "그린란드",
    "name_en": "Greenland",
    "country": "그린란드",
    "country_en": "Greenland",
    "lat": 71.7069,
    "lng": -42.6043,
    "tier": 2,
    "popularity": 72,
    "continent": "europe",
    "categories": ["nature", "adventure"],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계 최대 섬으로 빙상·피오르드·북극광이 어우러진 극지 여행지입니다. 코펜하겐(CPH) 등 덴마크·유럽 관문 경유 후 누크(GOH)·일룰리사트 등으로 이어지는 일정이 흔합니다.",
    "keywords": ["그린란드", "빙하", "CPH", "북극", "자연/오지"]
  },
  {
    "id": 376,
    "slug": "falkland-islands",
    "name": "포클랜드 제도",
    "name_en": "Falkland Islands",
    "country": "영국",
    "country_en": "United Kingdom",
    "lat": -51.6977,
    "lng": -57.8516,
    "tier": 2,
    "popularity": 58,
    "continent": "south_america",
    "categories": ["nature"],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남대서양의 영국령 군도로 펭귄·알바트로스 서식지와 스탠리 항구 마을이 유명합니다. 칠레·아르헨티나(SCL) 경유 후 MPM(마운트 플레전트) 도착 일정이 일반적입니다.",
    "keywords": ["포클랜드", "펭귄", "MPM", "SCL", "자연/오지"]
  },
  {
    "id": 377,
    "slug": "solomon-islands",
    "name": "솔로몬 제도",
    "name_en": "Solomon Islands",
    "country": "솔로몬 제도",
    "country_en": "Solomon Islands",
    "lat": -9.6457,
    "lng": 160.1562,
    "tier": 2,
    "popularity": 64,
    "continent": "oceania",
    "categories": ["nature", "adventure"],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "난파선 다이빙의 성지이자 미지의 멜라네시아 군도입니다. 호니아라(HIR) 직항 또는 브리즈번(BNE)·시드니 등 호주·태평양 관문 경유 일정이 흔합니다.",
    "keywords": ["솔로몬", "HIR", "BNE", "다이빙", "자연/오지"]
  },
  {
    "id": 378,
    "slug": "nauru",
    "name": "나우루",
    "name_en": "Nauru",
    "country": "나우루",
    "country_en": "Nauru",
    "lat": -0.5228,
    "lng": 166.9315,
    "tier": 3,
    "popularity": 45,
    "continent": "oceania",
    "categories": ["nature"],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "세계에서 가장 작은 섬나라 중 하나로, 인구·관광 인프라가 매우 제한적입니다. 나우루(INU) 직항 또는 브리즈번(BNE) 등 호주 관문 경유 일정이 일반적입니다.",
    "keywords": ["나우루", "INU", "BNE", "오지"]
  },
  {
    "id": 379,
    "slug": "queenstown",
    "name": "퀸스타운",
    "name_en": "Queenstown",
    "country": "뉴질랜드",
    "country_en": "New Zealand",
    "lat": -45.0312,
    "lng": 168.6626,
    "tier": 2,
    "popularity": 82,
    "continent": "oceania",
    "categories": ["nature", "adventure"],
    "primaryCategory": "adventure",
    "category": "adventure",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "와카티푸 호수와 만년설에 둘러싸인 뉴질랜드 남섬의 액티비티 허브입니다. 번지점프·스카이다이빙·밀포드 사운드 투어로 유명하며 관문은 퀸즈타운(ZQN)입니다.",
    "keywords": ["뉴질랜드", "퀸스타운", "ZQN", "액티비티", "밀포드"]
  },
  {
    "id": 380,
    "slug": "minneapolis",
    "name": "미니애폴리스",
    "name_en": "Minneapolis",
    "country": "미국",
    "country_en": "United States",
    "lat": 44.9778,
    "lng": -93.265,
    "tier": 2,
    "popularity": 70,
    "continent": "north_america",
    "categories": ["urban", "culture"],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "미네소타주의 트윈시티(미니애폴리스·세인트폴) 중심으로, 미술관·호수·몰 오브 아메리카가 있는 북미 중서부 도시입니다. MSP 국제공항이 관문입니다.",
    "keywords": ["미국", "미니애폴리스", "MSP", "트윈시티"]
  },
  {
    "id": 381,
    "slug": "perth",
    "name": "퍼스",
    "name_en": "Perth",
    "country": "호주",
    "country_en": "Australia",
    "lat": -31.9505,
    "lng": 115.8605,
    "tier": 2,
    "popularity": 74,
    "continent": "oceania",
    "categories": ["urban", "nature"],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "서호주의 고립된 대도시로, 쾌청한 해변과 로트네스트 섬(쿼카) 당일치기로 유명합니다. 코코스 제도·서호주 오지로 이어지는 관문이 PER입니다.",
    "keywords": ["퍼스", "PER", "서호주", "쿼카", "로트네스트"]
  },
  {
    "id": 382,
    "slug": "bahamas",
    "name": "바하마",
    "name_en": "Bahamas",
    "country": "바하마",
    "country_en": "Bahamas",
    "lat": 25.0343,
    "lng": -77.3963,
    "tier": 2,
    "popularity": 76,
    "continent": "north_america",
    "categories": ["paradise", "culture"],
    "primaryCategory": "paradise",
    "category": "paradise",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "카리브해의 군도 국가로 나소(NAS)·프리포트 등 섬별 관문이 나뉩니다. 마이애미(MIA)·애틀랜타(ATL) 등 미국 동부 경유 일정이 흔합니다.",
    "keywords": ["바하마", "NAS", "카리브", "휴양"]
  },
  {
    "id": 383,
    "slug": "sri-jayawardenapura",
    "name": "스리랑카 수도권",
    "name_en": "Sri Jayawardenepura Kotte",
    "country": "스리랑카",
    "country_en": "Sri Lanka",
    "lat": 6.9105,
    "lng": 79.8876,
    "tier": 2,
    "popularity": 68,
    "continent": "asia",
    "categories": ["culture", "urban"],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "행정 수도 코놈보와 상업·관광 거점 콜롬보가 인접한 수도권입니다. 국제선은 콜롬보 반다라나이크(CMB) 공항이 관문이며, 스리랑카 전역(시기리야·갈·Kandy)으로 이어지기 좋습니다.",
    "keywords": ["스리랑카", "CMB", "콜롬보", "코놈보"]
  },
  {
    "id": 384,
    "slug": "venezuela",
    "name": "베네수엘라",
    "name_en": "Venezuela",
    "country": "베네수엘라",
    "country_en": "Venezuela",
    "lat": 10.4806,
    "lng": -66.9036,
    "tier": 2,
    "popularity": 55,
    "continent": "south_america",
    "categories": ["nature", "culture"],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "남미 북부의 국가 단위 목적지로, 엔젤 폭포·로파 파라국립공원·카라카스 등이 대표입니다. 국제선은 카라카스(CCS) 관문이 일반적이며, 여행 안전·비자는 출발 전 외교부 안내를 확인해 주세요.",
    "keywords": ["베네수엘라", "CCS", "엔젤폭포", "남미"]
  },
  {
    "id": 385,
    "slug": "havana",
    "name": "아바나",
    "name_en": "Havana",
    "country": "쿠바",
    "country_en": "Cuba",
    "lat": 23.1136,
    "lng": -82.3666,
    "tier": 2,
    "popularity": 76,
    "continent": "north_america",
    "categories": ["urban", "culture"],
    "primaryCategory": "urban",
    "category": "urban",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "시간이 멈춘 듯한 올드카가 시가를 달리는 쿠바의 낭만 도시입니다. 말레콘 해변의 라틴 음악과 콜로니얼 거리가 매력적이며, 국제선은 호세 마르티(HAV) 공항이 관문입니다.",
    "keywords": ["쿠바", "아바나", "HAV", "말레콘", "낭만"]
  },
  {
    "id": 386,
    "slug": "malta",
    "name": "몰타",
    "name_en": "Malta",
    "country": "몰타",
    "country_en": "Malta",
    "lat": 35.8989,
    "lng": 14.5146,
    "tier": 2,
    "popularity": 74,
    "continent": "europe",
    "categories": ["culture", "urban"],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "지중해의 작은 섬나라로 발레타(Valletta) 구시가지·청사·요새가 유네스코 유산입니다. 국제선은 몰타(MLA) 공항이 관문이며, 발레타·슬리에마·고조 섬 투어가 흔합니다.",
    "keywords": ["몰타", "발레타", "MLA", "지중해", "유네스코"]
  },
  {
    "id": 387,
    "slug": "yap",
    "name": "야프",
    "name_en": "Yap",
    "country": "미크로네시아 연방",
    "country_en": "Federated States of Micronesia",
    "lat": 9.4989,
    "lng": 138.0827,
    "tier": 3,
    "popularity": 42,
    "continent": "oceania",
    "categories": ["culture", "nature"],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "미크로네시아 연방 서부의 섬으로, 세계에서 마지막까지 남은 돌화폐(라이) 문화와 전통 댄스로 유명합니다. 국제선은 야프(YAP) 공항이 관문입니다.",
    "keywords": ["미크로네시아", "YAP", "Ruul", "루울", "돌화폐", "오세아니아"]
  },
  {
    "id": 388,
    "slug": "chuuk",
    "name": "추크 라군",
    "name_en": "Chuuk Lagoon",
    "country": "미크로네시아 연방",
    "country_en": "Federated States of Micronesia",
    "lat": 7.4617,
    "lng": 151.843,
    "tier": 3,
    "popularity": 44,
    "continent": "oceania",
    "categories": ["nature", "adventure"],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "태평양의 거대한 산호초 라군으로, 2차 대전 이후 남겨진 수중 전함 잔해로 스쿠버 다이빙 성지로 불립니다. 국제선은 추크(TKK) 공항이 관문입니다.",
    "keywords": ["미크로네시아", "추크 라군", "Chuuk Lagoon", "TKK", "Truk", "트럭", "추크", "난파선", "다이빙"]
  },
  {
    "id": 389,
    "slug": "kosrae",
    "name": "코스라에",
    "name_en": "Kosrae",
    "country": "미크로네시아 연방",
    "country_en": "Federated States of Micronesia",
    "lat": 5.3569,
    "lng": 162.9583,
    "tier": 3,
    "popularity": 40,
    "continent": "oceania",
    "categories": ["nature", "culture"],
    "primaryCategory": "nature",
    "category": "nature",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "미크로네시아 연방 동부의 고립된 화산섬으로, 울창한 열대우림과 전통 마을이 보존된 청정 섬입니다. 국제선은 코스라에(KOS) 공항이 관문입니다.",
    "keywords": ["미크로네시아", "KOS", "Utwe", "우트웨", "오세아니아"]
  },
  {
    "id": 390,
    "slug": "pohnpei",
    "name": "폰페이",
    "name_en": "Pohnpei",
    "country": "미크로네시아 연방",
    "country_en": "Federated States of Micronesia",
    "lat": 6.9851,
    "lng": 158.2089,
    "tier": 3,
    "popularity": 46,
    "continent": "oceania",
    "categories": ["culture", "nature"],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": null,
    "desc": "미크로네시아 연방의 수도 섬으로, Nan Madol 유적과 맑은 폭포·산호초가 어우러진 문화·자연 목적지입니다. 국제선은 폰페이(PNI) 공항이 관문입니다.",
    "keywords": ["미크로네시아", "PNI", "Nan Madol", "나난마돌", "오세아니아"]
  },
  {
    "id": 391,
    "slug": "vatican",
    "name": "바티칸",
    "name_en": "Vatican City",
    "country": "바티칸",
    "country_en": "Vatican City",
    "lat": 41.9029,
    "lng": 12.4534,
    "tier": 2,
    "popularity": 78,
    "continent": "europe",
    "categories": ["culture"],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": "southern-europe",
    "desc": "세계 최소 독립국이자 가톨릭의 중심지로, 바티칸 박물관·시스티나 성당·성 베드로 대성당이 집결된 문화·예술 성지입니다. 국제선은 로마 피우미치노(FCO) 공항이 관문이며, 바티칸 시국은 로마 시내와 인접합니다.",
    "keywords": ["바티칸", "Vatican", "Vatican City", "시스티나", "FCO"]
  },
  {
    "id": 392,
    "slug": "ubud",
    "name": "우붓",
    "name_en": "Ubud",
    "country": "인도네시아",
    "country_en": "Indonesia",
    "lat": -8.5069,
    "lng": 115.2625,
    "tier": 2,
    "popularity": 74,
    "continent": "asia",
    "categories": ["culture", "nature"],
    "primaryCategory": "culture",
    "category": "culture",
    "showOnGlobe": true,
    "denseRegion": "southeast-asia",
    "desc": "발리 중부 고원의 예술·요가·전통 마을로, 테갈랄랑 라이스 테라스·우붓 왕궁·원숭이 숲이 어우러진 문화 허브입니다. 국제선은 응우라라이(DPS) 공항이 관문이며, 공항에서 차로 약 1~1.5시간 거리입니다.",
    "keywords": ["우붓", "Ubud", "DPS", "테갈랄랑", "요가"]
  }
];
