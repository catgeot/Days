...
export const TOURAPI_FACILITY_TITLE_RE =
  /화장실|공중변소|소변기|휠체어|점자블록|점자\s*블록|촉지도|무장애|장애인\s*(화장실|주차|엘리베이터|출입)|수유실|기저귀|핸드레일/;
...
function galleryPhotoCaption(img) {
  ...
  return String(img.alt_description || img.tourApi?.title || img.alt || '').trim();
}
...
export function isSparseTourApiGallery(images) {
  ...
  const keepers = list.filter((img) => !isTourApiFacilityPhotoTitle(galleryPhotoCaption(img)));
