...
function toGalleryImage(item, kind, index, rankScore = 0) {
  ...
  const title = String(
    item?.title || item?.galTitle || item?.imgname || item?.imgName || '',
  ).trim();
...
async function fetchTourApiGalleryInner(opts) {
  ...
  if (detail?.items?.length) {
    detail.items.forEach((it, i) => {
      const title = String(
        it?.title || it?.galTitle || it?.imgname || it?.imgName || '',
      ).trim();
  ...
  if (
    ...
    firstCommon &&
    !isTourApiFacilityPhotoTitle(firstCommon.title)
  ) {
