...
// ?ö® [Fix/New] ?òÏ†ï ?¥Ïú†: 
// 1. [Subtraction] Í∏∏Í≥† ?•Ìô©???úÏà†??Íº¨Íº¨Î¨?Î≤ÑÌäº??ÎØ∏ÎãàÎ©Ä??Í∞ÄÎ°úÌòï ?úÍ∑∏ Î¶¨Ïä§?∏Î°ú ?§Ïù¥?¥Ìä∏ ?†Ï?.
// 2. ?ö® [Fix/New] ÍµêÎëêÎ≥?Bridge) ?úÍ∞Å???õÏ? ?ÅÏö©: useSearchEngine?êÏÑú ?òÏñ¥??`isBridge: true` ?åÎûòÍ∑∏Î? Í∞êÏ?.
// 3. ?ö® [Fix/New] UI Î∂ÑÍ∏∞: ?ºÎ∞ò ?•ÏÜå???åÎ???Blue) + ?òÏπ®Î∞?Compass) / ÍµêÎëêÎ≥??•ÏÜå???†ÎπÑÎ°úÏö¥ Î≥¥Îùº??Fuchsia) + Î∞òÏßù??Sparkles) ?ÑÏù¥ÏΩòÏúºÎ°??§Ì??ºÏùÑ Î∞òÏ†Ñ?úÏºú ?¥Î¶≠???†ÎèÑ.
// 4. [Performance] React.memoÎ•??ÅÏö©?òÍ≥† Í¥Ä???•ÏÜå Í≥ÑÏÇ∞ Î°úÏßÅ??ÏµúÏ†Å??
...
const GalleryInfoView = React.memo(({ selectedPlace, selectedImg, onRelatedClick }) => {
    ...
    // ?ö® 4:1 (?∞Í?:ÍµêÎëêÎ≥? Î°úÏßÅ???ÅÏö©??Í≤∞Í≥ºÍ∞?5Í∞úÎ? Í∞Ä?∏Ïò¥
                ...
                {isPhotoMode ? (
                    ...
                    <div className="animate-fade-in space-y-10">
                        <p className="text-[14px] text-gray-300/90 leading-8 font-normal tracking-wide whitespace-pre-line">
                            {selectedPlace?.desc || "???•ÏÜå???Ä???ïÎ≥¥Í∞Ä ?ÖÎç∞?¥Ìä∏ Ï§ëÏûÖ?àÎã§."}
                        </p>
                        ...
                        <div className="pt-6 border-t border-white/5">
                            ...
                            {/* ?ö® [Fix/New] 2. Íº¨Íº¨Î¨??∞Í? ?•ÏÜå (4:1 ?úÍ∞Å???õÏ? ?ÅÏö©) */}
