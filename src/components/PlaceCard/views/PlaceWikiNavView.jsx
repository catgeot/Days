...
const PlaceWikiNavView = ({ wikiData, isWikiLoading, onNavClick, placeName }) => {
  const [activeSection, setActiveSection] = useState(null); // ?š¨ [New] ?œì„±???íƒœ
  ...
  return (
    // ?š¨ [Fix] ?„ì²´ ì»¨í…Œ?´ë„ˆë¥?h-full flex flex-colë¡?ë³€ê²½í•˜???˜ë‹¨ ê³ ì • ê¸°ë°˜ ë§ˆë ¨
    <div className="animate-fade-in flex flex-col h-full p-8 pb-6">
      ...
      {isWikiLoading ? (
        ...
        <>
            {/* ?š¨ [Fix] ëª©ì°¨ ?ì—­: ?´ìš©???˜ì¹˜ë©??ì²´ ?¤í¬ë¡?(flex-1 overflow-y-auto) */}
            ...
            {/* ?š¨ [Fix] ?˜ë‹¨ ê³ ì • ?ì—­ (mt-auto shrink-0) */}
        ...
        <p className="text-gray-400 text-sm flex-1">?±ë¡??ëª©ì°¨ê°€ ?†ìŠµ?ˆë‹¤.</p>
      )}
    </div>
  );
};
