...
// ?ö® [Fix/New] ?òÏ†ï ?¥Ïú†:
// 1. [Subtraction] useReport ?ÑÏó≠ ?ÅÌÉú ?òÏ°¥???ÑÏ†Ñ ?úÍ±∞.
// 2. [Routing] Log Î≤ÑÌäº??<button>?êÏÑú <Link to="/blog">Î°??ÑÎ©¥ ÍµêÏ≤¥?òÏó¨ ?•ÎßÅ???µÌï©.
// 3. [Safe Path] Í∏∞Ï°¥ openReportÎ°??ÑÎã¨?òÎçò location.id??Router??state={{ placeId: location.id }}Î°??àÏ†Ñ?òÍ≤å ?¥Í?.
// 4. [New] Íº¨Íº¨Î¨?Ï∂îÏ≤ú ?•ÏÜå ?¥Î¶≠ ???àÏ†Ñ?òÍ≤å ?¥Îèô?òÍ∏∞ ?ÑÌïú handleRelatedClick ?ºÏö∞???¥Îåë??Î°úÏßÅ Ï∂îÍ?.
// 5. [Performance] React.memoÎ•??ÅÏö©?òÏó¨ Î∂àÌïÑ?îÌïú Î¶¨Î†å?îÎßÅ Î∞©Ï?.
...
const PlaceChatPanel = React.memo(({ 
  ...
  const handleRelatedClick = (targetPlace) => {
      ...
      // 2. slugÍ∞Ä ?ÜÏùÑ Í≤ΩÏö∞ idÎ°?fallback
      if (targetPlace.id) {
          navigate(`/place/${targetPlace.id}`);
          return;
      }
      
      // 3. id???ÜÏùÑ Í≤ΩÏö∞ Ï¢åÌëúÎ°?fallback
      if (targetPlace.lat !== undefined && targetPlace.lng !== undefined) {
          navigate(`/place/city-${targetPlace.lat}-${targetPlace.lng}`);
          return;
      }

      console.warn("[Safe Path] ?ºÏö∞?ÖÏùÑ ?ÑÌïú ?ùÎ≥Ñ??slug, ID ?êÎäî Ï¢åÌëú)Í∞Ä ?ÜÏäµ?àÎã§.", targetPlace);
  };
  ...
  return (
    <div className={`flex flex-col transition-all duration-500
      ...
      <div className={`shrink-0 px-2 md:px-3 md:border-b md:border-white/5 bg-transparent z-20 py-2 md:py-3 xl:py-0 xl:h-20 flex flex-row md:flex-col xl:flex-row items-center md:items-stretch xl:items-center justify-between gap-2 md:gap-3 xl:gap-4 ${mediaMode === 'GALLERY' && selectedImg ? 'hidden md:flex' : 'flex'}`}>
         ...
         <div className="shrink-0 flex items-center justify-end md:justify-center xl:justify-end gap-1.5 md:gap-1.5 w-auto md:w-full xl:w-auto overflow-x-auto no-scrollbar">
            ...
            >
                ...
                <span className="text-[10px] md:text-[11px] font-bold">{mediaMode === 'WIKI' ? '?´Í∏∞' : 'Î∞±Í≥º'}</span>
            </button>
            ...
            {mediaMode === 'VIDEO' ? (
                ...
                >
                    ...
                    <span className="text-[10px] md:text-[11px] font-bold">Í∞§Îü¨Î¶?/span>
                </button>
            ) : (
                <button 
                ...
                >
                    ...
                    <span className="text-[10px] md:text-[11px] font-bold">?ÅÏÉÅ</span>
                </button>
            )}
         </div>
      </div>
      ...
      {!isChatMode && mediaMode !== 'WIKI' && (
          <div className="hidden md:block p-6 pt-4 bg-gradient-to-t from-[#05070a] via-[#05070a] to-transparent shrink-0 z-20">
              ...
              >
                  ...
                  <span className="text-sm text-gray-400 group-hover:text-gray-200 font-medium truncate">???•ÏÜå???Ä????Î¨ºÏñ¥Î≥¥ÏÑ∏??..</span>
      ...
      {/* ?ö® [New] Î™®Î∞î???ÑÏö© Íº¨Íº¨Î¨??∞Í? ?¨ÌñâÏßÄ ?∏ÌÑ∞ */}
