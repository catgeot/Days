            ...
            <div className={`flex gap-2 ${canStartTour || canPreviewFlightRoute ? 'flex-wrap' : ''}`}>
              {canPreviewFlightRoute && (
                <button
                  ...
                  aria-busy={!flightRouteInteractive}
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                >
                  <Plane
                    size={16}
                    className={
                      flightRouteInteractive || isFlightRoutePending ? 'text-sky-300' : 'text-gray-500'
                    }
                  />
                  <span
                    className={`text-xs font-bold ${
                      flightRouteInteractive || isFlightRoutePending ? 'text-sky-100' : 'text-gray-400'
                    }`}
                  >
