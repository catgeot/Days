...
const AiraloBannerWidget = ({ className = '', compact = false }) => {
    ...
    return (
        ...
        >
            <div className={`flex shrink-0 items-center justify-between gap-2 border-b border-cyan-100/90 bg-gradient-to-r from-cyan-50/90 to-white ${compact ? 'px-2 py-1' : 'px-2.5 py-1.5 md:px-3'}`}>
                <div className="flex min-w-0 flex-1 items-baseline gap-1.5">
                    <span className={`truncate font-black tracking-tight text-cyan-900 ${compact ? 'text-xs' : 'text-sm md:text-base'}`}>Airalo</span>
                <span className={`shrink-0 rounded-md bg-cyan-600/10 px-1.5 py-0.5 font-bold uppercase tracking-wide text-cyan-800 ${compact ? 'text-[9px]' : 'text-[10px] md:text-[11px]'}`}>
                    eSIM
                </span>
                </div>
                {compact ? null : (
