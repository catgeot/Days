...
const HolaflyBannerWidget = ({ className = '', compact = false }) => {
    ...
    return (
        ...
        >
            <div className={`flex shrink-0 items-center justify-between gap-2 border-b border-rose-100/90 bg-gradient-to-r from-rose-50/90 to-white ${compact ? 'px-2 py-1' : 'px-2.5 py-1.5 md:px-3'}`}>
                <div className="flex min-w-0 flex-1 items-baseline gap-1.5">
                    <span className={`truncate font-black tracking-tight text-rose-800 ${compact ? 'text-xs' : 'text-sm md:text-base'}`}>Holafly</span>
                <span className={`shrink-0 rounded-md bg-rose-600/10 px-1.5 py-0.5 font-bold uppercase tracking-wide text-rose-800 ${compact ? 'text-[9px]' : 'text-[10px] md:text-[11px]'}`}>
                    eSIM
                </span>
                </div>
                {compact ? null : (
            ...
            <div className={`relative overflow-hidden ${compact ? 'h-24 bg-white md:h-28' : ''}`}>
                <img
                    src={holaflyBannerImg}
                    alt="Holafly eSIM"
                    className={`${compact ? 'h-full w-full object-contain' : 'h-auto w-full object-cover object-center'} transition-transform duration-300 group-hover:scale-[1.02]`}
                    loading="lazy"
                    decoding="async"
                />
            </div>
