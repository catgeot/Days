...
for (const q of ['광천선굴', '광천성굴', '광천동굴', 'Gwangcheon Cave']) {
  ...
  assert.equal(hit.hub.hubId, 'pyeongchang', `${q} hub is pyeongchang`);
  assert.equal(hit.attraction.name, '광천선굴', `${q} canonical name`);
  assert.equal(hit.attraction.lat, 37.518306, `${q} Pyeongchang lat`);
}
assert.equal(resolveHubAttraction('광천동'), null, '광주 광천동 is not the cave');
