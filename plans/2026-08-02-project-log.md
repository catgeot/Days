...
**상태**: feature `cursor/mrt-package-strip-18d2` · 탐색 OK · 써머리 탭/모달 폐기

- **API**: Partner에 패키지 목록 없음 → `/pkc`·`/pkc/search` 딥링크 + mylink
- **탐색 ✅**: 일본 `q=일본` · 가족/유럽/휴양 promotionGroup · 선두=패키지 홈 (사람 QA 매끄러움)
- **써머리 패키지 탭·모달**: 목록 생성 불가 → **폐기** (다른 관점 재검토 대기)
- **유지**: 투어 모달 「패키지 더보기」딥링크 · `mrtPackageLinks` / smoke
