...
| **지금** | **#61 QA push** tip `ff38062f` · PR [#279](https://github.com/catgeot/Days/pull/279) · 통영팔경 3 + QA 1경 U투어·이순신공원 · **#62 경기 광주 결손 오버레이** | **main 병합 완료 ✅** · squash merge `53b21b00` · PR [#185](https://github.com/catgeot/Days/pull/185) |
...
**A 다음 제시어**: 사람은 같은 턴 Preview QA. 다음 채팅 = **다음 결손 허브**. `사람 Preview QA` 세션 생략. 피드백이 있으면 그때만 수정 세션. 전 주제: [`AGENTS.md`](../AGENTS.md) Cloud **같은 세션 QA**. 다음 허브 **광주8경 2**(경기 광주).
...
| 43 | `팔경 활용 #43, 함평 결손 오버레이` | A | **완료** · tip `5d812d09` · PR [#250](https://github.com/catgeot/Days/pull/250) · 함평8경 4 · QA 1경·2경 썸네일 분리 |
...
| 48 | `팔경 활용 #48, 동해 결손 오버레이` | A | **완료** · tip `3aeb5664` · PR [#258](https://github.com/catgeot/Days/pull/258) · 동해비경 3 · QA 용추·반석 Tour 125673 덮어쓰기 수정·망상 125713·어달 125708 |
...
| 55 | `팔경 활용 #55, 인천 결손 오버레이` | A | **완료** · tip `2652550e` · PR [#272](https://github.com/catgeot/Days/pull/272) · 인천9경 3 |
| 56 | `팔경 활용 #56, 양구 결손 오버레이` | A | **완료** · tip `777510d1` · PR [#272](https://github.com/catgeot/Days/pull/272) · 양구9경 3 |
| 57 | `팔경 활용 #57, 정읍 결손 오버레이` | A | **완료** · tip `4dd8d86a` · PR [#274](https://github.com/catgeot/Days/pull/274) · 정읍9경 3 |
| 58 | `팔경 활용 #58, 정선 결손 오버레이` | A | **완료** · tip `1de82ca5` · PR [#275](https://github.com/catgeot/Days/pull/275) · 화암8경 3 |
...
| 61 | `팔경 활용 #61, 통영 결손 오버레이` | A | **완료** · tip `ff38062f` · PR [#279](https://github.com/catgeot/Days/pull/279) · 통영팔경 3 + QA 1경 연화도 용머리 U투어 idx=16550(pstatic 403)·이순신공원 |
| 62 | `팔경 활용 #62, 경기 광주 결손 오버레이` | A | **열기 가능** · 광주8경 2 · Preview QA는 사람 병행(세션 생략) |
...
### §1.2 A #42 진도 결손 오버레이 (실행됨)

문경8경 4건은 사진·개요가 이미 채워져 건너뜀.
...
브랜치 cursor/palgyeong-use-e744 · Preview /qa/palgyeong-use
금지: JSON contentId 기입 · scenic 승격 · 축제 홈 파드 · feature에 plans/** 커밋
작업: 진도10경 사진·개요 없는 4건(조도관음도·의장대·돈대산·하조대)을 LOCAL_SCENIC_MEMBER_OVERLAYS로 보강. Preview /korea/theme/scenic?hub=jindo
```

### §1.2 A #43 함평 결손 오버레이 (실행됨)
...
### §1.2 A #43 QA 함평 엑스포 중복 썸네일 (실행됨)

```
팔경 활용 #43, 함평 엑스포 중복 썸네일
@plans/feature-handoff-index.md
@plans/2026-09-14-project-log.md
@plans/korea-local-scenic-use-plan.md
브랜치 cursor/palgyeong-use-e744 · Preview /qa/palgyeong-use
금지: JSON contentId 기입 · scenic 승격 · 축제 홈 파드 · feature에 plans/** 커밋
작업: 함평 1경·2경이 같은 3536105를 쓰던 GATEO 선정 엑스포 사진을 TourAPI 4065063(함평나비대축제)으로 분리. Preview /korea/theme/scenic?hub=hampyeong
```

### §1.2 A #44 해남 결손 오버레이 (실행됨)
...
브랜치 cursor/palgyeong-use-e744 · Preview /qa/palgyeong-use
금지: JSON contentId 기입 · scenic 승격 · 축제 홈 파드 · feature에 plans/** 커밋
작업: 정읍9경 사진·개요 없는 3건(동학농민혁명기념공원·용산호·월영습지와 솔티숲)을 LOCAL_SCENIC_MEMBER_OVERLAYS로 보강. Preview /korea/theme/scenic?hub=jeongeup
```

### §1.2 A #58 정선 결손 오버레이 (실행됨)
...
브랜치 cursor/palgyeong-use-e744 · Preview /qa/palgyeong-use
금지: JSON contentId 기입 · scenic 승격 · 축제 홈 파드 · feature에 plans/** 커밋
작업: 화암8경 사진·개요 없는 3건(거북바위·용마소·화표주)을 LOCAL_SCENIC_MEMBER_OVERLAYS로 보강. Preview /korea/theme/scenic?hub=jeongseon
```

### §1.2 A #59 태백 결손 오버레이 (실행됨)
...
브랜치 cursor/palgyeong-use-e744 · Preview /qa/palgyeong-use
금지: JSON contentId 기입 · scenic 승격 · 축제 홈 파드 · feature에 plans/** 커밋
작업: 태백8경 사진·개요 없는 3건(장성하부고생대화석산지·용연굴·절골마을관리휴양지)을 LOCAL_SCENIC_MEMBER_OVERLAYS로 보강. Preview /korea/theme/scenic?hub=taebaek
```

### §1.2 A #60 의정부 결손 오버레이 (실행됨)

```
팔경 활용 #60, 의정부 결손 오버레이
@plans/feature-handoff-index.md
@plans/2026-09-18-project-log.md
@plans/korea-local-scenic-use-plan.md
브랜치 cursor/palgyeong-use-e744 · Preview /qa/palgyeong-use
금지: JSON contentId 기입 · scenic 승격 · 축제 홈 파드 · feature에 plans/** 커밋
작업: 의정부8경 사진·개요 없는 3건(수락산 도정봉·의정부경전철·의정부제일시장)을 LOCAL_SCENIC_MEMBER_OVERLAYS로 보강. Preview /korea/theme/scenic?hub=uijeongbu
```

### §1.2 A #61 통영 결손 오버레이 (실행됨)
...
브랜치 cursor/palgyeong-use-e744 · Preview /qa/palgyeong-use
금지: JSON contentId 기입 · scenic 승격 · 축제 홈 파드 · feature에 plans/** 커밋
작업: 통영팔경 사진·개요 없는 3건(남망산공원·한산도제승당·통영운하 야경)을 LOCAL_SCENIC_MEMBER_OVERLAYS로 보강. Preview /korea/theme/scenic?hub=tongyeong
```

### §1.2 A #62 경기 광주 결손 오버레이 (다음)

```
팔경 활용 #62, 경기 광주 결손 오버레이
@plans/feature-handoff-index.md
@plans/2026-09-18-project-log.md
@plans/korea-local-scenic-use-plan.md
브랜치 cursor/palgyeong-use-e744 · Preview /qa/palgyeong-use
금지: JSON contentId 기입 · scenic 승격 · 축제 홈 파드 · feature에 plans/** 커밋
작업: 광주8경 사진·개요 없는 2건(무갑산·송정사)을 LOCAL_SCENIC_MEMBER_OVERLAYS로 보강. Preview /korea/theme/scenic?hub=gwangju_gi
```
