...
## 국내축제 — 검색 UX (모바일 확대·엔터·겨울 축제)

**상태**: ⏳ 사람 QA 대기

| | |
|--|--|
| 모바일 | 검색 input `text-base`(16px) — iOS 포커스 확대 완화 |
| 엔터 | form submit → 입력창만 접기·쿼리 유지 · 헤더 X는 검색 초기화 |
| 범위 | 검색 활성 시 시간 탭 무시 · 롤링 12개월 `title`·`addr1` · 지역·테마 유지 |
| 테마 | 산천어 칩 키워드 추가 · 겨울에 얼음 |

**QA**: 모바일에서 검색 입력 후 blur/엔터 시 화면 확대 여부 · `빙어`/`산천어` 결과 · 헤더 X로 해제
