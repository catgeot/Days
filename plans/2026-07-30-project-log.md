...
## 국내 TNA Phase 4 — 일정 연결 SSOT 1차

**상태**: ✅ `main` `5ca36aa` (push 대기 · 사람 요청 시)

| | |
|--|--|
| 등재 | 경주·단양·춘천·통영·거제·가평·제주 (hubId+한글) |
| LIVE | 인근 키워드 전부 n≥1 · 풍부 hub `nearbyExpanded=false` |
| 스모크 | 경주·제주 칩 true · 부산 false · `MRT_TNA_SMOKE_LIVE=1` PASS |
| 유지 | Edge ≤3 자동보강 · UI 비주얼 |

**QA**: 경주/제주 명소 — 본지 투어 많아도 「주변 지역 더보기」칩 · 부산은 칩 없음

---

## 국내 TNA Phase 4 — 일정 연결 SSOT 2차

**상태**: ✅ `main` `0c1f17d` (push 대기 · 사람 요청 시)

| | |
|--|--|
| 등재 | 영월·남해·담양·강릉·평창·홍천 (hubId+한글 · C표 합계 13) |
| LIVE | 인근 키워드 전부 n≥1 · 풍부 hub `nearbyExpanded=false` |
| 스모크 | 강릉·남해 칩 true · 부산 false · `MRT_TNA_SMOKE_LIVE=1` PASS |
| 유지 | Edge ≤3 자동보강 · UI 비주얼 |

**QA**: 강릉/남해/영월 등 — 본지 풍부해도 「주변 지역 더보기」칩
...
**상태**: ✅ `main` `81ec0f9` (push 대기 · 사람 요청 시)
...
**상태**: ✅ `releaseNotes` `2026-07-30` · push

| | |
|--|--|
| 제목 | 한국의 축제를 한곳에서 살펴볼 수 있어요 |
| 항목 | 국내 진입·지도/목록 · 지역·테마·검색·내 주변 · 상세·여행지 · 별표 |

## 국내축제 — 리스트 「크게」토글

**상태**: ✅ 사람 QA OK · `main` 커밋·푸시

| | |
|--|--|
| UI | `/korea` 리스트 헤더 「크게」↔「기본」 · 썸네일·제목·여백 확대 |
| 파일 | `src/pages/Korea/index.jsx` |

**QA**: 크게→행 확대 · 기본→복귀 · 사람 확인 OK
