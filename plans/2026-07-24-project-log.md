...
## 써머리 투어 모달 — 카드 다른 동작 시 닫기

**상태**: ✅ 수정 · 커밋

- 증상: PC 투어 패널이 뜬 채 「이 지역 보기」「3D 투어」등이 모달 뒤에서 실행됨
- 대응: `GlobeTourStrip`/`GlobeStayStrip`이 `close` 노출 · `HomePlaceCardSummary`에서 이 지역/3D투어/항공·확장·채팅 전에 패널 닫기
- QA: 써머리 → 투어 찾기 연 뒤 「이 지역 보기」「3D 투어」「항공 경로」·카드 확장 시 패널이 먼저 닫히는지
