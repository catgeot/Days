...
- **스케치**: 좌측 GYG·740 확장 **폐기** · 문서 목차(+패키지)로 원복
- **홈 투어 모달**: 맨 위 버튼 고대비(zinc-950/흰글씨) · iframe 로딩 중 스피너·안내 ·「더보기」는 로드 후
- 릴리스: QA 후 feature 초안만 · `releaseNotes.js` 미반영

### QA 체크
1. 플래너 map_poi — 투어 3개 + 겟유어가이드 더보기 (시티 위젯 없음)
2. 여행 스케치 좌측 — 목차만 (투어 위젯·740 없음)
3. 홈「투어 찾기」— 로딩 스피너/문구 → 목록 → 더보기 · 맨 위 버튼 시인성
