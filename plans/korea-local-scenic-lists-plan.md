...
| | 자동 오케 (F) | 사람 개입 |
|--|---------------|-----------|
| **루프** | 큐 다음 ⬜ R → 지휘자가 워커2 기동 → 직렬 머지 → VERIFY → feature push → **같은 지휘자가 다음 R**(Cloud 중첩 후임 금지) | **§6.2 정지** · **I#** · 주제 종료·PR merge |
...
| 2 | **Cloud 중첩 후임** | Cloud는 메인→L1만. 후임 Task에게 워커 기동을 맡기면 StepContext·워커 불가. **같은 지휘자가 워커 재기동**. |
...
| 조건 | 행동 |
|------|------|
| 컨텍스트 **~40% 이하** 여유 | **같은 지휘자**가 큐 다음 R 즉시 착수 (§C 반복) — Cloud 기본 |
| 컨텍스트 **부족** · Desktop | **컨트롤러가** 후임 지휘자 L1 기동 (`run_in_background: false`) · 프롬프트 = §E |
| 컨텍스트 **부족** · Cloud | 중첩 후임 **금지**. 일지 §4.1 + 복구 제시어 · **새 Cloud 런**(같은 고정 브랜치) |

**후임(Desktop L1) 띄운 뒤** 현 지휘자는 tip 추가 작업 중단. Cloud는 후임 Task 없이 연장하거나 런을 닫는다.
