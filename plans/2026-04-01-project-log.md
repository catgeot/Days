...
### ✅ 4. 피드백 반영 및 디테일 최적화
- **지명 한글화**: `onStyleData` 이벤트를 활용해 지도 내 모든 레이어의 `text-field`를 순회하며 `name_ko` 속성을 최우선 반영하도록 조치. (영문 및 현지어 폴백 지원)
- **컴팩트 UI & 풀스크린 복구**: 좌측 상단 컨트롤 버튼 그룹의 세로 패딩을 줄이고(`py-1.5`), PC/모바일 공통으로 사용할 수 있도록 풀스크린 버튼을 분리해 우측 상단에 고정 배치.
- **시네마틱 스케일 조정**: 초기 우주 뷰의 Zoom 레벨을 `1.2`에서 `0.5`로 더 줄이고, `flyTo` 비행 애니메이션 시간(duration)을 8초에서 12초로 늘려 더욱 극적인 우주 진입 연출 구현.

### 📦 커밋 내역
- `feat(wiki): 맵박스 시네마틱 FlyTo 애니메이션, 지도 스타일 토글 및 3D 빌딩 레이어 연동 적용`
- `fix(wiki): 피드백 반영 - 지명 한글화, 애니메이션 속도 및 줌 레벨 조정, 컨트롤 UI 컴팩트화`
