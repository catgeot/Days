...
**상태**: **계획 ✅** · 구현 **다음 세션** · QA: 서울 GPS→GMP→파리 `GMP~PEK~CPH~CDG` 오탐 확인
...
| 읽을 것 (3) | 금지 (3) |
|-------------|----------|
| 본 절 + [`flight-origin-metro-gateway-plan.md`](./flight-origin-metro-gateway-plan.md) | `travelSpots.js` 전체 |
| [`flightCinemaOriginSearch.js`](src/pages/Home/lib/flightCinemaOriginSearch.js) · [`rentalAirportHubs.js`](src/utils/rentalAirportHubs.js) | `travelSpotAirports.json` 직접 편집 |
| smoke 계획 표 · `globeFlightCinema.js` explicitDirect ICN gate(참고만) | Edge deploy · overrides `generate:airports` |

**제시어 (다음 세션 첫 메시지)**

```
@plans/2026-06-29-project-log.md @plans/flight-origin-metro-gateway-plan.md
항공 출발지 Metro Gateway 승격 세션 1 실행 — flightOriginMetroGateways.js + geolocation + smoke
```

**완료 기준**: `smoke:flight-origin-gateway` 5/5 · `smoke:flight-route-baseline` 13/13 · 서울 GPS→ICN·상하이→PVG·보홀 TAG 승격 없음
