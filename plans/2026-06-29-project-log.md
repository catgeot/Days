...
**계획 SSOT**: [`flight-route-non-icn-routing-plan.md`](./flight-route-non-icn-routing-plan.md) · **세션 1부터** A→C→E(일부) · **세션 2** B→E→Edge
...
| 단계 | 작업 |
|------|------|
| A | `getFlightRouteWaypoints` origin ICN 게이트 · `globeFlightCinema.js` 호출부 |
| C | `paris` `flightRouteHubIatas: []` → `generate:airports` → `audit:airports` |
| E | smoke `icn-paris-direct` · `icn-grand-canyon-waypoint` · `audit:flight-arcs` · 8/8 유지 |
...
| QA (S1) | 기대 |
|---------|------|
| ICN → paris | `ICN → CDG` 직항 |
| ICN → grand-canyon | Pacific waypoint·LAX hub 회귀 없음 |
| BDA → grand-canyon | arc 도쿄 **없음** (SGF는 S2까지 허용) |

**세션 1 제시어**

```
항공경로-세션1 @plans/2026-06-29-project-log.md @plans/flight-route-non-icn-routing-plan.md
A+C+E(일부) 실행. ICN 파리·ICN 그랜드캐니언 QA. smoke 8/8 유지.
```

---

### 세션 2 — B + E(완료) + Edge · **S1 완료 후**
