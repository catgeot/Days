...
**상태**: **S1 ✅** · smoke **5/5** · `smoke:flight-route-baseline` **13/13** · **S2** 브라우저 QA 대기
...
| 읽을 것 (3) | 금지 (3) |
|-------------|----------|
| 본 절 + [`flight-origin-metro-gateway-plan.md`](./flight-origin-metro-gateway-plan.md) §세션 2 | `travelSpots.js` 전체 |
| 브라우저 GPS QA: 서울→파리 ICN·상하이 PVG·보홀 TAG | Edge deploy · overrides `generate:airports` |
| GMP 수동 선택 회귀 · 릴리스 노트 초안(합의 후) | `travelSpotAirports.json` 직접 편집 |

**제시어 (S2)**

```
@plans/2026-06-29-project-log.md
항공 출발지 Metro Gateway S2 — 브라우저 GPS QA + 릴리스 노트 초안
```
