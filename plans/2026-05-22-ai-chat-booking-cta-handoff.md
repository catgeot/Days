...
**상태**: **구현·사용자 QA Pass** (2026-05-29) · **access L2 출발지 검색 교체** (2026-07-11 QA)

| 구분 | dock | 입력 |
|------|------|------|
| **모바일** (`max-md`) | L1 `mobileLabel` 단축 · wrap · 좌 **~60%** | 우 **~40%** · 포커스/타이핑 시 **칩 숨김·입력 전폭** |
| **데스크톱** (`md+`) | 전폭 wrap · 풀 L1 라벨 | dock **아래** 전폭 · `또는 직접 입력…` |

- **플래너**: L1 칩 제거 → 헤더 **📋 플래너 보기** 상시 (`omitPlanner`).
- **L2**: 「세부 질문을 골라보세요」 제거 · **← 주제 바꾸기** cyan pill.
- **access L2 (2026-07-11)**: 서울·부산·인천 **고정 칩 제거** → [`FlightOriginSelector`](../src/pages/Home/components/FlightOriginSelector.jsx) `chat`/`chat-header` (장소카드·Bar와 동일 · `gateo.flightOriginIata` 공유) · **물어보기**/선택 시 `access_origin` 발화 · 페리 칩만 보조 · 자유 텍스트 입력은 access 독에서 숨김.
- **잔여 UX**: 일반 L1/L2 모바일 **세로 스택**·칩 터치 영역 등 — 일지 `2026-07-11` 「다음 세션」.
...
| 7 | `getTripcomFlightAdForModal` — 채팅·`forceModal` 시 **데스크톱 900×200** / 모바일 320×480 · `TripcomFlightSearchModal` 크기 prop |
| 8 | 「교통 · 티켓」 Trip CTA 아래 `getMooniPlannerFlightGuideLabel` → `#planner-prep-flight` |
| 9 | 「렌터카·픽업」칩 — [`MooniTransportPlannerLinks`](../src/components/chat/MooniTransportPlannerLinks.jsx) 2버튼: `#planner-arrival-transfer`(또는 `#planner-rental-pickup`) + `#planner-local-transport` · `BookingActionCards`·`MooniPlannerFollowUp` |
...
| `prep_transport` | cyan 「공항→목적지 이동」(또는 「렌터카·픽업 기준」) + 회색 「교통·패스 안내」 |
...
| P3 | 「렌터카·픽업」칩 | follow-up **2버튼** — 공항 이동(cyan) · 교통·패스(회색) |
| P4 | 「항공권」칩 · 교통·티켓 Trip | 위젯 모달 · 데스크톱 900×200 · 아래 「플래너에서 {지} 항공권 안내 보기」 |
| P5 | 위젯 내 출발·도착 | ICN / BDA (동일 코드 X) |
...
| [`MooniTransportPlannerLinks.jsx`](src/components/chat/MooniTransportPlannerLinks.jsx) | 렌터카·픽업 칩 → 플래너 공항 이동 + 교통·패스 2링크 (§2.15) |
| [`partnerNavigation.js`](src/components/PlaceCard/common/partnerNavigation.js) | `getTripcomFlightAdForModal` — 모달 ad 크기 분기 (§2.15) |
| [`TripcomFlightSearchContext.jsx`](src/components/PlaceCard/tabs/planner/TripcomFlightSearchContext.jsx) | 채팅·플래너 Trip 항공 위젯 모달 · `forceModal` · 데스크톱 배너 (§2.15) |
