...
**증상**: bound 주제 칩 dock + 입력창이 동시에 하단에 노출되어 모바일 뷰포트가 협소.

**조치**: `ChatModal` — `max-md`에서 bound dock 표시 중 입력창 **숨김** · 「✏️ 직접 입력하기」(`accessInputHint`) 탭 시에만 표시 · 전송·blur·access L1 복귀 시 접힘. 데스크톱(`md+`)은 기존 상시 노출 유지.

### 2.12 채팅·Trip CTA 출발지 확대 — 로드맵 (Phase 2d · S8+)

**상태**: **합의·로드맵만** (2026-05-29) · 코드 **미착수**  
**착수 조건**: MOONi·PlaceCard **채팅 기능 완료**(§2.11 2단 칩·BookingActionCards·intent/CTA QA) **후**
