...
**상태**: **구현·사용자 QA Pass** (모바일·데스크톱 D0 · 2026-07-11) · **access L2 출발지 검색 교체** (2026-07-11 QA)

| 구분 | dock | 입력 |
|------|------|------|
| **모바일** (`max-md`) | L1 가로 스크롤 1줄 · 세로 스택 입력 · 헤더 Compact | 미사용 `h-8` · 포커스 확대 · 빈 blur 시 칩 복귀 |
| **데스크톱** (`md+`) | L1 **1줄 가로 스크롤** · 헤더 Compact · bound 시 **칩‖입력** 한 줄 | 칩 오른쪽 · cyan 테두리 시인성 · access는 독 내 출발 검색 |

- **플래너**: L1 칩 제거 → 헤더 **📋 플래너 보기** 상시 (`omitPlanner`).
- **L2**: **「다른 주제」 pill** + L1 라벨 맥락.
- **access L2 (2026-07-11)**: 고정 도시 칩 제거 → `FlightOriginSelector` `chat`/`chat-header`.
- **모바일·데스크톱 D0 QA Pass (2026-07-11)**. **잔여**: P2/P3 — 일지 `2026-07-11`.
