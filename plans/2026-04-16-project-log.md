...
### 5. AI 할루시네이션 URL 생성 방어 강화 (네팔 에베레스트 악성 링크 차단)
- 사용자가 에베레스트 툴킷에서 "입산 허가 및 가이드 고용" 버튼 클릭 시 도박 사이트로 연결되는 심각한 보안 이슈 발견.
- **원인 분석**: AI가 `pre_travel` 배열에 임의의 URL(`https://www.welcomenepal.com/...`)을 생성했으며, 해당 도메인이 악성 사이트로 바뀌었거나 처음부터 잘못된 링크였음.
- **즉각 조치**:
  - 네팔 비자 링크의 키워드에 '에베레스트', 'EBC', '베이스캠프' 추가하여 검색 정확도 향상.
  - `update-place-toolkit` Edge Function의 프롬프트에 **URL 생성 규칙 명시**:
    > "pre_travel 배열의 url 필드는 반드시 해당 국가의 공식 정부/이민국 웹사이트만 사용. 입산 허가, 투어 패키지, 가이드 고용 등 현지 에이전시 대행이 필요한 항목은 url을 null로 설정하고 advice에 설명만 작성. 절대로 임의의 관광 웹사이트나 여행사 URL을 생성하지 마세요."
  - Edge Function 재배포 완료.
- **효과**: 향후 AI가 새로운 여행지 데이터를 생성할 때, 검증되지 않은 임의의 URL을 만들어내지 못하도록 원천 차단. 현지 에이전시 대행 항목은 URL 없이 텍스트 설명만 제공.
