...
- **브랜치** `cursor/visitor-growth-1f90` · tip `9615ca0b` · PR [#247](https://github.com/catgeot/Days/pull/247)
- **완료**: 상단 공간을 점유하던 히어로 박스 및 모바일 우측 중복 아이콘(로그인/AI) 제거 → **기존 배포본 미니멀 UI 100% 복원**. 상단 공간 점유 제로.
  ...
  - **아이콘 import 복원**: 미사용 정리 중 누락되었던 데스크톱 푸터 `User`, `LogOut` 아이콘 import 복원(런타임 에러 방지).
...
- **VERIFY**: `npm run build` PASS
- **Preview** https://www.gateo.kr/qa/visitor-growth → git Preview `/`
- **QA**: `/about`·`/pricing`·`/product`·존재하지 않는 URL → 홈. 장소 없이 하단 「AI와 여행 대화」→ 환영 멘트·칩 4종. 여행지 선택 후 **모바일**: 헤더 컴패스(🧭) 또는 칩 위 「일반 탐색」→ 범용 칩 복귀.
