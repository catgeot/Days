...
**상태**: ✅ 공식 · 2026-09-07 (**v2.4** — 지휘자 1 + 워커 N(기본 2) · Cloud는 중첩 후임 금지)  
...
세대가 끝나면 지휘권은 **워커를 띄울 수 있는 깊이**의 후임 지휘자에게 넘긴다. Cloud에서 중첩 Task 후임은 워커를 못 띄우므로 **같은 지휘자가 워커만 재기동**하는 연장이 기본이다.
...
- **같은 지휘자 연장**: 컨텍스트 여유 시 후임 없이 워커 N만 재기동. 사람 제시어 불필요. Cloud의 **기본** 연속 경로.
...
**세대 권장 상한 (같은 지휘자)**  
- 워커N 라운드 **1~2회** 후 Desktop은 컨트롤러가 다음 지휘자. Cloud는 **연장 우선**(중첩 후임 금지).  
...
트리거: 대화가 대략 **절반**을 넘었거나, **세대**(워커 N 머지+VERIFY)를 1~2회 끝낸 뒤.  
Cloud는 이 시점에도 **같은 지휘자 연장**이 가능하면 연장을 고른다.
...
2. **Cloud**  
   - 컨텍스트 여유 → 후임 Task **없이** 워커 N 재기동.  
   - 여유 없음/런 종료 → 복구 제시어를 일지에 남긴다. **중첩 Task 후임 기동 금지**.  
   - 새 지휘자 = **새 Cloud 런**(같은 고정 브랜치). `environment: cloud` 형제로 띄우면 브랜치가 갈라져 Preview·고정 브랜치 규칙과 충돌하므로 **쓰지 않음**.
...
제시어: `오케스트레이터` + `팔경contentId` · 큐 다음 ⬜ · 플랜 §9 복붙 블록.
...
| 팔경 Tour contentId | `오케스트레이터` + `팔경contentId` + [`korea-local-scenic-contentid-queue.md`](./korea-local-scenic-contentid-queue.md) · 브랜치 **`cursor/palgyeong-cid`** · 「§5.7 · P0 · DB-only 우선 · 지휘자=Cloud 메인 · 워커2 Foreground · 중첩 후임 금지 · 워커 병렬 LIVE 금지 · 429 정지 · UI 금지 · §3.3·§3.4·§3.5」 |
