...
## 사모아 3D 투어 시선 및 비행 경로 교정 (2026-06-04)

- **문제**: 사모아(Samoa) 투어의 마지막 착륙 시, 카메라가 바다 쪽을 향하고 있어(Bearing 120도, 동남동 방향) 내륙 지형 대신 바다가 중심이 되는 현상 발생.
- **조치**: [`globeLandmarks.json`](../src/pages/Home/data/globeLandmarks.json)의 사모아 `keyframes` 경로와 앵글을 전면 수정함.
  - 섬의 서쪽(`[-172.10, -13.75]`)에서 출발하여 우폴루 섬의 북부 해안선을 따라 동쪽으로 매끄럽게 비행하도록 중간 프레임 좌표(`center`)들을 징검다리처럼 배치함.
  - 최종 목적지인 아피아(Apia, `[-171.77, -13.83]`) 앞바다에 도착했을 때, 카메라가 바다가 아닌 **남쪽 내륙(Bearing 180도)**을 정면으로 바라보도록 회전(orbit)시킴.
  - 이를 통해 아피아 해안선과 그 뒤로 펼쳐진 우폴루 섬의 웅장한 화산 지형을 완벽히 조망하며 투어를 마무리할 수 있도록 최적화 완료. 랜드마크 이름도 "Upolu & Apia Coast"로 세밀화함.
