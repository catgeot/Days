...
### 갤러리 순서 (`usePlaceGallery` · 캐시 v1.20)

**국내** (`isDomesticKoreaLocation` · curated SSOT)

1. sessionStorage  
2. **`place_stats`** — 히트 시 **즉시 표시**(스켈레톤 대기 없음)  
   - soft: TourAPI 우세 DB → 스킵 후 스톡 재수집(공지천)  
   - curated hub: 스톡 고착 DB → 스킵 후 Tour 우선(양구 Unsplash 오매칭 방지)  
   - 갤러리 탭만 **7일 스톡 SWR**: Unsplash page 1을 백그라운드 병합 · **hero(0번)·`image_url` 유지** · Tour 우세는 SWR 생략 · `thumbnailOnly`(써머리·검색 카드)는 SWR/LIVE 없음  
   - **더보기(append)는 DB 미반영**
...
**해외**: session → DB 즉시 → (갤러리 탭 7일 SWR) → miss 시 Unsplash/Pexels (**TourAPI 미호출**)
